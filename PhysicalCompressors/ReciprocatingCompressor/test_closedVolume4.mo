...
//   parameter Modelica.SIunits.Temperature T_in = 250;
...
equation
  ...
  if Fluid_in.p > p_gas then //Suction
    der(m_gas) = 1e-3*Aeff_in*sqrt(2*d_gas*abs((Fluid_in.p - p_gas)));
    U_flow = der(m_gas) * h_in;
    der(m_in) = der(m_gas);
    der(m_out) = small;
    c1 = 6.18;
  elseif p_gas > Fluid_out.p then //Discharge
    der(m_gas) = -1e-3*Aeff_out*sqrt(2*d_gas*abs((Fluid_out.p - p_gas)));
