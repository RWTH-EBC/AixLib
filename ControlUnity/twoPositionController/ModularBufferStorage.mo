...
model ModularBufferStorage
       ...
       parameter Modelica.SIunits.VolumeFlowRate V_flow_nominal=m_flow_nominal/Medium.d_const;
          parameter Modelica.SIunits.PressureDifference dp_nominal=7.143*10^8*exp(-0.007078*QNom/1000)*(V_flow_nominal)^2;
            ...
            parameter Modelica.SIunits.MassFlowRate m_flow_nominal=QNom/(Medium.cp_const*dTWaterNom);
             ...
             parameter Modelica.SIunits.Time t(displayUnit="min")=3600
  ...
  AixLib.Fluid.Storage.BufferStorage bufferStorage(
    ...
    TStart=303.15,
    x=1)           annotation (Placement(transformation(extent={{20,-26},{-20,24}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T(
        displayUnit="K") = TAmb)                                                      annotation(Placement(transformation(extent={{-96,-6},
            {-84,6}})));
  Modelica.Fluid.Interfaces.FluidPort_a fluidportTop1
    annotation (Placement(transformation(extent={{50,90},{70,110}})));
  Modelica.Fluid.Interfaces.FluidPort_a fluidportBottom2
    annotation (Placement(transformation(extent={{-70,-110},{-50,-90}})));
  Modelica.Fluid.Interfaces.FluidPort_b fluidportBottom1
    annotation (Placement(transformation(extent={{50,-110},{70,-90}})));
  Modelica.Fluid.Interfaces.FluidPort_b fluidportTop2
    annotation (Placement(transformation(extent={{-70,90},{-50,110}})));
  Modelica.Blocks.Interfaces.RealInput TLayer[n]
    "Temperatures of the different layers of the buffer storage"
    annotation (Placement(transformation(extent={{-80,18},{-120,58}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_b
    annotation (Placement(transformation(extent={{90,-10},{110,10}})));
equation
  connect(bufferStorage.fluidportTop2, fluidportTop2) annotation (Line(points={{-6.25,
          24.25},{-6.25,80},{-60,80},{-60,100}}, color={0,127,255}));
  connect(bufferStorage.fluidportTop1, fluidportTop1) annotation (Line(points={{7,24.25},
          {7,80},{60,80},{60,100}}, color={0,127,255}));
  connect(fixedTemperature.port, bufferStorage.heatportOutside) annotation (Line(
        points={{-84,0},{-52,0},{-52,0.5},{-19.5,0.5}}, color={191,0,0}));
  connect(bufferStorage.fluidportBottom1, fluidportBottom1) annotation (Line(points={
          {6.75,-26.5},{6.75,-80},{60,-80},{60,-100}}, color={0,127,255}));
  connect(bufferStorage.fluidportBottom2, fluidportBottom2) annotation (Line(points={
          {-5.75,-26.25},{-5.75,-80},{-60,-80},{-60,-100}}, color={0,127,255}));
  connect(bufferStorage.TLayer, TLayer) annotation (Line(points={{-20,13},{-58,13},{-58,
          40},{-100,40},{-100,38}}, color={0,0,127}));
  connect(bufferStorage.port_b, port_b) annotation (Line(points={{24.5,-1},{61.25,-1},
          {61.25,0},{100,0}}, color={0,127,255}));
