      ...
      model FloorHeatingdisVolume
        ...
        Modelica.Fluid.Sensors.TemperatureTwoPort TReturn(redeclare package
            Medium =
              Medium)
          annotation (Placement(transformation(extent={{50,-36},{70,-16}})));
        AixLib.Fluid.MixingVolumes.MixingVolume vol[dis](
          redeclare package Medium = Medium,
          energyDynamics=system.energyDynamics,
          T_start=T0,
          V=VWater,
...
"),           Line(points={{0,-92},{4,-88}},   color={28,108,200}),
      ...
      model FloorHeatingdisVolume
        ...
        AixLib.Utilities.Interfaces.Adaptors.ConvRadToCombPort convRadToCombPort2
          annotation (Placement(transformation(
              extent={{7,-6},{-7,6}},
              rotation=90,
              origin={1,-72})));
        AixLib.Utilities.Interfaces.RadPort radPort1
          annotation (Placement(transformation(extent={{-16,-90},{4,-110}})));
...
"),         Line(points={{0,-66},{0,-92}},   color={28,108,200}),
    ...
    parameter
        AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.HeatCapacityPerArea
        cTop;
    parameter
        AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.HeatCapacityPerArea
        cDown;
    ...
    model PanelHeatingSingle
      ...
      Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature fixedTemperature2
        annotation (Placement(transformation(
            ...
            rotation=180,
            origin={72,64})));
      AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.HeatConductionSegment
        panel_Segment1(
        kA=67,
        mc_p=16440,
    ...
    model PanelHeating
      ...
      extends Modelica.Fluid.Interfaces.PartialTwoPort;
         ...
         parameter AixLib.DataBase.ActiveWalls.ActiveWallBaseDataDefinition floorHeatingType=
          AixLib.DataBase.ActiveWalls.JocoKlimaBodenTOP2000_Parkett()
      ...
      Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a thermConv annotation (
          Placement(transformation(extent={{4,48},{24,68}}), iconTransformation(
              extent={{4,30},{24,50}})));
      AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.PanelHeatingSegment panelHeatingSegment1(
        redeclare package Medium = Medium,
        each final eps=eps,
        each final T0=T0,
      ...
      AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.PanelHeatingSegment panelHeatingSegment2(
        redeclare package Medium = Medium,
        each final eps=eps,
        each final T0=T0,
        each final kTop=kTop_nominal,
      ...
      AixLib.Fluid.HeatExchangers.ActiveWalls.BaseClasses.PanelHeatingSegment panelHeatingSegment3(
        redeclare package Medium = Medium,
        each final eps=eps,
        each final T0=T0,
        each final kTop=kTop_nominal,
  ...
  package AddParameters
    ...
    model PanelHeatingElement
      ...
      AixLib.ThermalZones.HighOrder.Components.Walls.BaseClasses.SimpleNLayer simpleNLayer_floor(
        ...
        T0=T0) annotation (Placement(transformation(
            ...
            rotation=90,
            origin={0,52})));
      AixLib.Utilities.HeatTransfer.HeatToRad twoStar_RadEx_floor(
        rad(T(start=T0)),
        conv(T(start=T0)),
        A=A,
        eps=eps_floor) annotation (Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=90,
            origin={-30,76})));
      ...
      AixLib.Utilities.HeatTransfer.HeatToRad twoStar_RadEx_ceiling(
        rad(T(start=T0)),
        conv(T(start=T0)),
        A=A,
        eps=eps_ceiling) if Ceiling annotation (Placement(transformation(
            extent={{10,-10},{-10,10}},
            rotation=90,
            origin={-24,-70})));
      AixLib.Utilities.Interfaces.RadPort radport_ceiling if Ceiling
    ...
    model PanelHeatingCircuit "One Circuit in a Panel Heating System"
      ...
      parameter Modelica.SIunits.Emissivity eps_ceiling = 0.95 "Emissivity of ceiling surface" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
        ...
        PanelHeatingNew.AddParameters.ZoneSpecification.ZoneDefinition ZoneType=
    ...
    model PanelHeatingRoom
      ...
      final parameter Modelica.SIunits.HeatFlux q = Q_Nf / A "set value for panel heating heat flux";
      ...
      parameter Boolean Ceiling "false if ground plate is under panel heating" annotation (Dialog(group = "Floor Layers"), choices(checkBox=true));
      parameter Integer n_ceiling( min = 1) = 3 "Number of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = false));
      parameter Modelica.SIunits.Thickness d_ceiling[n_ceiling] = zeros(n_ceiling) "Thickness of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.Density rho_ceiling[n_ceiling] = zeros(n_ceiling) "Density of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.ThermalConductivity lambda_ceiling[n_ceiling] = zeros(n_ceiling) "Thermal conductivity of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.SpecificHeatCapacity c_ceiling[n_ceiling] = zeros(n_ceiling) "Specific heat capacity of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.Emissivity eps_ceiling = 0.95 "Emissivity of ceiling surface" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
    ...
    equation
      ...
      for i in 1:CircuitNo loop
        ...
        connect(ports_a[i], roomCircuit[i].port_a) annotation (Line(points={{-100,0},
                {-16,0}},                                                                      color={0,127,255}));
        connect(roomCircuit[i].radport_floor, radport_floor)
          annotation (Line(points={{-2.32,18},{-2,18},{-2,42},{-20,42},{-20,100}},color={95,95,95}));
        connect(heatport_floor, roomCircuit[i].heatport_floor)
          annotation (Line(points={{18,96},{20,96},{20,42},{6.32,42},{6.32,17.28}},
                                                                                color={191,0,0}));
        if Ceiling then
          connect(roomCircuit[i].radport_ceiling, radport_ceiling)
            annotation (Line(points={{-2.32,-16.92},{-2.32,-32},{-20,-32},{-20,-98}},
                                                                                  color={95,95,95}));
        end if;
        connect(roomCircuit[i].heatport_ceiling, heatport_ceiling)
          annotation (Line(points={{6.32,-17.64},{6.32,-32},{24,-32},{24,-98},{26,-98}},
                                                                                     color={191,0,0}));
        end for;
    ...
    model PanelHeatingSystem
      ...
      final parameter Integer n_ceiling = 3 "Number of ceiling layers" annotation(Dialog(enable = false));
      parameter Modelica.SIunits.Thickness d_ceiling[RoomNo, n_ceiling] = zeros(RoomNo, n_ceiling) "Thickness of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.Density rho_ceiling[RoomNo, n_ceiling] = zeros(RoomNo, n_ceiling) "Density of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.ThermalConductivity lambda_ceiling[RoomNo, n_ceiling] = zeros(RoomNo, n_ceiling) "Thermal conductivity of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.SpecificHeatCapacity c_ceiling[RoomNo, n_ceiling] = zeros(RoomNo, n_ceiling) "Specific heat capacity of ceiling layers" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      parameter Modelica.SIunits.Emissivity eps_ceiling[RoomNo] = zeros(RoomNo) "Emissivity of ceiling surface" annotation(Dialog(group = "Floor Layers", enable = Ceiling));
      ...
      final parameter Modelica.SIunits.Temperature T_Room[RoomNo] = panelHeatingRoom.T_Room "Supposable Room Temperature";
      ...
      final parameter Real K_H[RoomNo] = panelHeatingRoom.K_H;
    ...
    equation
      ...
      for m in 1:CircuitNo[1] loop
        connect(distributor.flowPorts[m], panelHeatingRoom[1].ports_a[m]);
        connect(panelHeatingRoom[1].ports_b[m], distributor.returnPorts[m]);
      end for;
    ...
    package DetermineQ
      package q_TypeA
        ...
        model K_H_TypeA
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.a_T Determine_aT(R=
                R_lambdaB)
            annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.a_u Determine_au(T=T, R=
                R_lambdaB)
            annotation (Placement(transformation(extent={{-100,20},{-80,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.a_D Determine_aD(T=T, R=
                R_lambdaB)
            annotation (Placement(transformation(extent={{-100,-20},{-80,0}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.a_u Determine_au375(R=
                R_lambdaB, T=0.375)
            annotation (Placement(transformation(extent={{0,20},{20,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.a_D Determine_aD375(R=
                R_lambdaB, T=0.375)
        ...
        model qG_TypeA
          ...
          extends PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.K_H_TypeA;
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.B_G Determine_BG(
            s_u=s_u,
            T=T,
            lambda_E=lambda_E0)
            annotation (Placement(transformation(extent={{-80,20},{-60,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.n_G Determine_nG(
            s_u=s_u,
            T=T,
            lambda_E=lambda_E0)
            annotation (Placement(transformation(extent={{-80,-20},{-60,0}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.B_G Determine_BG375(
            s_u=s_u,
            T=0.375,
            lambda_E=lambda_E0)
            annotation (Placement(transformation(extent={{0,20},{20,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeA.n_G Determine_nG375(
            s_u=s_u,
            T=0.375,
            lambda_E=lambda_E0)
            annotation (Placement(transformation(extent={{0,-20},{20,0}})));
      ...
      package q_TypeB
        ...
        model q_TypeB
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.a_T Determine_aT(s_u=s_u,
              lambda_E=lambda_E)
            annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.a_K Determine_aK(T=T)
            annotation (Placement(transformation(extent={{-100,20},{-80,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.a_WL Determine_aWL(
            T=T,
            D=D,
            s_WL=s_WL,
            lambda_WL=lambda_WL,
            ...
            lambda_E=lambda_E)
            annotation (Placement(transformation(extent={{-100,-20},{-80,0}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.a_WL Determine_aWL0(
            T=T,
            D=D,
            lambda_WL=lambda_WL,
            s_u=s_u,
            ...
            s_WL=0) annotation (Placement(transformation(extent={{-100,-40},{-80,
                    -20}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.b_u Determine_bu(T=T)
        ...
        model qG_TypeB
          ...
          extends PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.q_TypeB;
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.B_G Determine_BG(
            T=T,
            D=D,
            s_WL=s_WL,
            lambda_WL=lambda_WL,
            ...
            lambda_E=lambda_E)
            annotation (Placement(transformation(extent={{0,60},{20,80}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.n_G Determine_nG(
            T=T,
            D=D,
            s_WL=s_WL,
            lambda_WL=lambda_WL,
            ...
            lambda_E=lambda_E)
            annotation (Placement(transformation(extent={{0,20},{20,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.B_G Determine_BGL(
            T=T,
            D=D,
            s_WL=s_WL,
            lambda_WL=lambda_WL,
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeB.n_G Determine_nGL(
            T=T,
            D=D,
            s_WL=s_WL,
            lambda_WL=lambda_WL,
      ...
      package q_TypeC
        ...
        model q_TypeC
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.a_T Determine_aT(R=
                R_lambdaB)
            annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.a_u Determine_au(T=T, R=
                R_lambdaB)
            annotation (Placement(transformation(extent={{-100,20},{-80,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.a_D Determine_aD(T=T, R=
                R_lambdaB)
            annotation (Placement(transformation(extent={{-100,-20},{-80,0}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.a_u Determine_au375(R=
                R_lambdaB, T=0.375)
            annotation (Placement(transformation(extent={{0,20},{20,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.a_D Determine_aD375(R=
                R_lambdaB, T=0.375)
            annotation (Placement(transformation(extent={{0,-20},{20,0}})));
        equation
        ...
        model qG_TypeC
          "Calculating the limiting heat flux for panel heating Types A and C"
          import Modelica.Constants.e;
          extends PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.q_TypeC;
          ...
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.B_G Determine_BG(
            s_u=s_u,
            T=T,
            lambda_E=lambda_E)
            annotation (Placement(transformation(extent={{-80,20},{-60,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.n_G Determine_nG(
            s_u=s_u,
            T=T,
            lambda_E=lambda_E)
            annotation (Placement(transformation(extent={{-80,-20},{-60,0}})));
          q_TypeC Determine_q375(
            ...
            T=0.375)
            annotation (Placement(transformation(extent={{0,60},{20,80}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.B_G Determine_BG375(
            s_u=s_u,
            lambda_E=lambda_E,
            T=0.375)
            annotation (Placement(transformation(extent={{0,20},{20,40}})));
          PanelHeatingNew.AddParameters.DetermineQ.q_TypeC.n_G Determine_nG375(
            s_u=s_u,
            lambda_E=lambda_E,
            T=0.375)
            annotation (Placement(transformation(extent={{0,-20},{20,0}})));
      ...
      package q_TypeD
        ...
        model qG_TypeD
          ...
          extends PanelHeatingNew.AddParameters.DetermineQ.q_TypeD.q_TypeD;
...
<p>Calculation of heat flux that is supposed to be generated from panel heating with given floor temperature (Temp_in[1]) and room temperature (Temp_in[2]). </p>
      ...
      model PanelHeatingCircuit_AllTypes
        ...
        parameter Modelica.SIunits.Temperature T_U = Modelica.SIunits.Conversions.from_degC(20) "Set value for Room Temperature lying under panel heating" annotation (Dialog(group="Room Specifications"));
          ...
          PanelHeatingNew.AddParameters.ZoneSpecification.ZoneDefinition ZoneType=
            PanelHeatingNew.AddParameters.ZoneSpecification.OccupancyZone()
          annotation (Dialog(group="Room Specifications"), choicesAllMatching=
              true);
        ...
        replaceable parameter
          PanelHeatingNew.AddParameters.Insulating_Materials.InsulatingMaterialDefinition
          InsulatingMaterial=
            PanelHeatingNew.AddParameters.Insulating_Materials.none()
          annotation (Dialog(group="Panel Heating"), choicesAllMatching=true);
        ...
        PanelHeatingNew.AddParameters.PanelHeatingElement panelHeatingParameters[dis](
          redeclare package Medium = Medium,
          each T=T,
          each m_H=m_H,
        ...
        AixLib.Utilities.Interfaces.RadPort radport_floor[dis]
          annotation (Placement(transformation(extent={{-40,90},{-20,110}})));
        Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a heatport_floor[dis]
          annotation (Placement(transformation(extent={{8,86},{28,106}}),
              iconTransformation(extent={{8,86},{28,106}})));
        AixLib.Utilities.Interfaces.RadPort radport_ceiling[dis]
          annotation (Placement(transformation(extent={{-34,-104},{-14,-84}})));
    ...
    package EN1264
      ...
      package TablesAndParameters
        ...
        model qG_TypeA
          ...
          extends
            PanelHeatingNew.AddParameters.EN1264.TablesAndParameters.K_H_TypeA;
...
<p>Calculation of heat flux that is supposed to be generated from panel heating with given floor temperature (Temp_in[1]) and room temperature (Temp_in[2]). </p>
      ...
      model HeatFlux "Upward and downward heat flux according to EN 1264-2"
        extends
          PanelHeatingNew.AddParameters.EN1264.TablesAndParameters.qG_TypeA;
    ...
    package TestModels
      ...
      model TestSystem
        ...
        PanelHeatingNew.AddParameters.PanelHeatingSystem panelHeatingSystem(
          sigma_des=5,
        ...
        Modelica.Blocks.Sources.Step TFloor(
          offset=299.15,
          startTime=43200,
          height=-1) annotation (Placement(transformation(extent={{94,50},{74,70}})));
        Modelica.Blocks.Sources.Constant TCeiling(k=21 + 273.15)
          annotation (Placement(transformation(extent={{92,-40},{72,-20}})));
      equation

        connect(TFloor.y, prescribedTemperatureFloor.T)
          annotation (Line(points={{73,60},{58,60}}, color={0,0,127}));
        connect(prescribedTemperatureCeiling.T, TCeiling.y)
          annotation (Line(points={{58,-30},{71,-30}}, color={0,0,127}));
        ...
        for i in 1:panelHeatingSystem.RoomNo loop
          ...
          if panelHeatingSystem.Ceiling[i] == true then
            connect(panelHeatingSystem.radport_ceiling[i],
              prescribedTemperatureCeiling.port) annotation (Line(points={{-6.4,
                    0.6},{-6.4,-30},{36,-30}}, color={95,95,95}));
          end if;
      ...
      model TestRoom
        ...
        final parameter Integer n = sum(CircuitNo);
