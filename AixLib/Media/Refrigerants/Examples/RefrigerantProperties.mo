...
model RefrigerantProperties
  ...
  //
  package Medium =
      AixLib.Media.Refrigerants.R32.R32_IIR_P1_70_T233_373_Formula
  ...
  parameter Modelica.SIunits.SpecificEnthalpy h_max=550e3
    ...
    annotation (Dialog(group="Fluid limits"));
  parameter Modelica.SIunits.Density d_min=12
  ...
  //
  Modelica.SIunits.SpecificEntropy s_dew;
  Modelica.SIunits.SpecificEntropy s_bubble;
...
equation
  ...
  // Calculate further thermodynamic state properties
  //
  ...
  satT = Medium.setSat_T(min(max(243.15,T),
    Medium.fluidConstants[1].criticalTemperature));
  //satP = Medium.setSat_p(satT.psat);
  satP = Medium.setSat_p(min(max(518000,p),Medium.fluidConstants[1].criticalPressure));

  // Calculate and check state functions
  //
  bubbleState = Medium.setBubbleState(satP,1);
  dewState = Medium.setDewState(satP,1);
