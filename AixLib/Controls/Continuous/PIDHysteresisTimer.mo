...
<p>
  The controller is similar to <a href=\"modelica://AixLib.Controls.Continuous.PIDHysteresis\">AixLib.Controls.Continuous.PIDHysteresis</a> but in addition, it has a timer that prevents the controller from switching to on too fast. When the controller switches off, the timer starts and avoids the controller from switching on until <code>minOffTime</code> seconds elapsed.
