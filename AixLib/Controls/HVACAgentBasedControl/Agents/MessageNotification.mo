...
<li><b>If you used an earlier version of this library and have problems running your agent system, add this component and the system should run again</b></li>
</ul>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">Concept</span></b></p>
<p>Every agent system needs a MessageNotification model in order to function, similar to the \"System\" model of the Modelica.Fluid library. The MessageNotification is responsible for ensuring that the agents receive their messages
and update their inbox.</p>
<p>There are two different possibilities to notify the agents when they receive a message. The standard one corresponds to earlier versions of the library and is set by setting usePoke=false in the MessageNotification models
and all other agents in the system. When this setting is made, the UDP inbox of all agents gets updated every n seconds (n set by user). This is straight forward but also leads to time event generation (weak simulation 
performance). If usePoke is set to true, the Boolean sendOut connectors of every agent in the system needs to be connected to the Boolean receive[n] input of the MessageNotification model. With this method, the agents only update
...
<li><a href=\"HVACAgentBasedControl.Examples.SimpleHeatingSystem.BuildingHeating_usePoke\">System with usePoke=true</a></li>
</ul>
</html>"));
