...
For the boundaries of the y-input value, a dynamic hysteresis is used to ensure a used device will stay off a certain time after shutdown."
  ...
  parameter Real dx = 1 "Delta value used for both upper and lower hysteresis. Used to avoid state-events when used as a security control.";
  ...
  AixLib.Utilities.Logical.DynamicHysteresis greaterLow
  ...
  Modelica.Blocks.Math.Add addLow
    annotation (Placement(transformation(extent={{-18,40},{-8,50}})));
initial equation
...
equation
  connect(x_in, uppCombiTable1Ds.u)
    annotation (Line(points={{-114,60},{-84,60},{-84,78},{-50,78}},
                                                  color={0,0,127}));
  connect(x_in, lowCombiTable1Ds.u) annotation (Line(points={{-114,60},{-72,60},
          {-72,12},{-56,12}},
                     color={0,0,127}));
  connect(nor1.y, noErr)
    annotation (Line(points={{93.5,0},{110,0}}, color={255,0,255}));
  connect(lessUpp.y, nor1.u[1]) annotation (Line(points={{51,78},{64,78},{64,5.25},
          {72,5.25}}, color={255,0,255}));
  connect(greaterLow.y, nor1.u[2]) annotation (Line(points={{51,28},{64,28},{64,
          1.75},{72,1.75}}, color={255,0,255}));
  connect(lessLef.y, nor1.u[3]) annotation (Line(points={{47,-30},{64,-30},{64,-1.75},
          {72,-1.75}},        color={255,0,255}));
  connect(greaterRig.y, nor1.u[4]) annotation (Line(points={{47,-60},{64,-60},{64,
          -5.25},{72,-5.25}},     color={255,0,255}));
  ...
  connect(lowCombiTable1Ds.y[1], greaterLow.uLow) annotation (Line(points={{-33,
          12},{-10,12},{-10,6},{35,6},{35,12},{35,14},{35,16}}, color={0,0,127}));
  connect(constDx.y, addLow.u1) annotation (Line(points={{-35.3,53},{-35.3,52},{
          -28,52},{-28,52},{-28,48},{-19,48}}, color={0,0,127}));
  connect(lowCombiTable1Ds.y[1], addLow.u2) annotation (Line(points={{-33,12},{-24,
          12},{-24,42},{-19,42}}, color={0,0,127}));
  connect(addLow.y, greaterLow.uHigh) annotation (Line(points={{-7.5,45},{0,45},
          {0,12},{42,12},{42,12},{42,12},{43,12},{43,14},{43,16}}, color={0,0,127}));
  connect(y_in, greaterLow.u) annotation (Line(points={{-114,-60},{-90,-60},{-90,
          100},{16,100},{16,28},{28,28}}, color={0,0,127}));
  annotation (Diagram(
