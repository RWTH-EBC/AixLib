...
block Derivative
  ...
  AixLib.Controls.OBC.CDL.Interfaces.RealInput T(
    final quantity="Time",
    final unit="s",
...
initial equation
  x= if abs(k) < AixLib.Controls.OBC.CDL.Constants.eps then u else u - T*y_start/k;

equation
  T_nonZero = max(T, 100*AixLib.Controls.OBC.CDL.Constants.eps);
  der(x) = (u-x)/T_nonZero;
  y = (k/T_nonZero)*(u-x);
