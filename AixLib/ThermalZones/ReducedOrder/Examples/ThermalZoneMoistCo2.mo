...
model ThermalZoneMoistCo2 "Illustrates the use of ThermalZoneMoistAir"
  extends Modelica.Icons.Example;

  package Medium=AixLib.Media.Air(extraPropertiesNames={"CO2"});

  AixLib.ThermalZones.ReducedOrder.ThermalZone.ThermalZoneMoistCo2 thermalZone(
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    ROM(extWallRC(thermCapExt(each der_T(fixed=true))), intWallRC(thermCapInt(each der_T(fixed=true)))),
    redeclare package Medium = Medium,
    internalGainsMode=3,
    recOrSep=true,
    nPorts=2,
    zoneParam=DataBase.ThermalZones.OfficePassiveHouse.OPH_1_OfficeNoHeaterCooler()) "Thermal zone"
  ...
  Fluid.Sources.Boundary_pT sinAir(redeclare package Medium = Medium,
