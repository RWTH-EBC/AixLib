...
model CDD "VDI 6007 Test Case 3 model"
 ...
 parameter Integer location(min = 1, max = 2) = 1 "Location of validation" annotation(choices(choice = 1 "Hamburg", choice = 2 "Mannheim"));
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature
    ...
    annotation (Placement(transformation(extent={{-70,6},{-58,18}})));
  Modelica.Thermal.HeatTransfer.Components.Convection theConWall
    "Outdoor convective heat transfer"
    annotation (Placement(transformation(extent={{-40,18},{-52,6}})));
  ...
  Modelica.Blocks.Math.Product product1
    ...
    direction"
    annotation (Placement(transformation(extent={{-16,67},{-6,78}})));
  ...
  Modelica.Blocks.Math.MultiSum
    ...
    "Reverses ventilation rate"
    annotation (Placement(transformation(extent={{118,55},{132,69}})));
  ...
  Modelica.Blocks.Sources.CombiTimeTable OutdoorTemp_Mannheim(
    ...
    columns={2}) "Table with internal gains"
    annotation (Placement(transformation(extent={{-148,-8},{-132,8}})));
  Modelica.Blocks.Sources.CombiTimeTable OutdoorTemp_Hamburg(
    ...
    columns={2}) "Table with internal gains"
    annotation (Placement(transformation(extent={{-148,20},{-132,36}})));
  ...
  Modelica.Blocks.Continuous.LimPID PID(
    ...
    each yMin=PowerTabs,
    y_start=-440)
    annotation (Placement(transformation(extent={{-90,-158},{-66,-134}})));
  Modelica.Blocks.Math.MultiProduct
                            multiProduct(nu=1)
    "Reverses ventilation rate"
    annotation (Placement(transformation(extent={{-2,-153},{12,-139}})));
  ...
  Modelica.Blocks.Sources.CombiTimeTable SollTempHamburg(
    ...
    columns={2}) "Table with internal gains"
    annotation (Placement(transformation(extent={{-124,-134},{-108,-118}})));
  Modelica.Blocks.Sources.CombiTimeTable tableSolRadWindow(
    ...
    columns={2})
    "Solar radiation"
    annotation (Placement(transformation(extent={{-146,68},{-130,84}})));
  ...
  Modelica.Blocks.Math.Mean OutOpTemp(f=1/3600)
    ...
    annotation (Placement(transformation(extent={{164,56},{176,68}})));
  Modelica.Blocks.Math.Mean OutAirTemp(f=1/3600)
    "Hourly mean of indoor air temperature"
    annotation (Placement(transformation(extent={{190,36},{202,48}})));
  ...
  Modelica.Blocks.Math.Gain gain3(k=1)
    "Reverses ventilation rate"
    annotation (Placement(transformation(extent={{-114,69},{-100,83}})));
  Modelica.Blocks.Sources.CombiTimeTable SollTempMannheim(
    extrapolation=Modelica.Blocks.Types.Extrapolation.Periodic,
    table=[0,299.15; 3600,299.15; 7200,299.15; 10800,299.15; 10800,294.15; 14400,
        294.15; 18000,294.15; 21600,294.15; 25200,294.15; 28800,294.15; 32400,294.15;
        36000,294.15; 39600,294.15; 43200,294.15; 46800,294.15; 50400,294.15; 54000,
        294.15; 57600,294.15; 61200,294.15; 61200,299.15; 64800,299.15; 72000,299.15;
        75600,299.15; 79200,299.15; 82800,299.15; 86400,299.15],
    columns={2}) "Table with internal gains"
    annotation (Placement(transformation(extent={{-124,-164},{-108,-148}})));
protected
...
equation
 ...
 for i in 1:RoomRecord.nOrientations loop
  ...
  connect(theConWall.fluid,preTem. port)
    annotation (Line(points={{-52,12},{-58,12}},           color={191,0,0}));
  connect(thermalZoneTwoElements.extWall,theConWall. solid)
    annotation (Line(points={{44,12},{-40,12}},                  color={191,0,0}));
  connect(hConWall.y,theConWall. Gc)
    annotation (Line(points={{-48,0.4},{-46,0.4},{-46,6}},   color={0,0,127}));
  ...
  connect(switch1.y,product1. u2)
    annotation (Line(points={{-36,35.4},{-36,32},{-20,32},{-20,69.2},{-17,69.2}},
                                color={0,0,127}));
  ...
  connect(thermalZoneTwoElements.TRad, multiSum.u[1]) annotation (Line(points={{93,28},
          {106,28},{106,65.2667},{118,65.2667}},        color={0,0,127}));
  connect(thermalZoneTwoElements.TAir, multiSum.u[2]) annotation (Line(points={{
          93,32},{104,32},{104,62},{118,62}}, color={0,0,127}));
  connect(ToKelvin.y, multiSum.u[3]) annotation (Line(points={{94.4,64},{118,64},
          {118,58.7333}},                   color={0,0,127}));
  connect(sum1.y, preTem.T) annotation (Line(points={{-87.3,12},{-71.2,12}},
                           color={0,0,127}));
  ...
  connect(thermalZoneTwoElements.TAir, PID.u_m) annotation (Line(points={{93,32},
          {158,32},{158,24},{202,24},{202,-176},{-78,-176},{-78,-160.4}}, color=
         {0,0,127}));
  connect(PID.y, multiProduct.u[1]) annotation (Line(points={{-64.8,-146},{-2,-146}},
                                  color={0,0,127}));
  ...
  connect(sum2.u[1], ToCelsius.y) annotation (Line(points={{162.6,41.3},{156.4,41.3},
          {156.4,22},{150.4,22}}, color={0,0,127}));
  connect(thermalZoneTwoElements.TAir, sum2.u[2]) annotation (Line(points={{93,32},
          {128,32},{128,42.7},{162.6,42.7}}, color={0,0,127}));
  connect(product1.y, thermalZoneTwoElements.solRad[1]) annotation (Line(points={{-5.5,
          72.5},{2.25,72.5},{2.25,31},{43,31}},   color={0,0,127}));
  connect(tableSolRadWindow.y[1], gain3.u) annotation (Line(points={{-129.2,76},
          {-115.4,76}},                           color={0,0,127}));
  connect(gain3.y, product1.u1) annotation (Line(points={{-99.3,76},{-58,76},{-58,
          75.8},{-17,75.8}},
                         color={0,0,127}));
  ...
  if location == 1 then
    connect(OutdoorTemp_Hamburg.y[1], sum1.u[1]) annotation (Line(points={{-131.2,
          28},{-114,28},{-114,11.3},{-103.4,11.3}},
                                             color={0,0,127}));
    connect(SollTempHamburg.y[1], PID.u_s) annotation (Line(points={{-107.2,-126},
          {-98,-126},{-98,-146},{-92.4,-146}}, color={0,0,127}));
  else
    connect(OutdoorTemp_Mannheim.y[1], sum1.u[1]) annotation (Line(points={{-131.2,
            0},{-114,0},{-114,11.3},{-103.4,11.3}},
                                             color={0,0,127}));
    connect(SollTempMannheim.y[1], PID.u_s) annotation (Line(points={{-107.2,
            -156},{-98,-156},{-98,-146},{-92.4,-146}},
                                               color={0,0,127}));
  end if;

  connect(thermalZoneTwoElements.TAir, rCTABS.TAir) annotation (Line(points={{93,
          32},{108,32},{108,-108},{116,-108},{116,-114},{135,-114}}, color={0,0,
          127}));
  connect(heatFlowSensor.port_b, thermalZoneTwoElements.intGainsConv)
    annotation (Line(points={{188,-54},{188,2},{112,2},{112,20},{92,20}}, color=
         {191,0,0}));
  connect(rCTABS.port_int, convTABS.fluid)
    annotation (Line(points={{156,-118},{162,-118}}, color={191,0,0}));
  connect(convTABS.solid, heatFlowSensor.port_a) annotation (Line(points={{182,-118},
          {188,-118},{188,-74}}, color={191,0,0}));
