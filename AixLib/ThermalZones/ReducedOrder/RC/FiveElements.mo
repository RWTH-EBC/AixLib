...
model FiveElements "Thermal Zone with five elements for exterior walls, interior walls, floor plate, roof, and neighboured zone borders\""
  ...
  parameter Modelica.Units.SI.Area ANZ[nNZs] "Area of neighboured zone borders" annotation(
    Dialog(group = "Zone borders"));
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer hConNZ[nNZs] "Convective coefficient of heat transfer of neighboured zone borders (indoor)" annotation(
    Dialog(group = "Zone borders"));
  parameter Integer nNZ(min = 1) "Number of RC-elements of neighboured zone borders" annotation(
    Dialog(group = "Zone borders"));
  parameter Modelica.Units.SI.ThermalResistance RNZ[nNZs, nNZ](each min = Modelica.Constants.small) "Vector of resistances of neighboured zone borders, from inside to outside" annotation(
    Dialog(group = "Zone borders"));
  parameter Modelica.Units.SI.ThermalResistance RNZRem[nNZs](each min = Modelica.Constants.small) "Vector of resistances of remaining resistor RNZRem between capacity n and outside" annotation(
    Dialog(group = "Zone borders"));
  parameter Modelica.Units.SI.HeatCapacity CNZ[nNZs, nNZ](each min = Modelica.Constants.small) "Vector of heat capacities of neighboured zone borders, from inside to outside" annotation(
    Dialog(group = "Zone borders"));
  parameter Boolean indoorPortNZ = false "Additional heat port at indoor surface of neighboured zone borders" annotation(
    Dialog(group = "Zone borders"),
    choices(checkBox = true));
  parameter Integer otherNZIndex[nNZs] "index of the zone each neighboured zone border is adjacent to" annotation(
    Dialog(group = "Zone borders"));
  parameter Integer thisZoneIndex "index of this zone" annotation(
    Dialog(group = "Zone borders"));
      ...
      if ANZ[i] > 0 then
        connect(nz[i], nzRC[i].port_b) annotation (Line(points={{215,178},{216,178},
                {216,174},{102,174},{102,165},{103,165}},
                                             color={191,0,0}));
        connect(convNZ[i].solid, nzRC[i].port_a) annotation(
          Line(points = {{102, 134}, {102, 140}, {102, 145}, {103, 145}}, color = {191, 0, 0}));
        connect(convNZ[i].Gc, hConNZ_const[i].y) annotation(
