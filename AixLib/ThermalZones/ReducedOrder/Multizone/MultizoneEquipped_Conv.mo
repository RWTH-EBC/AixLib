...
model MultizoneEquipped_Conv
  ...
  Modelica.Blocks.Sources.Constant ToKelvin[numTabs](k=273.15)
    "Outdoor coefficient of heat transfer for walls" annotation (Placement(
        ...
        rotation=270,
        origin={180,38})));
  Modelica.Blocks.Continuous.LimPID ConvPID[numTabs](
    each controllerType=Modelica.Blocks.Types.SimpleController.PI,
    each k=100,
    each Ti=500,
    yMax=TabsHeatLoad,
    each yMin=0)
    annotation (Placement(transformation(extent={{-198,-28},{-182,-12}})));
  Modelica.Blocks.Sources.Constant RefTemp1[numTabs](k=21)
    "Outdoor coefficient of heat transfer for walls" annotation (Placement(
        ...
        rotation=0,
        origin={-210,-20})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow HeatConv[numTabs]
...
equation
  ...
  connect(TOpe.y, ConvPID.u_m) annotation (Line(points={{159,-10.19},{159,-114},
          {-190,-114},{-190,-29.6}}, color={0,0,127}));
  connect(RefTemp1.y, ConvPID.u_s)
    annotation (Line(points={{-205.6,-20},{-199.6,-20}}, color={0,0,127}));
  connect(ConvPID.y, HeatConv.Q_flow)
    annotation (Line(points={{-181.2,-20},{-172,-20}}, color={0,0,127}));
  connect(HeatConv.port, heatFlowSensor.port_b) annotation (Line(points={{-152,
          -20},{-139,-20},{-139,-24}}, color={191,0,0}));
  connect(heatFlowSensor.port_a, intGainsConv) annotation (Line(points={{-139,
          -38},{-138,-38},{-138,-70},{-100,-70}}, color={191,0,0}));
