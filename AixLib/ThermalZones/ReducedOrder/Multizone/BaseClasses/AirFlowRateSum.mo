...
<ul>
  ...
  <li>A maxmial volume flow (maxAHU = deltaAHU + minAHU) <span style=\"font-family:" ms="" shell="" dlg="">in m3/(m2*h) per zone</span>
  </li>
  <li>A given profile or relative occupation per zone
  </li>
</ul>
<p>
  A vector of <a href=\"AixLib.DataBase.ThermalZones.ZoneBaseRecord\">AixLib.DataBase.ThermalZones.ZoneBaseRecord</a> records is necessary to evaluate which zones are supplied by an AHU. As AHUs typically work with m3/s, the model calculates the output air flow rate in m3/s.
</p>
<h4>
...
<ul>
  ...
  <li>February 26, 2016, by Moritz Lauster:<br/>
    To get the correct share of volume flow for each zone, more outputs are generated.
  </li>
