...
<p>This model splits a given volume flow in m3/s (e.g. for an Air Handling Unit) into several flows for one zone each based on:</p>
<ul>
...
<p>A vector of <a href=\"AixLib.DataBase.ThermalZones.ZoneBaseRecord\">AixLib.DataBase.ThermalZones.ZoneBaseRecord</a> records is necessary to evaluate which zones are supplied by an AHU. The model calculates the single ratios of each zone of the ingoing volume flow into the AHU and weights the outcoming volume flow of the AHU correspondingly. As thermal zones as consumer typically use 1/h, this model converts m3/s to 1/h.</p>
<h4>Typical use and important parameters</h4>
<p>The governing equations are:</p>
<p>airFlow[nZones] = (minAHU + deltaAHU * (profile OR relative Occupation)) * AZone * 3600^-1 </p>
<p>airFlowShareZones[nZones] = airFow[nZones] * isSuppliedByAHU[nZones]</p>
<p>airFlowOut[nZones] = airFlowShareZones[nZones] .* airFlowIn </p>
<p>All parameter come from the vector of <a href=\"AixLib.DataBase.ThermalZones.ZoneBaseRecord\">AixLib.DataBase.ThermalZones.ZoneBaseRecord</a> records. The model is typically used as output adapter for AHU model in <a href=\"AixLib.ThermalZones.ReducedOrder.Multizone.MultizoneEquipped\">AixLib.ThermalZones.ReducedOrder.Multizone.MultizoneEquipped</a>.</p>
</html>", revisions="<html>
<ul>
  <li>
