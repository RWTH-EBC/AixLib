...
model ThermalZone_withPools
  ...
  Modelica.Blocks.Math.MultiSum SumQLat2_flow(nu=1) if (ATot > 0 or
...
// ***************************************
 ...
 //***************************************
  ...
  Modelica.Blocks.Math.MultiSum SumPoolPPump(nu=1) if swimmingPools
    "Sum up electriical energy consumption of the circulation pump of all swimming pools"
    annotation (Placement(transformation(extent={{-36,-86},{-26,-76}})));
  Modelica.Blocks.Math.MultiSum SumPoolQHeat(nu=1) if swimmingPools
    "Sum up heat demands of all swimming pools"
    annotation (Placement(transformation(extent={{-36,-86},{-26,-76}})));
  Modelica.Blocks.Math.MultiSum SumPoolFreshWater(nu=1) if
    swimmingPools "Sum up fresh water demands of all swimming pools"
    annotation (Placement(transformation(extent={{-36,-86},{-26,-76}})));
  Modelica.Blocks.Math.MultiSum SumPoolRecycledWater(nu=1) if
...
equation
 if swimmingPools and (ATot > 0) then
    ...
    for i in 1:numPools loop
      connect(TRad, indoorSwimmingPool[i].TRad) annotation (Line(points={{110,60},{88,
              60},{88,22},{-67.76,22},{-67.76,-71.58}},
                                               color={0,0,127}));
      connect(humVolAirROM.y, indoorSwimmingPool[i].X_w) annotation (Line(points={{-59.5,
              -50},{-58,-50},{-58,-71.65},{-56.16,-71.65}},
                                                        color={0,0,127}));
      connect(TSoil.y, indoorSwimmingPool[i].TSoil) annotation (Line(points={{43.4,22},
              {44,22},{44,-75.15},{-53.44,-75.15}}, color={0,0,127}));
      connect(ROM.intGainsConv, indoorSwimmingPool[i].ConvPoolSurface)
        annotation (Line(points={{86,78},{10,78},{10,-72},{-62.8,-72}}, color={
              191,0,0}));
      connect(indoorSwimmingPool[i].TAir, TAir) annotation (Line(points={{-58.56,-71.65},
              {-58,-71.65},{-58,-50},{0,-50},{0,80},{110,80}},
                                           color={0,0,127}));
      connect(indoorSwimmingPool[i].PPump, SumPoolPPump.u) annotation (Line(points={{-53.36,
              -80.12},{-36,-80.12},{-36,-80},{-46,-80},{-46,-81},{-36,-81}},
                                                               color={0,0,127}));
      connect(indoorSwimmingPool[i].QPool, SumPoolQHeat.u) annotation (Line(points={{-53.36,
              -81.1},{-53.36,-82},{-36,-82},{-36,-81}}, color={0,0,127}));
      connect(indoorSwimmingPool[i].MFlowWasteWater, SumPoolWasteWater.u) annotation (
        Line(points={{-53.52,-83.9},{-46.76,-83.9},{-46.76,-81},{-36,-81}},       color=
           {0,0,127}));
      connect(indoorSwimmingPool[i].MFlowFreshWater, SumPoolFreshWater.u) annotation (Line(
          points={{-53.52,-85.02},{-46,-85.02},{-46,-81},{-36,-81}},       color={0,0,127}));
      connect(indoorSwimmingPool[i].MFlowRecycledWater, SumPoolRecycledWater.u) annotation (
      Line(points={{-53.52,-84.46},{-54,-84.46},{-54,-84},{-46,-84},{-46,-80},{-36,-80},
              {-36,-81}},                                                       color=
         {0,0,127}));
  ...
  if internalGainsMode == 3 then
    ...
    connect(humanTotHeaDependent.QLat_flow, SumQLat2_flow.u[1]) annotation (
        Line(points={{75.6,-16},{92,-16},{92,-4},{8,-4},{8,-54},{-42,-54},{-42,-62},{-40,
            -62}},        color={0,0,127}));
  else
    connect(noMoisturePerson.y, SumQLat1_flow.u[1]) annotation (Line(points={{-47.6,-56},
            {-40,-56},{-40,-62}},        color={0,0,127}));
    connect(noMoisturePerson.y, SumQLat2_flow.u[1]) annotation (Line(points={{-47.6,-56},
            {-40,-56},{-40,-62}},        color={0,0,127}));
  end if;
  ...
  connect(airExcMoi.HumIn, ventHum) annotation (Line(points={{-21.2,-8},{-22,-8},{-22,
          -90},{-82,-90},{-82,-88},{-108,-88}},
                                 color={0,0,127}));
  connect(airExcMoi.port_b, ROM.intGainsConv) annotation (Line(points={{-6,-4},
          {58,-4},{58,78},{86,78}}, color={191,0,0}));
  connect(airExcMoi.QLat_flow, SumQLat2_flow.u[1]) annotation (Line(points={{-5.68,-8.96},
          {-6,-8.96},{-6,-40},{-42,-40},{-42,-62},{-40,-62}},
        color={0,0,127}));
