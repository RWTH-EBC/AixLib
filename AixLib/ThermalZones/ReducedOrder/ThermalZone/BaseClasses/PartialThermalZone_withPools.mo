...
partial model PartialThermalZone_withPools
  ...
  RC.FourElements ROM(
    ...
    final nu = if zoneParam.use_swimmingPools then zoneParam.numPools+1 else 1)
    "RC calculation core" annotation (Placement(transformation(extent={{38,56},
            {86,92}})));
...
<ul>
  ...
  Rectangle(
    ...
    fillPattern=FillPattern.Solid)}));
end PartialThermalZone_withPools;
