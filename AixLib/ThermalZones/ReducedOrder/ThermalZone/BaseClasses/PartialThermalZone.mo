...
partial model PartialThermalZone "Partial model for thermal zone models"
  ...
  RC.FourElements ROM(
    ...
    final nOrientations=max({size(zoneParam.AWin, 1),size(zoneParam.ATransparent,
        1),size(zoneParam.AExt, 1)}),
    ...
    final nExt=max({size(zoneParam.RExt, 1),size(zoneParam.CExt, 1)}),
    ...
    final nInt=max({size(zoneParam.RInt, 1),size(zoneParam.CInt, 1)}),
    final RInt=zoneParam.RInt,
    final CInt=zoneParam.CInt,
    final AFloor=zoneParam.AFloor,
    final hConFloor=zoneParam.hConFloor,
    final nFloor=max({size(zoneParam.RFloor, 1),size(zoneParam.CFloor, 1)}),
    ...
    final nRoof=max({size(zoneParam.RRoof, 1),size(zoneParam.CRoof, 1)}),
