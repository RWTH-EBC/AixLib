...
model ThermalZone_TABS_Sprungantwort "Thermal zone containing moisture balance"
  extends
...
public
  ...
  Modelica.Blocks.Sources.Constant const1(k=0)
    annotation (Placement(transformation(extent={{8,60},{18,70}})));
  Modelica.Blocks.Sources.Constant const2(k=273.15)
    annotation (Placement(transformation(extent={{-28,28},{-18,38}})));
equation
  ...
  connect(heaterCoolerWithTabs6007C1.heatOrCool, ROM.heatingOrCooling)
    annotation (Line(points={{82.65,38.6},{66.325,38.6},{66.325,54},{51.2,54}},
        color={255,0,255}));
  connect(const1.y, ROM.solRad[1]) annotation (Line(points={{18.5,65},{18.5,
          60.5},{37,60.5},{37,89}}, color={0,0,127}));
  connect(const1.y, ROM.solRad[2]) annotation (Line(points={{18.5,65},{18.5,
          61.5},{37,61.5},{37,89}}, color={0,0,127}));
  connect(const1.y, ROM.solRad[3]) annotation (Line(points={{18.5,65},{18.5,
          60.5},{37,60.5},{37,89}}, color={0,0,127}));
  connect(const1.y, ROM.solRad[4]) annotation (Line(points={{18.5,65},{18.5,
          60.5},{37,60.5},{37,89}}, color={0,0,127}));
  connect(const1.y, ROM.solRad[5]) annotation (Line(points={{18.5,65},{18.5,
          60.5},{37,60.5},{37,89}}, color={0,0,127}));
  connect(const1.y, ROM.solRad[6]) annotation (Line(points={{18.5,65},{18.5,
          60.5},{37,60.5},{37,89}}, color={0,0,127}));
  connect(const2.y, preTemWin.T) annotation (Line(points={{-17.5,33},{-6.75,33},
...
"),     Rectangle(
        ...
        Text(
          ...
          fillPattern=FillPattern.Solid,
          textString="CO2")}));
end ThermalZone_TABS_Sprungantwort;
