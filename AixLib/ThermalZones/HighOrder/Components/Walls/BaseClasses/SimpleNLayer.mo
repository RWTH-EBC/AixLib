...
model SimpleNLayer "Wall consisting of n layers"
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor HeatConda[wallRec.n](G = A .* wallRec.lambda ./ (wallRec.d / 2)) annotation(Placement(transformation(extent={{-52,-10},
            {-32,10}})));
  // n Loads
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor Load[wallRec.n](T(start=fill(T0, wallRec.n), each fixed=true),
                                                                                         final C = wallRec.c .* wallRec.rho .* A .* wallRec.d) annotation(Placement(transformation(extent={{-10,-42},{10,-22}})));
...
equation
  ...
  connect(HeatConda[1].port_a, port_a) annotation (Line(
      ...
      color={191,0,0},
      pattern=LinePattern.DashDotDot));
  connect(HeatCondb[wallRec.n].port_b, port_b)
                                      annotation (Line(
      points={{50,0},{100,0}},
      color={191,0,0},
      pattern=LinePattern.DashDotDot));
