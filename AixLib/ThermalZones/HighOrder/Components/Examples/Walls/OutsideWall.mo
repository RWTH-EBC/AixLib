...
equation
  connect(Toutside.port, outerWall.port_outside) annotation(Line(points = {{20, 22}, {4, 22}, {4, 25}, {-23.7, 25}}, color = {191, 0, 0}));
  connect(outerWall.SolarRadiationPort, varRad.solarRad_out[1]) annotation(Line(points = {{-22.2, 77.25}, {9.9, 77.25}, {9.9, 80}, {41, 80}}, color = {255, 128, 0}));
  connect(heatStarToComb.portConvRadComb, outerWall.thermStarComb_inside) annotation (Line(points={{-48.2,
          24.7},{-43.3,24.7},{-43.3,25},{-36,25}},                                                                                                   color={191,0,0}));
  ...
  connect(varRad.I[1], Solarradiation.y) annotation (Line(
      ...
      color={0,0,127}));
  connect(outerWall.WindSpeedPort, WindSpeed.y) annotation (Line(
      points={{-23.7,66.8},{-6,66.8},{-6,56},{11.1,56}},
      color={0,0,127}));
  connect(outerWall.solarRadWinTrans, outerWall.solarRadWin) annotation (
      Line(points={{-37.5,-4.45},{-42,-4.45},{-42,66.8},{-36.6,66.8}}, color={0,
          0,127}));
