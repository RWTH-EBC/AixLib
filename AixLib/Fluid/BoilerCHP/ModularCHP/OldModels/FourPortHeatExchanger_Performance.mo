...
model FourPortHeatExchanger_Performance
  ...
  replaceable AixLib.Fluid.MixingVolumes.BaseClasses.MixingVolumeHeatPort vol1[nNodes](
    each V = m1_flow_nominal*tau1/rho1_nominal/nNodes, nPorts=3)
                                                       constrainedby
  ...
  replaceable AixLib.Fluid.MixingVolumes.MixingVolume vol2[nNodes](
    each V = m2_flow_nominal*tau2/rho2_nominal/nNodes, nPorts=3)
                                                       constrainedby
    MixingVolumes.BaseClasses.PartialMixingVolume(
...
equation
  ...
  connect(port_a1, vol1[1].ports[1]) annotation (Line(
      points={{-100,60},{-2.66667,60}},
      color={0,127,255}));
  connect(vol1[nNodes].ports[2], port_b1) annotation (Line(
      points={{0,60},{100,60}},
      color={0,127,255}));
  connect(port_a2, vol2[1].ports[1]) annotation (Line(
      points={{100,-60},{2.66667,-60}},
      color={0,127,255}));
  ...
  for i in 1:nNodes-1 loop
    connect(vol1[i].ports[1],vol1[i+1].ports[1]);
    connect(vol2[i].ports[1],vol2[i+1].ports[1]);
  end for;
...
First implementation.
    ...
    Icon(coordinateSystem(
        ...
        Rectangle(
          ...
          fillColor={0,0,0},
          fillPattern=FillPattern.Solid)}));
end FourPortHeatExchanger_Performance;
