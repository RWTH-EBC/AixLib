...
//  MM_Exh = sum(n_ComExh[i]*MM_ComExh[i] for i in 1:size(n_ComExh, 1))/sum(n_ComExh[i] for i in 1:size(n_ComExh, 1));
 ...
 // end for;
  ...
  Mmot = P_eff*60/(nEng*2*Modelica.Constants.pi);
  Q_therm = P_Fue - P_eff - H_Exh;
  if m_Fue>0 then
  ...
  connect(effectiveMechanicalTorque.y, mechanicalPower) annotation (Line(points=
         {{15.4,22},{26,22},{26,46},{0,46},{0,112}}, color={0,0,127}));
