...
for i in 1:size(n_ComExh, 1) loop
  ...
  elseif inertia.w<80 and inertia.w>=5 then
  Mmot = -nEng*60/800*CHPEngData.i*p_mf*CHPEngData.VEng/(2*Modelica.Constants.pi);
  nEng = inertia.w/(2*Modelica.Constants.pi);
  else
  Mmot = 0;
  nEng = 0;
