...
model BoilerSystem "Example that illustrates use of boiler model"
  ...
  Modelica.Blocks.Sources.BooleanConstant on(k=false)
    "Boiler is always on"
  ...
  Sensors.Temperature before( redeclare package Medium =
        Media.Specialized.Water.TemperatureDependentDensity) annotation (Placement(transformation(extent={{-38,14},{-20,30}})));
  Sensors.Temperature after( redeclare package Medium =
        Media.Specialized.Water.TemperatureDependentDensity) annotation (Placement(transformation(extent={{12,16},{30,32}})));
equation
  ...
  connect(before.port, boiler.port_a) annotation (Line(points={{-29,14},{-28,14},{-28,6},{-28,0},{-10,
          0}}, color={0,127,255}));
  connect(after.port, boiler.port_b)
    annotation (Line(points={{21,16},{20,16},{20,10},{20,0},{10,0}}, color={0,127,255}));
  annotation (
