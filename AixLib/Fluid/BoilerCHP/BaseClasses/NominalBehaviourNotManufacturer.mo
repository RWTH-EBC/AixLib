...
parameter Modelica.SIunits.TemperatureDifference deltaTCoolingCircuit=3.47 "Nominal temperature difference heat circuit";
  ...
  Modelica.Blocks.Routing.Multiplex2 multiplex2_1
    annotation (Placement(transformation(extent={{-50,38},{-34,22}})));
  ...
  Modelica.Blocks.Sources.RealExpression pelNom(y=PelNom)
    ...
    annotation (Placement(transformation(extent={{-106,16},{-90,36}})));
  SDF.NDTable SDFEta(
    nin=2,
    readFromFile=true,
  ...
  Modelica.Blocks.Interfaces.RealOutput mWaterHC "Water mass flow heat circuit"
  ...
  Modelica.Blocks.Sources.RealExpression TempDiffHC(y=4180*
        deltaTHeatingCircuit)
    "Temperature difference Cooling Water Engine"
    annotation (Placement(transformation(extent={{30,2},{50,22}})));
  Modelica.Blocks.Math.Division division3
    annotation (Placement(transformation(extent={{66,42},{80,56}})));
...
equation
  connect(multiplex2_1.y, SDFStromkennzahl.u)
    annotation (Line(points={{-33.2,30},{-22,30}}, color={0,0,127}));
  connect(PLRNominal.y, multiplex2_1.u2[1]) annotation (Line(points={{-79,44},{
          -72,44},{-72,34},{-51.6,34},{-51.6,34.8}}, color={0,0,127}));
  connect(pelNom.y, multiplex2_1.u1[1]) annotation (Line(points={{-89.2,26},{-84,
          26},{-84,25.2},{-51.6,25.2}}, color={0,0,127}));
  connect(multiplex2_1.y, SDFEta.u) annotation (Line(points={{-33.2,30},{-30,30},
  ...
  connect(pelNom.y, division.u1) annotation (Line(points={{-89.2,26},{-84,26},{-84,
          14},{-112,14},{-112,64},{12,64},{12,37.2},{18.6,37.2}}, color={0,0,127}));
  connect(multiplex2_1.y, SDFRatioCoolingWater.u) annotation (Line(points={{
          -33.2,30},{-30,30},{-30,-58},{-22,-58}}, color={0,0,127}));
  connect(SDFEta.y, division1.u2) annotation (Line(points={{1,-10},{14,-10},{14,
          -17.2},{30.6,-17.2}}, color={0,0,127}));
  connect(pelNom.y, division1.u1) annotation (Line(points={{-89.2,26},{-84,26},
          {-84,10},{16,10},{16,-8.8},{30.6,-8.8}}, color={0,0,127}));
  ...
  connect(division.y, division3.u1) annotation (Line(points={{34.7,33},{50,33},
          {50,53.2},{64.6,53.2}}, color={0,0,127}));
  connect(TempDiffHC.y, division3.u2) annotation (Line(points={{51,12},{62,12},
          {62,44.8},{64.6,44.8}}, color={0,0,127}));
  connect(division3.y, mWaterHC) annotation (Line(points={{80.7,49},{88,49},{88,
