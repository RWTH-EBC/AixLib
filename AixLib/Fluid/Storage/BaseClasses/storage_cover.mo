...
final parameter Modelica.SIunits.Area A_wall = D1^2/4*Modelica.Constants.pi "Area";
...
//////components///
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor cond_ins1(G=(
        A_wall)*(lambda_ins)/(d_ins/2)) annotation (Placement(
  ...
  AixLib.Utilities.HeatTransfer.HeatConv conv_outside(alpha=alpha_outside, A=
        A_wall) annotation (Placement(transformation(
        origin={72,8},
        extent={{-10,-10},{10,10}},
        rotation=180)));
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor load_ins(C=(c_ins)
        *(rho_ins)*(A_wall)*(d_ins)) annotation (Placement(transformation(
