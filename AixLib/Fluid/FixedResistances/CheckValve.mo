...
protected
  ...
  parameter Modelica.Units.SI.MassFlowRate m2_flow =
    AixLib.Fluid.BaseClasses.FlowModels.basicFlowFunction_dp(
      dp=dpValve_closing,
      k=k_max,
...
equation
  m_flow_smooth=noEvent(smooth(2,
    if dp <= 0 then AixLib.Fluid.BaseClasses.FlowModels.basicFlowFunction_dp(
      dp=dp,
      k=k_min,
      m_flow_turbulent=m_flow_turbulent)
    elseif dp >= dpValve_closing then AixLib.Fluid.BaseClasses.FlowModels.basicFlowFunction_dp(
      dp=dp,
      k=k_max,
      m_flow_turbulent=m_flow_turbulent)
