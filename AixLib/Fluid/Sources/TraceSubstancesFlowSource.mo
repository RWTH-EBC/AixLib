...
<p>
  This model can be used to inject trace substances into a system. The model adds a mass flow rate to its port with a trace substance concentration of <i>1</i>.
</p>
<h4>
  Typical use and important parameters
</h4>
<p>
  A typical use of this model is to add carbon dioxide to room air, since the carbon dioxide concentration is typically so small that it need not be added to the room mass balance, and since the mass flow rate can be made small compared to the room volume if the medium that leaves this component has a carbon dioxide concentration of <i>1</i>. The parameter <code>substanceName</code> must be set to the name of the substance that is injected into the fluid.
</p>
<p>
  Note however that mixing volumes from the package <a href=\"modelica://AixLib.Fluid.MixingVolumes\">AixLib.Fluid.MixingVolumes</a> allow to directly add a trace substance mass flow rate, which is more efficient than using this model.
...
<ul>
  ...
  <li>October 30, 2015, by Matthis Thorade:<br/>
    Removed <code>nPorts=1</code> in extension of the base class as the default must be <i>0</i>. This avoids a warning in the pedantic model check of Dymola 2016.
  </li>
  <li>May 29, 2014, by Michael Wetter:<br/>
    Removed undesirable annotation <code>Evaluate=true</code>.
  </li>
  <li>September 10, 2013, by Michael Wetter:<br/>
    Added missing <code>each</code> in declaration of <code>C_in_internal</code>. This eliminates a compilation error in OpenModelica.
  </li>
