...
model HeatPumpDetailed
 ...
 parameter SI.Volume volume_con=0.004
  ...
  parameter Boolean CorrFlowCo=false
    ...
    "Correction of mass flow different from nominal flow in condenser (polynomials: make sure mass flow rate is not corrected in polynomials)"
  ...
  parameter SI.MassFlowRate mFlow_conNominal=0.5
    ...
    "Nominal mass flow rate in condenser (only with polynomials, as already given in data tables)"  annotation(Dialog(group="Mass flow correction",tab="Advanced",enable=(CorrFlowCo and capCalcType==1)));



  parameter Boolean CorrFlowEv=false

    "Correction of mass flow different from nominal flow in evaporator (polynomials: make sure mass flow rate is not corrected in polynomials)"
...
<ol>
  <li>Icing and Defrosting: Simple model that uses the evaporator inlet temperature and calculates a factor for CoP correction (according to Wetter and Afjei, 1996). Not enabled for table data, as usually already included (data according EN255 and EN14511).
  </li>
  <li>Mass flow in condenser/evaporator: Model according to Pahud and Lachal, 2004, that corrects the temperatures used within table and polynomial. If the mass flow rates are already taken into account in the chosen polynomial, this sould be deactivated!
  </li>
  ...
  <li>Scaling factor: A scaling facor is implemented for scaling of the heat pump power and capacity without effecting the heat pump efficiency which is not physically correct but may be helpful for rough calculation.
  </li>
  ...
  <li>Allowed sink temperature: A maximum condenser outlet temperature limits the condenser heat flow through a PID controller.
  </li>
</ol>
<h4>
...
<ul>
  ...
  <li>Defrost: No direct implementation of defrost behavior. This heat pump does not operate in a defrost mode. Only a general CoP correction can be chosen therefore.
  </li>
</ul>
<h4>
...
<p>
  See <a href=\"AixLib.Fluid.HeatExchangers.Examples.TestHeatPump\">AixLib.Fluid.HeatExchangers.Examples.TestHeatPump</a>
