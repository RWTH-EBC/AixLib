...
model UHPM
  ...
  extends
    ...
    final use_rev=false,
    redeclare final AixLib.Fluid.HeatPumps.ModularReversible.BaseClasses.RefCycleUHPM refCyc(
      final refrigerant=refrigerant,
      final TConOutNom=TConOutNom,
      final TEvaInNom=TEvaInNom,
      final QConNom=QConNom,
  ...
  parameter Modelica.Units.SI.HeatFlowRate QCoo_flow_nominal(max=0)=0
   ...
   model RefrigerantCycleHeatPumpHeating =
      AixLib.Fluid.HeatPumps.ModularReversible.RefrigerantCycle.UHPM(
       ...
       final cpCon=cpCon,
       final cpEva=cpEva) "Refrigerant cycle module for the heating mode"
