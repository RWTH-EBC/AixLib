...
model Carnot_RE1
 ...
 package Medium2 = AixLib.Media.Water "Medium model";
  ...
  Carnot_TCon_RE heaPum(redeclare package Medium1 = Medium1,
    ...
    dp1_nominal=6000,
    dp2_nominal=6000,
    Q_heating_nominal=QCon_flow_nominal,
    Q_cooling_nominal=-1000,
    m2_flow_nominal=2)
...
equation
  ...
  connect(sou1.ports[1], heaPum.port_a1) annotation (Line(points={{-40,6},{-26,6},
          {-26,4},{-10,4}}, color={0,127,255}));
  connect(sou2.ports[1], heaPum.port_a2) annotation (Line(points={{40,-6},{26,-6},
          {26,-8},{10,-8}}, color={0,127,255}));
  connect(heaPum.port_b2, sin2.ports[1]) annotation (Line(points={{-10,-8},{-26,
          -8},{-26,-30},{-40,-30}}, color={0,127,255}));
  connect(heaPum.port_b1, sin1.ports[1]) annotation (Line(points={{10,4},{26,4},
          {26,30},{40,30}}, color={0,127,255}));
  connect(TConLvg.y, heaPum.TSet)
    annotation (Line(points={{-59,50},{-12,50},{-12,7}}, color={0,0,127}));
  connect(heaPum.QCon_flow, QEva_flow.u1) annotation (Line(points={{11,7},{22,7},
          {22,-32},{30,-32}}, color={0,0,127}));
  connect(heaPum.P, QEva_flow.u2) annotation (Line(points={{11,-2},{16,-2},{16,-44},
          {30,-44}}, color={0,0,127}));
