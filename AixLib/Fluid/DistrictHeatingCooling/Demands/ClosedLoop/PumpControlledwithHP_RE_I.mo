...
model PumpControlledwithHP_RE_I "Substation model for  low-temperature networks for buildings with 
...
public
  ...
  Utilities.Sensors.FuelCounter fuelCounter annotation (Placement(
        ...
        rotation=90,
        origin={-38,58})));
  Modelica.Blocks.Sources.RealExpression realExpression3(y=gain.y*(1 - opening.y))
    annotation (Placement(transformation(extent={{-244,-150},{-224,-130}})));
  Modelica.Blocks.Nonlinear.FixedDelay Delay(delayTime=5400)
    annotation (Placement(transformation(extent={{-214,-150},{-194,-130}})));
  Modelica.Blocks.Sources.RealExpression withDelay(y=gain.y*opening.y + Delay.y)
    annotation (Placement(transformation(extent={{-216,-186},{-196,-166}})));
  Modelica.Blocks.Sources.RealExpression erroy(y=withDelay.y + HZ.m_flow*
        cp_default*(senTem7.T - realinput.y))
    annotation (Placement(transformation(extent={{-214,-204},{-194,-184}})));
equation
  ...
  connect(realExpression3.y, Delay.u)
    annotation (Line(points={{-223,-140},{-216,-140}}, color={0,0,127}));
