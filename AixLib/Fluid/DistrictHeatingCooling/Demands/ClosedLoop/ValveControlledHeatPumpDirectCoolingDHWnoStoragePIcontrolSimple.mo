...
model ValveControlledHeatPumpDirectCoolingDHWnoStoragePIcontrolSimple
...
public
  ...
  Modelica.Blocks.Sources.RealExpression Sup_Temp_set(y=273.15 + 20)
    "Set Temperature of Supply" annotation (Placement(transformation(
        extent={{-21,-12},{21,12}},
        rotation=0,
        origin={-353,90})));
...
equation
  ...
  connect(cooling.y,T_HP_supply1. u2) annotation (Line(points={{-329,60},{-256.4,
          60}},             color={255,0,255}));
  connect(Sup_Temp_set.y, heat_ret.u2)
    annotation (Line(points={{-329.9,90},{-302,90}}, color={0,0,127}));
  connect(deltaT_Network.y, heat_ret.u1) annotation (Line(points={{-322.8,24},{-310,
          24},{-310,78},{-302,78}}, color={0,0,127}));
  connect(Sup_Temp_set.y, cold_ret.u1) annotation (Line(points={{-329.9,90},{-314,
          90},{-314,44},{-302,44}}, color={0,0,127}));
  connect(deltaT_Network.y, cold_ret.u2) annotation (Line(points={{-322.8,24},{-310,
          24},{-310,32},{-302,32}}, color={0,0,127}));
...
end ValveControlledHeatPumpDirectCoolingDHWnoStoragePIcontrolSimple;
