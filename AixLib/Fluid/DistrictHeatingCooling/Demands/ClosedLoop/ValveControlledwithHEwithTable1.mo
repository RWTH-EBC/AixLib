...
model ValveControlledwithHEwithTable1
...
public
  ...
  Modelica.Blocks.Math.Add T_return_sec annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-4,-70})));
  Modelica.Blocks.Sources.Constant const(k=273.15 + 15)
    annotation (Placement(transformation(extent={{-46,-98},{-26,-78}})));
  Modelica.Blocks.Sources.Constant limit(k=0) annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=-90,
        origin={66,-62})));
...
equation
  ...
  connect(Q_flow_input, add.u2) annotation (Line(points={{-86,84},{14,84},{14,
          88},{114,88},{114,-50},{106,-50}},
                           color={0,0,127}));
  connect(add.y, gain.u)
    annotation (Line(points={{100,-73},{100,-78},{90,-78}},
                                                   color={0,0,127}));
  ...
  connect(m_flow_building.y, division.u2) annotation (Line(points={{59,-18},{48,
          -18},{48,-82},{30,-82}}, color={0,0,127}));
  connect(division.y, T_return_sec.u2)
    annotation (Line(points={{7,-88},{2,-88},{2,-82}}, color={0,0,127}));
  connect(max.y, division.u1)
    annotation (Line(points={{49,-94},{30,-94}}, color={0,0,127}));
  connect(limit.y, max.u1)
    annotation (Line(points={{66,-73},{72,-73},{72,-88}}, color={0,0,127}));
  connect(T_return_sec.y, boundary.T_in)
    annotation (Line(points={{-4,-59},{32,-59},{32,-36}}, color={0,0,127}));
  connect(const.y, T_return_sec.u1)
    annotation (Line(points={{-25,-88},{-10,-88},{-10,-82}}, color={0,0,127}));
  connect(gain.y, max.u2) annotation (Line(points={{90,-101},{82,-101},{82,-100},
          {72,-100}}, color={0,0,127}));
  connect(efficient.y, add.u1) annotation (Line(points={{75,70},{108,70},{108,
          -30},{94,-30},{94,-50}}, color={0,0,127}));
...
end ValveControlledwithHEwithTable1;
