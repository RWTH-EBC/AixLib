...
model PumpControlledwithHP_DC_I "Substation model for  low-temperature networks for buildings with 
...
public
  ...
  Sensors.MassFlowRate HZ(redeclare package Medium = MediumBuilding)
    annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={-120,-170})));
  Modelica.Blocks.Sources.RealExpression withDelay(y=min(0, gain.y)*opening.y
         + Delay.y)
    annotation (Placement(transformation(extent={{-264,-202},{-244,-182}})));
  ...
  Modelica.Blocks.Sources.RealExpression realinput(y=max(278.15, senTem7.T +
        withDelay.y/(cp_default*m_flow_nominal)))
  ...
  Utilities.Sensors.EnergyMeter eva_HM annotation (Placement(transformation(
        ...
        rotation=90,
        origin={2,6})));
  Modelica.Blocks.Nonlinear.FixedDelay Delay(delayTime=7200)
    annotation (Placement(transformation(extent={{-236,-170},{-216,-150}})));
  Modelica.Blocks.Sources.RealExpression realExpression7(y=min(gain.y, 0)*(1 -
        opening.y))
    annotation (Placement(transformation(extent={{-276,-170},{-256,-150}})));
  Modelica.Blocks.Sources.RealExpression error(y=withDelay.y + HZ.m_flow*
        cp_default*(senTem7.T - realinput.y))
    annotation (Placement(transformation(extent={{-240,-222},{-220,-202}})));
equation
  ...
  connect(realinput.y, boundary1.T_in)
  ...
  connect(realExpression7.y, Delay.u)
    annotation (Line(points={{-255,-160},{-238,-160}}, color={0,0,127}));
