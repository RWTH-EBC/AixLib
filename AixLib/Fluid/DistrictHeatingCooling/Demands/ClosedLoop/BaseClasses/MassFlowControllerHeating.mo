...
model MassFlowControllerHeating
  "mass flow controller for substation with decentral storage for heating (SubstationHeatingCooling_DC_CH_HP_HeatStorage)"
    ...
    annotation (Placement(transformation(
        ...
        origin={-114,22}),   iconTransformation(extent={{-21,-21},{21,21}},
          origin={-121,-491})));
    Modelica.Blocks.Interfaces.RealInput P_renewable "Input signal of renewable power used for charging/discharging of storage"
    annotation (Placement(
        transformation(extent={{-142,196},{-102,236}}),iconTransformation(
          extent={{-146,192},{-102,236}})));
    ...
    Modelica.Blocks.Logical.Hysteresis hysteresis_discharge(uLow=T_storage_min + 0.1,
      uHigh=T_storage_max - 0.2)
    ...
    annotation (Placement(transformation(extent={{232,-62},{252,-42}})));
    ...
    Modelica.Blocks.Sources.RealExpression realExpression(y=P_storageCharging)
    "Electrical power theoretically available for storage charging"
    annotation (Placement(transformation(extent={{-54,114},{-30,138}})));
    Modelica.Blocks.Continuous.CriticalDamping criticalDamping(n=2, f=1/60)
    "Damping that makes model also run if P_storageCharging ist very near to 0 "
...
equation
  ...
  //charging
  if charging.y then
   m_flow_storage = -max(P_storageCharging*COP_charging/(cp_default*deltaT_he), (capacity-heat_Demand)/(cp_default*deltaT_he));
   Q_demandHP =heat_Demand - m_flow_storage*cp_default*10;
   Charging = true;
  ...
  connect(greater.y, charging.u1) annotation (Line(points={{333,94},{378,94},{378,
          70},{430,70}}, color={255,0,255}));
  connect(lessEqualThreshold.y, discharging.u1) annotation (Line(points={{253,-52},
          {298,-52},{298,-124},{422,-124}}, color={255,0,255}));
  ...
  connect(criticalDamping.y, greater.u1) annotation (Line(points={{51,126},{210,
          126},{210,94},{310,94}},
                                 color={0,0,127}));
  connect(criticalDamping.y, lessEqualThreshold.u) annotation (Line(points={{51,126},
          {210,126},{210,-52},{230,-52}},     color={0,0,127}));
  connect(not1.y, charging.u2) annotation (Line(points={{358.8,46},{378,46},{378,
          62},{430,62}}, color={255,0,255}));
  connect(realExpression.y, criticalDamping.u)
    annotation (Line(points={{-28.8,126},{28,126}}, color={0,0,127}));
  connect(const.y, greater.u2) annotation (Line(points={{253,72},{278,72},{278,86},
          {310,86}}, color={0,0,127}));
