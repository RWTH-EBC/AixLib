...
model PumpControlledwithHP_V1_ze "Substation model for  low-temperature networks for buildings with 
...
public
  ...
  Modelica.Blocks.Interfaces.RealOutput P_el
    annotation (Placement(transformation(extent={{212,88},{232,108}})));
  Modelica.Blocks.Sources.RealExpression realExpression6(y=fuelCounter.counter)
  ...
  Modelica.Blocks.Sources.RealExpression realExpression8(y=(-1/9*(limiter.y -
        273.15) + 1.15)/0.6)
    annotation (Placement(transformation(extent={{-218,54},{-198,74}})));
  Modelica.Blocks.Logical.Switch switch2
    annotation (Placement(transformation(extent={{-184,20},{-164,40}})));
  ...
  Modelica.Blocks.Nonlinear.Limiter limiter1(uMax=273.15 + 15, uMin=273.15 +
        10.5) annotation (Placement(transformation(extent={{34,78},{14,98}})));
...
equation
  ...
  connect(tan.heaPorSid, fixedTemperature.port) annotation (Line(points={{14,-171.6},
          {14,-176},{16,-176},{16,-188}},
                                    color={191,0,0}));
  connect(tan.port_a, senTem5.port_b) annotation (Line(points={{14,-156},{14,-158},
          {66,-158},{66,-152},{68,-152}},            color={0,127,255}));
  ...
  connect(prescribedHeatFlow1.port, vol2.heatPort) annotation (Line(points={{-78,22},
          {-78,14},{-76,14}},                  color={191,0,0}));
  connect(tan.port_b, senTem4.port_a) annotation (Line(points={{14,-176},{46,-176},
          {46,-180},{76,-180}}, color={0,127,255}));
  connect(senTem7.port_b, HZ.port_a)
    annotation (Line(points={{-120,-150},{-120,-158},{-116,-158}},
                                                       color={0,127,255}));
  connect(HZ.port_b, bou2.ports[1]) annotation (Line(points={{-116,-178},{-116,-204},
  ...
  connect(cooling.y, prescribedHeatFlow1.Q_flow)
    annotation (Line(points={{-95,40},{-98,40},{-98,22}},    color={0,0,127}));
  connect(realExpression6.y, P_el)
    annotation (Line(points={{183,98},{222,98}}, color={0,0,127}));
  connect(realExpression1.y, switch2.u3) annotation (Line(points={{-197,14},{-192,
          14},{-192,22},{-186,22}}, color={0,0,127}));
  connect(senTem.T, limiter.u)
    annotation (Line(points={{54,1},{50,1},{50,48},{40,48}}, color={0,0,127}));
  connect(booleanTable.y, switch2.u2)
    annotation (Line(points={{-223,30},{-186,30}}, color={255,0,255}));
  connect(senTem.T, limiter1.u)
    annotation (Line(points={{54,1},{46,1},{46,88},{36,88}}, color={0,0,127}));
  connect(realExpression9.y, dpOut)
    annotation (Line(points={{187,72},{226,72}}, color={0,0,127}));
  connect(realExpression8.y, switch2.u1) annotation (Line(points={{-197,64},{
          -190,64},{-190,52},{-186,52},{-186,38}}, color={0,0,127}));
  connect(port_a, val1.port_a)
    annotation (Line(points={{-260,0},{-166,0}}, color={0,127,255}));
  connect(val1.port_b, senTem2.port_a)
    annotation (Line(points={{-146,0},{-130,0}}, color={0,127,255}));
  connect(switch2.y, val1.y) annotation (Line(points={{-163,30},{-160,30},{-160,
          12},{-156,12}}, color={0,0,127}));
...
end PumpControlledwithHP_V1_ze;
