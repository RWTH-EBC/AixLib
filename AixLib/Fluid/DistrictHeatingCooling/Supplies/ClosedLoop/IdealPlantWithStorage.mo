...
model IdealPlantWithStorage
  ...
  AixLib.Fluid.HeatExchangers.PrescribedOutlet cooler(redeclare package Medium =
...
equation
  ...
  connect(heater.port_b, senTem1.port_b)
    annotation (Line(points={{62,0},{70,0}}, color={0,127,255}));
  connect(senTemStoHea.port_a, heater.port_a)
  ...
  connect(max.y, heater.TSet) annotation (Line(points={{32.6,34},{36,34},{36,8},
          {40,8}}, color={0,0,127}));
  ...
  connect(senTemStoCoo.port_b,cooler. port_a)
  ...
  connect(senTem.port_a,cooler. port_b)
    annotation (Line(points={{-92,0},{-86,0}}, color={0,127,255}));
  connect(cooler.TSet, min.y) annotation (Line(points={{-64,8},{-58,8},{-58,34},
...
This model represents the supply node of a bidirectional network with indeal heater and ideal cooler and storage tank in which the tank is integrated into the network directly.
The operation mode of the depends on the flow direction. In the case that port_b is the outlet, heating operation takes place. In the case that port_a is the outlet, cooling operation takes place. 
</html>"));
