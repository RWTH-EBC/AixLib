...
package ControlPump "Pump models with control block and new configuration approach"
...
<p>Using measurement or catalogue data of volume flow rate (m&sup3;/h), pump head (m), rotational speed set (rpm) and rotational speed act (rpm) a python script will generate a record with all necessary parameters for the model. Documentation with a visualization of the respective pump curves is included in the record.</p>
<p>In order to limit the pump&apos;s power consumption manufacturer normally will reduce the allowed pump speed for large volume flow rates, hence deforming the pump curves. This model takes the limitation into account based on the available catalogue or measurement data. Note that a design engineer normally only has catalogue data at his command.</p>
<p>This package is currently work in progress.</p>
<ul>
<li>Documentation needs updating.</li>
<li>Some models in Examples package might be transferred to a Validation package.</li>
<li>The names of the pump models might change in oder to better suite the naming convention in Fluid.Movers.</li>
<li>The Fluid.Movers package is designed around Michael Wetter&apos;s pump models. Integrating the pumps in this package directly might confuse users as it is not clear what the differences are. Especially intergrating models in the sub-packages might lead to confusion to which top level package they belong to. Therefore, we decided to keep them a separate. Maybe it is possible to merge ideas (stable mathematical approach) from the Fluid.Movers-pumps into here.</li>
</ul>
</html>", revisions="<html>
<ul>
