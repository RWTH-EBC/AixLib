...
model PumpN
  "pump with physics model and replaceable controller that uses pump speed as input"
  ...
  replaceable AixLib.Fluid.Movers.ControlPump.Controls.PumpControlNset pumpStateController(pumpParam=
       pumpParam) constrainedby Controls.BaseClasses.PumpStateController
    annotation (
  ...
  parameter Modelica.SIunits.Conversions.NonSIunits.AngularVelocity_rpm Nnom=
    ...
    N might be smaller than nMax for higher Q.</html>" annotation (Dialog(tab=
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_a(redeclare package Medium =
        Medium, m_flow(min=if physics.system.allowFlowReversal then -1.5*max(
          pumpParam.maxMinSpeedCurves[:, 1]) else 0))
    "Fluid connector a (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{-110,-10},{-90,10}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium =
        Medium, m_flow(max=if physics.system.allowFlowReversal then +1.5*max(
          pumpParam.maxMinSpeedCurves[:, 1]) else 0))
    "Fluid connector b (positive design flow direction is from port_a to port_b)"
    annotation (Placement(transformation(extent={{110,-10},{90,10}}),
        iconTransformation(extent={{110,-10},{90,10}})));
  BaseClasses.PumpControllerBus pumpBus annotation (Placement(transformation(
          extent={{-20,80},{20,120}}), iconTransformation(extent={{-20,80},{20,
            120}})));

  PumpPhysicsN physics(
    pumpParam=pumpParam,
    Qnom=Qnom,
    Nnom=Nnom,
    Hnom=Hnom,
    ...
    redeclare function efficiencyCharacteristic =
        BaseClasses.efficiencyCharacteristic.Wilo_Formula_efficiency)
    annotation (Placement(transformation(extent={{-30,-72},{30,-12}})));

equation
  connect(pumpBus, pumpStateController.pumpControllerBus) annotation (Line(
      points={{0,100},{0,60}},
      color={255,204,51},
      thickness=0.5));
  connect(pumpStateController.pumpBus, physics.pumpBus) annotation (Line(
      points={{0,20},{0,-12}},
      color={255,204,51},
      thickness=0.5));
  connect(port_a, physics.port_a) annotation (Line(points={{-100,0},{-66,0},{-66,
          -42},{-30,-42}}, color={0,127,255}));
  connect(physics.port_b, port_b) annotation (Line(points={{30,-42},{66,-42},{
...
<li>2018-01-26 by Peter Matthes:<br />* In calculation of m_flow_start changes reference to X_start into physics.X_start.<br />* Changes Nnom from 80 &#37; to 100 &#37; of Nmax.</li>
