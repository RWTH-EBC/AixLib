...
public
  ...
  Modelica.Blocks.Interfaces.RealOutput head(
    ...
    "Pump head";
  Modelica.Blocks.Interfaces.RealOutput headUnbound(
    quantity="Length",
    unit="m",
    min=0,
    max=constHmax,
    nominal=constHnom) "pump head before on/off switch";
...
protected
  ...
  Modelica.Blocks.Sources.Constant off(k=Modelica.Constants.small)
    annotation (Placement(transformation(extent={{-56,-33},{-46,-23}})));
  Modelica.Blocks.Continuous.CriticalDamping
    ...
    annotation (Placement(transformation(extent={{22,-49},{42,-29}})));
public
  Modelica.Blocks.Logical.Switch onOffHead "turns head variable off or on."
    annotation (Placement(transformation(extent={{-6,-49},{14,-29}})));
...
equation
  ...
  headUnbound =max(0, AixLib.Fluid.Movers.ControlPump.BaseClasses.polynomial2D(
    pumpParam.cHQN,
    Vflow_m3h.y,
    n));
  ...
  connect(pumpEfficiency.y, pumpBus.efficiency) annotation (Line(points={{-79,60},
      ...
      index=1,
      extent={{6,3},{6,3}}));
  connect(onOffHead.u2, pumpBus.onOff_Input) annotation (Line(points={{-8,-39},
          {-37,-39},{-37,-16},{75,-16},{75,100.597},{0.5975,100.597}},color={255,
          0,255}), Text(
      string="%second",
      index=3,
      extent={{6,3},{6,3}}));
  connect(off.y, onOffHead.u3) annotation (Line(points={{-45.5,-28},{-41,-28},{-41,
          -47},{-8,-47}}, color={0,0,127}));
  ...
  connect(variableLimiter.u, pumpBus.rpm_Input) annotation (Line(points={{-12,20},
      ...
      index=1,
      extent={{6,3},{6,3}}));
  connect(onOffHead.u1, headUnbound)
    annotation (Line(points={{-8,-31},{-14,-31}}, color={0,0,127}));
  connect(onOffHead.y, criticalDamping.u)
    annotation (Line(points={{15,-39},{20,-39}}, color={0,0,127}));
  ...
  connect(Vflow_m3h.y, pumpBus.vFRcur_m3h) annotation (Line(points={{-79,45},{0.5,
      ...
      index=1,
      extent={{6,3},{6,3}}));
  connect(off.y, onOffn.u3) annotation (Line(points={{-45.5,-28},{-41,-28},{-41,
  ...
  annotation (
    Diagram(coordinateSystem(
        ...
        Text(
          ...
          textString="n"),
        Polygon(
          points={{-24,-26},{-14,-31},{-24,-36},{-24,-26}},
          lineColor={95,95,95},
          fillColor={215,215,215},
          fillPattern=FillPattern.Solid),
        Text(
...
<li>2018-01-26 by Peter Matthes:<br>Changes Nnom from 80 &percnt; to 100 &percnt; of Nmax.</li>
<li>2018-01-16 by Peter Matthes:<br>Fixes power and efficiency calculation by using truth value &quot;<span style=\"font-family: Courier New;\">n&nbsp;&gt;=&nbsp;pumpParam.nMin*0.9<span style=\"color: #0000ff;\">&nbsp;and&nbsp;</span>pumpBus.onOff_Input&quot; instead of &quot;head &gt; 0.0&quot;.</li>
<li>2018-01-15 by Peter Matthes:<br>Changes minimum mass flow rate in ports to +/-<span style=\"font-family: Courier New;\">1.5*<span style=\"color: #ff0000;\">max</span>(pumpParam.maxMinSpeedCurves[:, 1]) in order to reduce search space.</li>
<li>2017-12-13 by Peter Matthes:<br>Adds assertions to check for unset pump record and improves the checks for cHQN matrix.</li>
<li>2017-12-12 by Peter Matthes:<br>* Changed Qnom from 1 m&sup3;/h to &quot;<span style=\"font-family: Courier New;\">0.5*<span style=\"color: #ff0000;\">max</span>(pumpParam.maxMinSpeedCurves[:,1])&quot;.<br>* Changed parameter name <i><span style=\"font-family: Courier New;\">n_start</i> to <i>Nnom</i> as it was only used to determin <i>Hnom</i>.<br>* Changed m_flow_start value from 1 to &quot;Qnom*<span style=\"color: #ff0000;\">Medium.density_pTX</span>(p_b_start,&nbsp;T_start,&nbsp;X_start)/3600&quot;.<br>* Changed p_b_start from system.p_start to &quot;p_a_start&nbsp;+&nbsp;Hnom*system.g_n*<span style=\"font-family: Courier New; color: #ff0000;\">Medium.density_pTX</span>(p_b_start,&nbsp;T_start,&nbsp;X_start)&quot; and changed p_start in medium state to p_b_start.<br>* Added assertions to check initialization parameters.</li>
