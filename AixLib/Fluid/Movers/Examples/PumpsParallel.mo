...
model PumpsParallel "Two flow machines in parallel"
  ...
  AixLib.Fluid.FixedResistances.PressureDrop dpOut1(
  ...
  AixLib.Fluid.FixedResistances.PressureDrop dpIn(
  ...
  AixLib.Fluid.FixedResistances.PressureDrop dpOut3(
  ...
  AixLib.Fluid.FixedResistances.PressureDrop dpIn2(
  ...
  AixLib.Fluid.Movers.SpeedControlled_y floMac2(
    ...
    inputType=AixLib.Fluid.Types.InputType.Constant) "Model of a flow machine"

    annotation (Placement(transformation(extent={{20,0},{40,20}})));

  AixLib.Fluid.FixedResistances.PressureDrop dpOut2(
...
This example tests the configuration of two flow machines that are installed in parallel. Both flow machines start with full speed. At <i>t=150</i> second, the speed of the flow machine on the top is reduced to zero. As its speed is reduced, the mass flow rate changes its direction in such a way that the flow machine at the top has reverse flow. </html>",revisions="<html>
<ul>
  ...
  <li>April 2, 2015, by Filip Jorissen:<br/>
    Set constant speed for pump using a <code>parameter</code> instead of a <code>realInput</code>.
  </li>
  <li>May 29, 2014, by Michael Wetter:<br/>
    Removed undesirable annotation <code>Evaluate=true</code>, and set <code>rho_nominal</code> to a constant to avoid a non-literal nominal value for <code>V_flow_max</code> and <code>VMachine_flow</code>.
  </li>
