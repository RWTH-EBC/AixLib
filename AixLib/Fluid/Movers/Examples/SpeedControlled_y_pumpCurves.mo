...
<p>
  This example demonstrates how the pump curves changes for different (constant) input signal <code>y</code>. If <code>y ≥ delta = 0.05</code>, the pump curves are polynomials. For <code>y &lt; delta = 0.05</code>, the pump curves convert to linear functions to avoid a singularity at the origin.
</p></html>",revisions="<html>
<ul>
  <li>December 2, 2016, by Michael Wetter:<br/>
    Changed the valve opening signal to not take on zero as otherwise <code>pum.port_a.p</code> is negative, violating the <code>min</code> attribute on the pressure variable.<br/>
  ...
  <li>June 14, 2015, by Filip Jorissen:<br/>
