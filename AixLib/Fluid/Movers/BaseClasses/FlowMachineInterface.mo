...
protected
  ...
  final parameter Real etaDer[size(per.efficiency.V_flow,1)]=
    if not per.etaHydMet==AixLib.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate
    ...
    elseif (size(per.efficiency.V_flow, 1) == 1)
      ...
      AixLib.Utilities.Math.Functions.splineDerivatives(
        ...
        ensureMonotonicity=AixLib.Utilities.Math.Functions.isMonotonic(
  ...
  final parameter Real motDer[size(per.motorEfficiency.V_flow, 1)]=
    if not per.etaMotMet==AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate
  ...
  final parameter Real motDer_yMot[size(per.motorEfficiency_yMot.y,1)]=
    if not per.etaMotMet==
      AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
  ...
  final parameter Real motDer_yMot_generic[9]=
    if per.etaMotMet==
      AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve
      or  (per.etaMotMet==
      AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.NotProvided
           and per.haveWMot_nominal)
      then AixLib.Utilities.Math.Functions.splineDerivatives(
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.etaHydMet==
      AixLib.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
     AixLib.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
  ...
  final parameter AixLib.Fluid.Movers.BaseClasses.Euler.powerWithDerivative powEu_internal=
    if (curve == 1) then
      AixLib.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur1)
    elseif (curve == 2) then
      AixLib.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur2)
    else
      AixLib.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,pressure=pCur3)
    "Intermediate parameter";
  final parameter AixLib.Fluid.Movers.BaseClasses.Characteristics.powerParameters powEu(
...
equation
  ...
  if (curve == 1) then
    if homotopyInitialization then
       V_flow*kRes + dp_internal = homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     V_flow=V_flow,
                                                     r_N=r_N,
                                                     dpMax=dpMax,
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1),
                                           simplified=r_N * (AixLib.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1)
                               +(V_flow-V_flow_nominal) * (AixLib.Fluid.Movers.BaseClasses.Characteristics.pressure(
                                                     ...
                                                     d=preDer1,
                                                     per=pCur1)
                                       -AixLib.Fluid.Movers.BaseClasses.Characteristics.pressure(V_flow=(1-delta)*V_flow_nominal,
                                                     r_N=1,
                                                     dpMax=dpMax,
                                                     V_flow_max=V_flow_max,
                                                     d=preDer1,
       ...
       V_flow*kRes + dp_internal= AixLib.Fluid.Movers.BaseClasses.Characteristics.pressure(V_flow=V_flow,
                                               r_N=r_N,
                                               dpMax=dpMax,
                                                     V_flow_max=V_flow_max,
                                               d=preDer1,
  ...
  if per.powerOrEfficiencyIsHydraulic then
    ...
    etaHyd = AixLib.Utilities.Math.Functions.smoothMin(
  ...
  if per.powerOrEfficiencyIsHydraulic then
    P_internal=WHyd;
    eta_internal=etaHyd;
    PEle = WFlo / AixLib.Utilities.Math.Functions.smoothMax(
                    x1=eta, x2=1E-2, deltaX=1E-3);
  else
    P_internal=PEle;
    eta_internal=eta;
    WHyd = WFlo / AixLib.Utilities.Math.Functions.smoothMax(
                    x1=etaHyd, x2=1E-2, deltaX=1E-3);
  end if;
  if per.etaHydMet==
       AixLib.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
    if homotopyInitialization then
      P_internal = homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow, r_N=r_N, d=powDer, delta=delta),
                      simplified=V_flow/V_flow_nominal*
                            AixLib.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow_nominal, r_N=1, d=powDer, delta=delta));
    else
      P_internal = (rho/rho_default)*AixLib.Fluid.Movers.BaseClasses.Characteristics.power(per=per.power, V_flow=V_flow, r_N=r_N, d=powDer, delta=delta);
    end if;
    eta_internal = WFlo/AixLib.Utilities.Math.Functions.smoothMax(
                     x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet==
       AixLib.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber then
    if homotopyInitialization then
      P_internal = homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow, r_N=r_N, d=powEuDer, delta=delta),
                      simplified=V_flow/V_flow_nominal*
                            AixLib.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow_nominal, r_N=1, d=powEuDer, delta=delta));
    else
      P_internal = (rho/rho_default)*AixLib.Fluid.Movers.BaseClasses.Characteristics.power(per=powEu, V_flow=V_flow, r_N=r_N, d=powEuDer, delta=delta);
    end if;
    eta_internal = WFlo / AixLib.Utilities.Math.Functions.smoothMax(
                            x1=P_internal, x2=deltaP, deltaX=deltaP/2);
  elseif per.etaHydMet == AixLib.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
      eta_internal = homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency,     V_flow=V_flow, d=etaDer, r_N=r_N, delta=delta),
                        simplified=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency, V_flow=V_flow_max,   d=etaDer, r_N=r_N, delta=delta));
    else
      eta_internal = AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.efficiency, V_flow=V_flow, d=etaDer, r_N=r_N, delta=delta);
    end if;
    P_internal=WFlo/AixLib.Utilities.Math.Functions.smoothMax(
  ...
  if per.etaMotMet == AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
      etaMot = homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency,     V_flow=V_flow, d=motDer, r_N=r_N, delta=delta),
                        simplified=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency, V_flow=V_flow_max,   d=motDer, r_N=r_N, delta=delta));
    else
      etaMot = AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency,     V_flow=V_flow, d=motDer, r_N=r_N, delta=delta);
    end if;
  elseif per.etaMotMet==
       AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio then
    if homotopyInitialization then
      etaMot =homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=yMot,
        d=motDer_yMot), simplified=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=1,
        d=motDer_yMot));
    else
      etaMot =AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
        per=per.motorEfficiency_yMot,
        y=yMot,
        d=motDer_yMot);
    end if;
  elseif per.etaMotMet==
       AixLib.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve then
      if homotopyInitialization then
        etaMot =homotopy(actual=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
          per=per.motorEfficiency_yMot_generic,
          y=yMot,
          d=motDer_yMot_generic), simplified=AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency_yMot(
          per=per.motorEfficiency_yMot_generic,
          y=1,
...
AixLib.Fluid.Movers.UsersGuide</a>
...
<a href=\"modelica://AixLib.Fluid.Movers.BaseClasses.PowerInterface\">
AixLib.Fluid.Movers.BaseClasses.PowerInterface</a>.
</li>
<li>
...
<code>etaMot = AixLib.Fluid.Movers.BaseClasses.Characteristics.efficiency(per=per.motorEfficiency, V_flow=V_flow, d=motDer, r_N=r_N, delta=1E-4)</code>
