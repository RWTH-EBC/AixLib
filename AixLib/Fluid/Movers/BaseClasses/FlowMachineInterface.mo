...
partial model FlowMachineInterface
  ...
  extends AixLib.Fluid.Movers.BaseClasses.PowerInterface(
  ...
  parameter Real powDer[size(power.V_flow,1)]=
   if use_powerCharacteristic then
     AixLib.Utilities.Math.Functions.splineDerivatives(
                   x=power.V_flow,
                   y=power.P,
                   ensureMonotonicity=AixLib.Utilities.Math.Functions.isMonotonic(x=power.P,
                                                                                     strict=false))
   else
...
initial algorithm
  ...
  assert(AixLib.Utilities.Math.Functions.isMonotonic(x=pressure.V_flow, strict=true) and
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1:=AixLib.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow, y=pCur1.dp);
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    pCur3.dp     := zeros(nOri + 2);
    preDer1:=zeros(nOri);
    preDer2:=AixLib.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow, y=pCur2.dp);
    preDer3:=zeros(nOri+2);
    ...
    preDer1:=zeros(nOri);
    preDer2:=zeros(nOri+1);
    preDer3:=AixLib.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow, y=pCur3.dp);
...
equation
  ...
  if use_powerCharacteristic then
    ...
    // a division by zero.
    eta = WFlo / AixLib.Utilities.Math.Functions.smoothMax(x1=P, x2=1E-5, deltaX=1E-6);
    ...
    // a division by zero.
    P = WFlo / AixLib.Utilities.Math.Functions.smoothMax(x1=eta, x2=1E-5, deltaX=1E-6);
...
<a href=\"modelica://AixLib.Fluids.Movers.BaseClasses.PrescribedFlowMachine\">PrescribedFlowMachine</a>.
</p>
<p>
