within AixLib.Fluid.Movers.BaseClasses.Euler;
function getPeak
  "Find peak condition from power characteristics"
  extends Modelica.Icons.Function;
  input AixLib.Fluid.Movers.BaseClasses.Characteristics.flowParameters
    pressure "Pressure vs. flow rate";
  input BaseClasses.Characteristics.powerParameters
    power "Power vs. flow rate";
  output AixLib.Fluid.Movers.BaseClasses.Euler.peak
...
protected
  ...
  parameter Modelica.Units.SI.PressureDifference dpHalFlo=
    if max(pressure.V_flow) < 1E-6
      then 0
    else AixLib.Utilities.Math.Functions.smoothInterpolation(
               x=V_flow_hal,
  ...
  elseif max(power.P) < 1E-6 then
    ...
    if nPre == nPow then
      ...
      if nPre > nPow then
        ...
        dp:= pressure.dp;
        for i in 1:n loop
          P[i]:= AixLib.Utilities.Math.Functions.smoothInterpolation(
                   x=V_flow[i],
                   xSup=power.V_flow,
                   ySup=power.P,
        ...
        P:= power.P;
        for i in 1:n loop
          dp[i]:= AixLib.Utilities.Math.Functions.smoothInterpolation(
                    x=V_flow[i],
                    xSup=pressure.V_flow,
                    ySup=pressure.dp,
                    ensureMonotonicity=true);
    ...
    eta:=V_flow.*dp./P;
    etaLes:=n<4;
    etaMon:=AixLib.Utilities.Math.Functions.isMonotonic(
      x=eta,
      strict=false);
    ...
    //   2. the available point with the highest efficiency.
      peak.eta:=AixLib.Utilities.Math.Functions.smoothInterpolation(
               x=V_flow_hal,
               xSup=V_flow,
               ySup=eta,
               ensureMonotonicity=
                 AixLib.Utilities.Math.Functions.isMonotonic(eta, strict=false));
      ...
      peak.eta:=AixLib.Utilities.Math.Functions.smoothInterpolation(
        ...
        ySup=eta,
        ensureMonotonicity=false);
      peak.dp:=AixLib.Utilities.Math.Functions.smoothInterpolation(
        x=peak.V_flow,
        xSup=V_flow,
        ySup=dp,
        ensureMonotonicity=false);
...
<a href=\"modelica://AixLib.Fluid.Movers.BaseClasses.Euler.peak\">
AixLib.Fluid.Movers.BaseClasses.Euler.peak</a>.
