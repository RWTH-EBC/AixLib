within AixLib.Fluid.Movers.BaseClasses.Euler;
 function power
   ...
   input AixLib.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal
     pressure "Pressure curve with both max flow rate and max pressure";
   output AixLib.Fluid.Movers.BaseClasses.Euler.powerWithDerivative power(
 ...
 algorithm
   ...
   for i in 2:10 loop
     power.P[i]:=V_flow[i] * dp[i]
                 / AixLib.Fluid.Movers.BaseClasses.Euler.efficiency(
                     peak=peak,
                     dp=dp[i],
                     V_flow=V_flow[i],
   ...
   power.d[2:10]:=AixLib.Utilities.Math.Functions.splineDerivatives(
                    x=V_flow[2:10],
                    y=power.P[2:10],
                    ensureMonotonicity=false);
