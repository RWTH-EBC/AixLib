...
initial algorithm
 ...
 motDer :=
   ...
   elseif ( size(_perPow.motorEfficiency.V_flow, 1) == 1)  then
      ...
      ensureMonotonicity=AixLib.Utilities.Math.Functions.isMonotonic(x=_perPow.motorEfficiency.eta,
                                                                        strict=false));
  hydDer :=
     if _perPow.use_powerCharacteristic then
...
equation
  ...
  if homotopyInitialization then
    Q_flow = homotopy(actual=AixLib.Utilities.Math.Functions.spliceFunction(pos=QThe_flow, neg=0,
                       x=noEvent(abs(VMachine_flow))-2*delta_V_flow, deltax=delta_V_flow),
                     simplified=0);
  else
    Q_flow = AixLib.Utilities.Math.Functions.spliceFunction(pos=QThe_flow, neg=0,
                       x=noEvent(abs(VMachine_flow))-2*delta_V_flow, deltax=delta_V_flow);
  end if;
  annotation (
