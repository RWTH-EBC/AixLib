...
model BaseModelStaticHeatPump
  ...
  parameter Boolean useTRetFlo=false
    "= true, if evaporator state is computed seperated from compressor's outlet"
  ...
  ModularCompressors.ModularCompressorsSensors modCom(
    ...
    redeclare model IsentropicEfficiency =
        Utilities.IsentropicEfficiency.SpecifiedEfficiencies.Buck_R134aR450aR1234yfR1234zee_VarDisVol_Scroll)
  ...
  Modelica.Blocks.Interaction.Show.RealValue COP(
    ...
    number=inpHeaCap.y/modCom.modCom[1].comPro.PEle)
...
protected
  ...
  model StaticCondenser
    ...
    //
    parameter Boolean useTRetFlo = false
      "= true, if evaporator state is computed by reurn flow temperature"
      annotation(Dialog(tab="General",group="General"));
    ...
    Modelica.Blocks.Sources.RealExpression inlTCon(y=TInl)
  ...
  public
    ...
    Modelica.SIunits.HeatFlowRate Q_flow_con=
      m_flow*(inStream(port_a.h_outflow)-hOut)
      "Heat capacity (control value)";
...
equation
  ...
  connect(inpHeaCap.y, con.inpQ_flow)
    annotation (Line(points={{-69,-42},{-69,-42},{90,-42},{90,88},{-18,88},{-18,
          80}},                                      color={0,0,127}));
