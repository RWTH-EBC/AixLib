...
polynomial and a power approach.
A generic polynomial approach is presented below:<br /><br />
<code>&eta;<sub>eng</sub> = corFact * 
sum(a[i]*P[i]^b[i] for i in 1:nT)</code><br /><br />
A generic power approach is presented below:<br /><br />
<code>&eta;<sub>eng</sub> = corFact * a * 
product(P[i]^b[i] for i in 1:nT)</code><br /><br />
All engine efficiency models presented in this library are based
...
<code>&eta;<sub>eng</sub> = Q&#775;<sub>ref</sub> / 
P<sub>el</sub></code> or from <code>&eta;<sub>eng</sub> = 
Q&#775;<sub>refIse</sub> / P<sub>el</sub></code>, respectively.
</p>
...
<h4>Polynomial engine efficiency models</h4>
<p>
...
and expand the <code>if-structure</code> defined in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.EngineEfficiency.PolynomialEngineEfficiency\">
...
<h4>Power engine efficiency models</h4>
<p>
...
AixLib.Fluid.Movers.Compressors.Utilities.Types</a>
and expand the <code>if-structure</code> defined in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.EngineEfficiency.PowerEngineEfficiency\">
...
polynomial and a power approach.
A generic polynomial approach is presented below:<br /><br />
<code>&eta;<sub>vol</sub> = corFact * 
sum(a[i]*P[i]^b[i] for i in 1:nT)</code><br /><br />
A generic power approach is presented below:<br /><br />
<code>&eta;<sub>vol</sub> = corFact * a * 
product(P[i]^b[i] for i in 1:nT)</code><br /><br />
...
<h4>Polynomial volumetric efficiency models</h4>
<p>
Actually, four polynomial approaches are implemented in this package.
To add further calculation procedures, just add its name in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.Types\">
AixLib.Fluid.Movers.Compressors.Utilities.Types</a>
and expand the <code>if-structure</code> defined in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.VolumetricEfficiency.PolynomialVolumetricEfficiency\">
...
<h4>Power volumetric efficiency models</h4>
<p>
Actually, one power approache is implemented in this package.
To add further calculation procedures, just add its name in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.Types\">
AixLib.Fluid.Movers.Compressors.Utilities.Types</a>
and expand the <code>if-structure</code> defined in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.VolumetricEfficiency.PowerVolumetricEfficiency\">
...
polynomial and a power approach.
A generic polynomial approach is presented below:<br /><br />
<code>&eta;<sub>ise</sub> = corFact * 
sum(a[i]*P[i]^b[i] for i in 1:nT)</code><br /><br />
A generic power approach is presented below:<br /><br />
<code>&eta;<sub>ise</sub> = corFact * a * 
product(P[i]^b[i] for i in 1:nT)</code><br /><br />
...
<h4>Polynomial isentropic efficiency models</h4>
<p>
...
AixLib.Fluid.Movers.Compressors.Utilities.Types</a>
and expand the <code>if-structure</code> defined in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.IsentropicEfficiency.PolynomialIsentropicEfficiency\">
...
<h4>Power isentropic efficiency models</h4>
<p>
Actually, one power approache is implemented in this package.
To add further calculation procedures, just add its name in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.Types\">
AixLib.Fluid.Movers.Compressors.Utilities.Types</a>
and expand the <code>if-structure</code> defined in
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.Utilities.IsentropicEfficiency.PowerIsentropicEfficiency\">
...
<p>
Compressor models implemented in this library often use three different
efficiencies to calculate the mass flow rate, thermodynamic change of 
state and power consumption of the compressor. In the following, all 
efficiency models implemented in this library are shortly summarised.
Furthermore, all compressor models have a parameter to calculate 
transient behaviour of changing the rotational speed. This approach is
also summarised in this information section.
</p>
<h4>Efficiency modeling approaches</h4>
<p>
Actually, three different efficiency models are suggested and the modelling
approaches of these efficiencies are shortly characterised below:<br />
</p>
<table summary=\"Efficiencies\" border=\"1\" cellspacing=\"0\" 
cellpadding=\"2\" style=\"border-collapse:collapse;\">
<tr>
<th>Efficiency</th>
<th>Formula</th> 
...
<tr>
<td><b>Engine</b></td> 
<td><code>&eta;<sub>eng</sub> = Q&#775;<sub>ref</sub> / 
P<sub>el</sub></code></td> 
<td>Used for calculation of compressor's power consumption</td> 
</tr> 
<tr>
<td><b>Isentropic</b></td> 
<td><code>&eta;<sub>ise</sub> = (h<sub>outIse</sub> - h<sub>inl</sub>) /
(h<sub>out</sub> - h<sub>inl</sub>)</code></td> 
<td>Used for calculation of thermodynamic change of state</td> 
</tr> 
<tr>
<td><b>Volumetric</b></td>
<td><code>&eta;<sub>vol</sub> = V&#775;<sub>ide</sub> / 
V&#775;<sub>rea</sub></code></td> 
<td>Used for calculation of mass flow rate</td> 
</tr> 
</table>
<p>
...
Therefore, the calculation procedure of the efficiencies are introduced as
replaceable models and must be defined by the User. Further information
is given in the following sections:
</p>
<ol>
<li>
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.UsersGuide.Approaches.MechanicEfficiency\">
Engine efficiencies</a></li>
<li>
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.UsersGuide.Approaches.VolumetricEfficiency\">
Volumetric efficiencies</a></li>
<li>
<a href=\"modelica://AixLib.Fluid.Movers.Compressors.UsersGuide.Approaches.IsentropicEfficiency\">
Isentropic efficiencies</a></li>
</ol>
...
<i>Approach of calculating efficiency</i> _ 
<i>Valid refrigerants</i> _
<i>Displacement volume</i> _
<i>Type of compressor</i>
</p>
<ol>
<li><u>Approach:</u> Approach of calculating efficiency,
e.g. polynomial or power.</li>
<li><u>Refrigerant:</u> Refrigerants the efficiency model is
valid for, e.g. R134a or R410a.</li>
<li><u>Displacement volume:</u> Displacement volume the
efficiency model is valid for, e.g. 170cm³.</li>
<li><u>Type:</u> Type of compressor,
e.g. scroll compressor.</li>
</ol>
<h4>Transient behaviour</h4>
<p>
The base model has a parameter <code>useInpFil</code> that is used to model
the compressors's transient behaviour while changing rotational speed. 
Generally, this approach uses the same modeling attempt as the stat-up and 
shut-down transients introtuced for flow machines (see 
<a href=\"modelica://AixLib.Fluid.Movers.UsersGuide\">
AixLib.Fluid.Movers.UsersGuide</a>). Therefore, just the parameter's 
affections are presented here:
</p>
<ol>
<li>
If <code>useInpFil=false</code>, then the input signal <code>opeSet.y</code> 
is equal to the compressor's rotational speed. Thus, a step change in the 
input signal causes a step change in the rotational speed.
</li>
<li>
If <code>useInpFil=true</code>, which is the default, then the rotational 
speed is equal to the output of a filter. This filter is implemented as a 
2nd order differential equation. Thus, a step change in the compressor's 
input signal will cause a gradual change in the rotational speed. The filter 
has a parameter <code>risTim</code>, which by default is set to <i>1</i> second. 
The rise time is the time required to reach <i>99.6%</i> of the full rotational 
speed, or, if the compressor is shut-down, to reach a rotational speed
of <i>0.4%</i>.
</li>
</ol>
</html>"));
