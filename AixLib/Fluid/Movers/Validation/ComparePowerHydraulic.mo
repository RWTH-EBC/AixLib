...
model ComparePowerHydraulic
  ...
  extends AixLib.Fluid.Movers.Validation.BaseClasses.ComparePower(
    redeclare replaceable AixLib.Fluid.Movers.Data.Fans.Greenheck.BIDW15 per(
    ...
    redeclare AixLib.Fluid.Movers.SpeedControlled_y mov1(
      ...
      use_riseTime=false),
    redeclare AixLib.Fluid.Movers.SpeedControlled_y mov2(
      redeclare final package Medium = Medium,
      per(
      ...
      addPowerToMedium=false,
      use_riseTime=false),
    redeclare AixLib.Fluid.Movers.SpeedControlled_y mov3(
      redeclare final package Medium = Medium,
      per(
        powerOrEfficiencyIsHydraulic=per.powerOrEfficiencyIsHydraulic,
        pressure=per.pressure,
  ...
  final parameter AixLib.Fluid.Movers.BaseClasses.Euler.peak peak =
    AixLib.Fluid.Movers.BaseClasses.Euler.getPeak(
      pressure=per.pressure,
      power=per.power)
...
equation
  ...
  annotation(experiment(Tolerance=1e-6, StopTime=200),
    __Dymola_Commands(file="modelica://AixLib/Resources/Scripts/Dymola/Fluid/Movers/Validation/ComparePowerHydraulic.mos"
        "Simulate and plot"),
