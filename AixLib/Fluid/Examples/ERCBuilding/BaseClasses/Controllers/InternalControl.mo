within AixLib.Fluid.Examples.ERCBuilding.BaseClasses.Controllers;
model InternalControl "Internal control model for boiler"
  ...
  Modelica.Blocks.Interfaces.RealInput Tflow_set(
    ...
    final unit="K",
    displayUnit="degC") "Target temperature of the controller"
  ...
  Modelica.Blocks.Interfaces.RealOutput QflowHeater(
    final unit="W") "Connector of real output signal"
  ...
  Modelica.Blocks.Interfaces.RealInput mFlow(
    ...
    final unit="kg/s") "Mass flow through the boiler"
  ...
  Modelica.Blocks.Interfaces.RealInput TFlowCold(
    final quantity="ThermodynamicTemperature",
    final unit="K",
    displayUnit="degC") "Temperature of the cold water"
    annotation (Placement(transformation(
          extent={{-125,-51.5},{-85,-11.5}}), iconTransformation(extent={{114.5,
            -30},{87,-2.5}})));
  AixLib.Controls.Continuous.PITemp ControlerHeater(
    final KR=KR,
    final TN=TN,
  ...
  AixLib.Utilities.Sensors.EnergyMeter eEnergyMeter_P
    "For primary energy consumption"
    annotation (Placement(transformation(extent={{30,63},{49.5,84}})));
  AixLib.Utilities.Sensors.EnergyMeter eEnergyMeter_S
