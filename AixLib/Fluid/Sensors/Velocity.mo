...
<p>
  This model outputs the flow velocity flowing from <code>port_a</code> to <code>port_b</code>.
</p>
<p>
  The sensor is ideal, i.e., it does not influence the fluid. If the parameter <code>tau</code> is non-zero, then the measured density that is used to convert the mass flow rate into volumetric flow rate is computed using a first order differential equation. Setting <code>tau=0</code> is <i>not</i> recommend. See <a href=\"modelica://AixLib.Fluid.Sensors.UsersGuide\">AixLib.Fluid.Sensors.UsersGuide</a> for an explanation.
</p>
<h4>
  Typical use and important parameters
</h4>
<p>
  In order to use the flow velocity sensor, the cross sectional area of the flow channel <code>crossSection</code> must be specified in <code>m<sub>2</sub></code>. For a circular flow channel (e.g. a round pipe) of diameter <code>diameter</code>, the cross sectional area can be specified as <code>crossSection = diameter * diameter / 4 * Modelica.Constants.pi</code>. See <a href=\"modelica://AixLib.Fluid.Sensors.Examples.FlowVelocity\">AixLib.Fluid.Sensors.Examples.FlowVelocity</a> for an example implementation with <code>diameter = 0.1</code>.
</p></html>",revisions="<html>
<ul>
  <li>May 20, 2016, by Marcus Fuchs:<br/>
    First implementation. Implementation is based on <a href=\"modelica://AixLib.Fluid.Sensors.VolumeFlowRate\">AixLib.Fluid.Sensors.VolumeFlowRate</a>.
  </li>
