...
<p>
  This model outputs the <i>latent</i> enthalphy flow rate of the medium in the flow between its fluid ports. In particular, if the total enthalpy flow rate is
</p>
<p align=\"center\" style=\"font-style:italic;\">
  Ḣ<sub>tot</sub> = Ḣ<sub>sen</sub> + Ḣ<sub>lat</sub>,
</p>
<p>
  where <i>Ḣ<sub>sen</sub> = ṁ (1-X<sub>w</sub>) c<sub>p,air</sub></i>, then this sensor outputs <i>Ḣ = Ḣ<sub>lat</sub></i>.
</p>
<p>
  If the parameter <code>tau</code> is non-zero, then the measured specific latent enthalpy <i>h<sub>out</sub></i> that is used to compute the latent enthalpy flow rate <i>Ḣ<sub>lat</sub> = ṁ h<sub>out</sub></i> is computed using a first order differential equation. See <a href=\"modelica://AixLib.Fluid.Sensors.UsersGuide\">AixLib.Fluid.Sensors.UsersGuide</a> for an explanation.
</p>
<p>
  ...
  For a sensor that measures <i>Ḣ<sub>sen</sub></i>, use <a href=\"modelica://AixLib.Fluid.Sensors.SensibleEnthalpyFlowRate\">AixLib.Fluid.Sensors.SensibleEnthalpyFlowRate</a>.
</p>
<p>
