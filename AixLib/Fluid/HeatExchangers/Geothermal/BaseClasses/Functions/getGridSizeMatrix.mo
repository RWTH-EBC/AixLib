...
function getGridSizeMatrix
  "Function returning a Matrix containing the grid dimensions"
  ...
  input Integer zSteps "Discretizations in z-Direction";
  input Integer selectOutput "1 for xSizeGrid, 2 for ySizeGrid";

  output Real[:,:,:] outputMatrix;
...
protected
  ...
  Real previousElement "Temporary variable";
...
algorithm
  ...
  if selectOutput == 1 then
    outputMatrix :={{{xWidth[x] for z in 1:zSteps} for y in 1:size(yWidth, 1)} for x in 1:size(xWidth, 1)};
//    Modelica.Utilities.Streams.print(Modelica.Math.Vectors.toString(xWidth),"outX.txt");
  elseif selectOutput == 2 then
    outputMatrix :=fill(transpose(fill(yWidth, zSteps)), size(xWidth, 1));
//    Modelica.Utilities.Streams.print(Modelica.Math.Vectors.toString(yWidth),"outY.txt");
  end if;

  annotation (Documentation(info="<html>
<p><b><font style=\"color: #008000; \">Overview</font></b> </p>
<p>Function returning a Matrix containing the grid dimensions.</p>
