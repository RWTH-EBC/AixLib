...
function alphaAnnular1
  "single phase medium in annular gap Boundary condition 1 (outer pipe isolated)"
  extends
...
algorithm
  ...
  d_h :=d_o - d_i;
  // equations from VDI-Waermeatlas 3.0, 2006. Boundary condition 1 (outer pipe isolated).
  // laminar or turbulent flow? no negative velocity.
  Re := abs(v) * d_h / nue;
  f := 1.615 * (1+0.14*sqrt(d_o/d_i));

  if Re < 2320 then // laminar flow range
    Nu_m := ( (3.66 + 1.2*(d_i/d_o)^(-0.8))^3 + ( f*(Re*Pr*d_h/L)^(1/3))^3) ^(1/3);
    ...
    if Re < 10e4 and Re >= 2300 then // transition flow range
      Nu_Rt :=xi/8*10^4*Pr/(1 + 12.7*sqrt(xi/8)*(Pr^(2/3) - 1))*(1 + (d_h/L)^(2
        /3));
      Nu_m:=(1 - gamma)*((3.66 + 1.2*(d_i/d_o)^(-0.8))^3 + (f*(2300*Pr*d_h/L)^(
        1/3))^3)^(1/3) + gamma*(Nu_Rt*0.86*(d_o/d_i)^0.16);
    else
      Nu_R :=xi/8*Re*Pr/(1 + 12.7*sqrt(xi/8)*(Pr^(2/3) - 1))*(1 + (d_h/L)^(2/3));
      Nu_m := Nu_R * 0.86*(d_o/d_i)^0.16;
    end if;
...
<p>Source:</p>
<ul>
<li>&QUOT;single&nbsp;phase&nbsp;medium&nbsp;in&nbsp;annular&nbsp;gap&nbsp;Boundary&nbsp;condition&nbsp;1&nbsp;(outer&nbsp;pipe&nbsp;isolated)&QUOT; according to VDI-Waermeatlas&nbsp;3.0,&nbsp;2006.</li>
</ul>
</html>",
  revisions="<html>
<p><ul>
