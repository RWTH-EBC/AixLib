...
function alphaAnnular2
  "single phase medium in annular gap Boundary condition 2 (inner pipe isolated)"
 extends
...
algorithm
 ...
 if Re < 2320 then // laminar flow range
   Nu_m := ( (3.66 + 1.2*(d_i/d_o)^(0.5))^3 + ( f*(Re*Pr*d_h/L)^(1/3))^3) ^(1/3);
...
<p>Source:</p>
<ul>
<li>&QUOT;single&nbsp;phase&nbsp;medium&nbsp;in&nbsp;annular&nbsp;gap&nbsp;Boundary&nbsp;condition&nbsp;2&nbsp;(inner&nbsp;pipe&nbsp;isolated)&QUOT; according to VDI-Waermeatlas&nbsp;3.0,&nbsp;2006.</li>
</ul>
</html>",
  revisions="<html>
<p><ul>
