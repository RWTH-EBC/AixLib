...
model CounterCurrentHeatExchanger "Validation model of a secondary fluid cell working in counter-current 
  heat exchanger mode"
  ...
  Utilities.FluidCells.SecondaryFluidCell couCur(
...
equation
  ...
  //
  connect(sou.ports[1], couCur.port_b)
    annotation (Line(points={{40,40},{26,40},{10,40}}, color={0,127,255}));
  connect(sin.ports[1], couCur.port_a)
    annotation (Line(points={{-40,40},{-26,40},{-10,40}}, color={0,127,255}));
...
secondary fluid cell working in counter-current
heat exchanger mode.
</p>
</html>"), experiment(StopTime=25));
end CounterCurrentHeatExchanger;
