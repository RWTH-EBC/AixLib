...
algorithm
  if (flowRegime == Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.ParallelFlow)) then // parallel flow
    a := 0;
    eps := (1 - Modelica.Math.exp(-NTU*(1 + Z)))/(1 + Z);
  elseif (flowRegime == Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.CounterFlow)) then// counter flow
   // a is constraining Z since eps is not defined for Z=1.
    ...
    eps := (1 - Modelica.Math.exp(-NTU*(1 - a)))/(1 - a*Modelica.Math.exp(-NTU*(
      1 - a)));
  elseif (flowRegime == Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.CrossFlowUnmixed)) then
   a := NTU^(-0.22);
    eps := 1 - Modelica.Math.exp( ( Modelica.Math.exp( - NTU * Z * a)  - 1)  / (Z * a));
  elseif (flowRegime == Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.CrossFlowCMinUnmixedCMaxMixed)) then
    // cross flow, (single pass), CMax mixed, CMin unmixed. (Coil with one row.)
    a := 0;
    eps := (1 - Modelica.Math.exp(-Z*(1 - Modelica.Math.exp(-NTU))))/Z;
  elseif (flowRegime == Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.CrossFlowCMinMixedCMaxUnmixed)) then
    // cross flow, (single pass), CMin mixed, CMax unmixed.
    a := 0;
    eps := 1 - Modelica.Math.exp(-(1 - Modelica.Math.exp(-Z*NTU))/Z);
  elseif (flowRegime == Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.ConstantTemperaturePhaseChange)) then
    ...
    a := 0;
    eps := 0;
    assert(Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.ParallelFlow) <= flowRegime and
           flowRegime <= Integer(AixLib.Fluid.Types.HeatExchangerFlowRegime.ConstantTemperaturePhaseChange),
