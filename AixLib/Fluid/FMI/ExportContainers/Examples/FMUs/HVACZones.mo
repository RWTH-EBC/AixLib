...
<p>
  This example demonstrates how to export a model of an HVAC system that only provides convective cooling to two thermal zones. The example is similar to <a href=\"modelica://AixLib.Fluid.FMI.ExportContainers.Examples.FMUs.HVACZone\">AixLib.Fluid.FMI.ExportContainers.Examples.FMUs.HVACZone</a> except that is serves two thermal zones rather than one.
</p>
<p>
  The example extends from <a href=\"modelica://AixLib.Fluid.FMI.ExportContainers.HVACZones\">AixLib.Fluid.FMI.ExportContainers.HVACZones</a> which provides the input and output signals that are needed to interface the acausal HVAC system model with causal connectors of FMI. The instance <code>hvacAda</code> is the HVAC adapter that contains on the left a fluid port, and on the right signal ports which are then used to connect at the top-level of the model to signal ports which are exposed at the FMU interface.
