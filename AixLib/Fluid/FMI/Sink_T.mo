...
<p>
  Model of a sink that takes as an input the medium properties temperature, mass fractions (if <code>Medium.nXi &gt; 0</code>) and trace substances (if <code>Medium.nC &gt; 0</code>). These properties are used during reverse flow.
</p>
<p>
  For a system of components with the connectors of the <a href=\"modelica://AixLib.Fluid.FMI\">AixLib.Fluid.FMI</a> package, this component is required to set the medium properties for the reverse flow.
</p>
<p>
  If the parameter <code>use_p_in</code> is set to <code>true</code>, then the model has an output connector <code>p</code>. This can be used to obtain the pressure of the sink, which may be needed to iteratively solve for the mass flow rate.
