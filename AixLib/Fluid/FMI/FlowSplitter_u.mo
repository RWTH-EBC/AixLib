...
info="<html>
<p>
  Block that takes as an input the inflowing fluid at the port <code>inlet</code> and a vector of control signals for the mass flow rates <code>u</code>. The mass flow of all outlet ports <code>i</code> is set to the value of <code>u[i] m_flow_nominal[i]</code>. If the inflowing mass flow rate at the port <code>inlet</code> is not equal to the sum of <code>u[i] m_flow_in[i]</code>, the simulation stops with an assert.
</p>
<h4>
...
<ul>
  <li>June 9, 2016, by Thierry S. Nouidui:<br/>
    Removed <code>assert()</code> statement which was triggered when the model was exported as an FMU depending on the execution sequence of the master algorithm.
  </li>
  <li>May 27, 2016, by Michael Wetter:<br/>
    Replaced <code>outlet[i].forward = inlet.forward;</code> with a <code>connect</code> statement as accesses to composite components other than records are not allowed. This was done for JModelica compliance.
  </li>
  <li>July 28, 2015, by Thierry S. Nouidui:<br/>
