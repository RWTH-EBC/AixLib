...
<p>
  Block that takes an inlet connector of type <a href=\"modelica://AixLib.Fluid.FMI.Interfaces.Inlet\">AixLib.Fluid.FMI.Interfaces.Inlet</a> and converts it to real outputs for properties of an air-based HVAC system.
</p>
<p>
  The output signal <code>m_flow</code> is equal to <code>inlet.m_flow</code>, whereas the output signals <code>T</code>, <code>X_w</code> and <code>C</code> are set the the properties <code>inlet.forward</code>. Similarly, the properties of <code>inlet.backward</code> are set the the values of the input signals <code>TAirZon</code>, <code>X_wZon</code> and <code>CZon</code>.
</p>
<p>
  If <code>allowFlowReversal = true</code>, input signal connectors are enabled that are used to set the fluid properties for backflow in the connector <code>inlet.backward</code>. This can be used if this block is used to connect an HVAC system to a thermal zone, in which case the supply air properties are in the connector <code>inlet.forward</code>, and the return air properties are in the connector <code>inlet.backward</code>.
</p>
<p>
  See <a href=\"modelica://AixLib.Fluid.FMI.Adaptors.ThermalZone\">AixLib.Fluid.FMI.Adaptors.ThermalZone</a> for its usage.
</p></html>",revisions="<html>
<ul>
  <li>June 28, 2016, by Michael Wetter:<br/>
