...
<p>
  This example demonstrates how to configure a model with a heater and a fan that causes a pressure rise in the air stream. Note that the mass flow rate and the absolute pressure are set by the component <code>sou</code>. Therefore, the fan simply increases the pressure of the medium, and it also computes how much power is needed for this pressure rise, which is an input to the fan model.
</p>
<p>
  For a model with that computes the mass flow rate based on the pressure drop, see <a href=\"modelica://AixLib.Fluid.FMI.Validation.HeaterFanPressureDriven\">AixLib.Fluid.FMI.Validation.HeaterFanPressureDriven</a>.
</p>
<p>
