...
model Admix "Admix circuit with three way valve and rpm controlled pump"
  ...
  replaceable BaseClasses.BasicPumpInterface basicPumpInterface(redeclare
    ...
    annotation (Dialog(group="Actuators"), choicesAllMatching=true, Placement(transformation(extent={{22,8},{
            46,32}})));
  ...
  FixedResistances.PlugFlowPipe
    ...
    final m_flow_nominal=m_flow_nominal)
                     annotation (Dialog(
        enable=true, group="Pipes"), Placement(transformation(extent={{-80,30},
            {-60,10}})));
  FixedResistances.PlugFlowPipe
                        pipe2(
    redeclare final package Medium = Medium,
    ...
    final m_flow_nominal=m_flow_nominal)
                     annotation (Dialog(
        enable=true, group="Pipes"), Placement(transformation(extent={{-10,30},
            {10,10}})));
  FixedResistances.PlugFlowPipe
                        pipe3(
    redeclare final package Medium = Medium,
    ...
    final m_flow_nominal=m_flow_nominal)
                     annotation (Dialog(
        enable=true, group="Pipes"), Placement(transformation(extent={{60,30},{
            80,10}})));
  FixedResistances.PlugFlowPipe
                        pipe4(
    redeclare final package Medium = Medium,
...
protected
  ...
  Sensors.VolumeFlowRate                VFSen_in(redeclare package Medium =
    ...
    T_start=T_start)
        ...
        rotation=90,
        origin={100,42})));
  MixingVolumes.MixingVolume          junc456(
    V=vol,
    redeclare package Medium = Medium,
    T_start=T_start,
    nPorts=3,
...
protected
  Modelica.Blocks.Continuous.FirstOrder Pt1Rtrn_in1(
    ...
    final T=tau)     annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-110,-30})));
  Modelica.Blocks.Continuous.FirstOrder Pt1Rtrn_in2(
    initType=Modelica.Blocks.Types.Init.SteadyState,
    y_start=T_start,
    final T=tau)     annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=270,
        origin={70,70})));
  Modelica.Blocks.Continuous.FirstOrder Pt1Rtrn_in3(
    initType=Modelica.Blocks.Types.Init.SteadyState,
    y_start=T_start,
    final T=tau)     annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        ...
        origin={-70,70})));
protected
  Modelica.Blocks.Continuous.FirstOrder Pt1Rtrn_in4(
    initType=Modelica.Blocks.Types.Init.SteadyState,
    y_start=T_start,
    final T=tau)     annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
...
equation
  ...
  connect(val.y, hydraulicBus.valveSet) annotation (Line(points={{-30,32},{-30,
          100},{-14,100},{-14,100},{0,100}},
                        color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}}));
  ...
  connect(basicPumpInterface.port_b, pipe3.port_a)
    annotation (Line(points={{46,20},{60,20}},    color={0,127,255}));
  connect(basicPumpInterface.pumpBus, hydraulicBus.pumpBus) annotation (Line(
      points={{34,32},{34,100.1},{0.1,100.1}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=1,
  ...
  connect(pipe2.ports_b[1], basicPumpInterface.port_a)
    annotation (Line(points={{10,20},{22,20}},color={0,127,255}));
  connect(pipe6.ports_b[1], val.port_3)
    annotation (Line(points={{-30,-10},{-30,10}}, color={0,127,255}));
  ...
  connect(pipe2.heatPort, prescribedTemperature.port)
    annotation (Line(points={{0,10},{0,-20}}, color={191,0,0}));
  ...
  connect(pipe4.heatPort, prescribedTemperature.port)
    annotation (Line(points={{30,-50},{0,-50},{0,-20}}, color={191,0,0}));
  ...
  connect(Pt1Rtrn_in1.u, senT_b2.T) annotation (Line(points={{-98,-30},{-84,-30},
          {-84,-53.4}}, color={0,0,127}));
  connect(senT_a1.T, Pt1Rtrn_in3.u) annotation (Line(points={{-94,26.6},{-82,
          26.6},{-82,58},{-70,58}},
                              color={0,0,127}));
  connect(senT_b1.T, Pt1Rtrn_in2.u) annotation (Line(points={{94,26.6},{86,26.6},
          {86,58},{70,58}}, color={0,0,127}));
  connect(Pt1Rtrn_in2.y, hydraulicBus.TFwrd_out) annotation (Line(points={{70,81},
          {70,100.1},{0.1,100.1}},         color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}}));
  connect(Pt1Rtrn_in3.y, hydraulicBus.TFwrd_in) annotation (Line(points={{-70,81},
          {-70,100},{0.1,100},{0.1,100.1}},        color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}}));
  connect(Pt1Rtrn_in1.y, hydraulicBus.TRtrn_out) annotation (Line(points={{-121,
          -30},{-122,-30},{-122,100},{0.1,100},{0.1,100.1}}, color={0,0,127}),
      Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}}));
  connect(senT_a2.T, Pt1Rtrn_in4.u)
    annotation (Line(points={{78,-53.4},{78,-30},{98,-30}}, color={0,0,127}));
  connect(Pt1Rtrn_in4.y, hydraulicBus.TRtrn_in) annotation (Line(points={{121,-30},
          {130,-30},{130,100},{116,100},{116,100.1},{0.1,100.1}},
                                                                color={0,0,127}),
      Text(
      string="%second",
      index=1,
  ...
  connect(pipe6.port_a, junc456.ports[2]) annotation (Line(points={{-30,-30},{
          -30,-60}},       color={0,127,255}));
  connect(pipe4.ports_b[1], junc456.ports[3])
    annotation (Line(points={{20,-60},{-27.8667,-60}}, color={0,127,255}));
