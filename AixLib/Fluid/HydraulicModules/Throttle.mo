...
public
  AixLib.Fluid.Actuators.Valves.TwoWayLinear val(
    ...
    dpValve_nominal=8000) annotation (Dialog(enable=true), Placement(
        transformation(extent={{-14,50},{6,70}})));
  FixedResistances.DPEAgg_ambientLoss pipe1(redeclare package Medium = Medium)
    annotation (Dialog(enable=true), Placement(transformation(extent={{-48,70},
            {-28,50}})));
...
equation
  connect(hydraulicBus.Tambient, prescribedTemperature.T) annotation (Line(
      ...
      index=-1,
      extent={{-6,3},{-6,3}}));
  connect(val.y, hydraulicBus.valveSet) annotation (Line(points={{-4,72},{-4,80},
          {-39.9,80},{-39.9,100.1}},     color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}}));
  connect(pipe1.heatPort_outside, pipe2.heatPort_outside) annotation (Line(
        points={{-36.4,54.4},{-36.4,40},{31.6,40},{31.6,54.4}},
                                                        color={191,0,0},visible=false));
  connect(prescribedTemperature.port, pipe2.heatPort_outside) annotation (Line(
        points={{-68,84},{-72,84},{-72,40},{31.6,40},{31.6,54.4}},
                                                               color={191,0,0},visible=false));
  connect(pipe3.heatPort_outside, pipe2.heatPort_outside) annotation (Line(
        points={{8.4,-54.4},{8.4,-40},{-72,-40},{-72,40},{31.6,40},{31.6,54.4}},
        color={191,0,0},visible=false));
  ...
  connect(pipe2.port_b, port_fwrdOut)
    annotation (Line(points={{40.4,60},{100,60}}, color={0,127,255}));
  ...
  connect(vFRfwrdIn.port_b, pipe1.port_a)
    annotation (Line(points={{-76,60},{-48.4,60}}, color={0,127,255}));
  connect(pipe1.port_b, val.port_a)
    annotation (Line(points={{-27.6,60},{-14,60}}, color={0,127,255}));
  connect(val.port_b, pipe2.port_a)
    annotation (Line(points={{6,60},{19.6,60}}, color={0,127,255}));
