...
model IndoorSwimmingPool2
  ...
  Modelica.Blocks.Interfaces.RealInput wavePool "Input profil wavePool "
    annotation (Placement(transformation(extent={{-120,-20},{-94,6}}),
  ...
  BaseClasses.waveMachine waveMachine(h_wave=poolParam.h_wave, w_wave=poolParam.w_wave)
    ...
    annotation (Placement(transformation(extent={{-74,-18},{-58,-2}})));
...
Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow preHeatFlowEvapLoss
  ...
  "Convection at the watersurface" annotation (Placement(transformation(
      ...
      rotation=90,
      origin={-14,74})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow preHeatFlowQPool
    "Return Lost Heat to Watercycle"
    annotation (Placement(transformation(extent={{-74,-50},{-54,-30}})));
  Modelica.Blocks.Math.MultiSum sumQ(nu=6) annotation (Placement(transformation(
        extent={{-4,-4},{4,4}},
        rotation=-90,
        origin={-86,-24})));
  ...
  Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor heatFlowSensor2
    annotation (Placement(transformation(
        ...
        rotation=0,
        origin={8,54})));
  Modelica.Blocks.Math.Gain gain(k=Medium.cp_const*(poolParam.T_pool - souFW.T))
    annotation (Placement(transformation(extent={{-88,-82},{-96,-74}})));
  Modelica.Blocks.Math.Gain gain1(k=Medium.cp_const*(poolParam.T_pool - souRW.T))
...
if poolParam.use_waterRecycling then
  ...
  connect(mFlowFW.port_b, watertreatmentWR.ports[4]) annotation (Line(points={{-36,-94},
            {-14,-94},{-14,-54},{-16.4,-54}},          color={0,127,255}));
  ...
  connect(mFlowFW.port_b, watertreatment.ports[4]) annotation (Line(points={{-36,-94},
            {-14,-94},{-14,-54},{-15,-54}},        color={0,127,255}));
...
connect(souFW.ports[1], mFlowFW.port_a)
  ...
  connect(openingHours,lunchbreak. u) annotation (Line(points={{-105,87},{
        -93.5,87},{-93.5,66},{-79.2,66}},
                                      color={0,0,127}));
  connect(wavePool, waveMachine.wavePool) annotation (Line(points={{-107,-7},{-100.5,
          -7},{-100.5,-6.8},{-74.64,-6.8}},
                                      color={0,0,127}));
  ...
  if poolParam.use_wavePool then
    connect(circPump.P, sumP.u[1]) annotation (Line(points={{20.32,-35.52},{20.32,
            -36},{20,-36},{20,-22},{54,-22},{54,-18.6}},
                                    color={0,0,127}));
    connect(waveMachine.PWaveMachine, sumP.u[2]) annotation (Line(points={{-57.52,
            -10},{20,-10},{20,-20},{54,-20},{54,-21.4}},
                                            color={0,0,127}));
...
connect(convWaterSurface.fluid, convPoolSurface)
  ...
  connect(circPump.port_b, poolWater.ports[4]) annotation (Line(points={{24,-36},
          {12,-36},{12,6},{1,6}}, color={0,127,255}));
  connect(sumQ.y, preHeatFlowQPool.Q_flow) annotation (Line(points={{-86,-28.68},
          {-86,-40},{-74,-40}}, color={0,0,127}));
  connect(poolWater.heatPort, heatFlowSensor.port_a)
    annotation (Line(points={{-12,16},{-45,16},{-45,32}}, color={191,0,0}));
  connect(heatFlowSensor.port_b, radWaterSurface.port_a)
    annotation (Line(points={{-45,42},{-45,66}}, color={191,0,0}));
  connect(heatFlowSensor.Q_flow, sumQ.u[1]) annotation (Line(points={{-50,37},{
          -86,37},{-86,-20},{-83.6667,-20}},
                                         color={0,0,127}));
  connect(convWaterSurface.solid, heatFlowSensor1.port_b)
    annotation (Line(points={{-14,68},{-14,66}}, color={191,0,0}));
  connect(heatFlowSensor1.port_a, poolWater.heatPort)
    annotation (Line(points={{-14,58},{-14,16},{-12,16}}, color={191,0,0}));
  connect(heatFlowSensor1.Q_flow, sumQ.u[2]) annotation (Line(points={{-18,62},{
          -30,62},{-30,58},{-84.6,58},{-84.6,-20}}, color={0,0,127}));
  connect(poolWater.heatPort, heatFlowSensor2.port_a) annotation (Line(points={{
          -12,16},{-14,16},{-14,54.16},{4,54}}, color={191,0,0}));
  connect(heatFlowSensor2.port_b, heatTransferConduction.heatport_a)
    annotation (Line(points={{12,54},{21,54},{21,54.16},{30,54.16}}, color={191,
          0,0}));
  connect(heatFlowSensor2.Q_flow, sumQ.u[3]) annotation (Line(points={{8,58},{
          -86,58},{-86,-20},{-85.5333,-20}},
                                         color={0,0,127}));
  connect(hEvapGain.y, sumQ.u[4]) annotation (Line(points={{64,34.4},{44,34.4},
          {44,30},{-88,30},{-88,-20},{-86.4667,-20}},color={0,0,127}));
  connect(getMFlowFW.y, gain.u) annotation (Line(points={{-57.3,-88},{-48,-88},{
          -48,-78},{-87.2,-78}},  color={0,0,127}));
  connect(gain.y, sumQ.u[5]) annotation (Line(points={{-96.4,-78},{-98,-78},{-98,
          -20},{-87.4,-20}}, color={0,0,127}));
  connect(gain1.y, sumQ.u[6]) annotation (Line(points={{-94.4,-50},{-96,-50},{
          -96,-20},{-88.3333,-20}},
                                color={0,0,127}));
  connect(getMFlowRW.y, gain1.u) annotation (Line(points={{-57.3,-60},{-52,-60},
          {-52,-50},{-85.2,-50}},                     color={0,0,127}));
  connect(sumQ.y, QPool) annotation (Line(points={{-86,-28.68},{-62,-28.68},{-62,
          -28},{110,-28}}, color={0,0,127}));
  connect(getTPool.y, TPool)
    annotation (Line(points={{94.7,12},{108,12}}, color={0,0,127}));
  connect(preHeatFlowQPool.port, watertreatmentWR.heatPort) annotation (Line(
        points={{-54,-40},{-42,-40},{-42,-44},{-28,-44}}, color={191,0,0}));
