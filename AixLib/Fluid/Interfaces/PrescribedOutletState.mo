...
equation
  ...
  if not restrictHeat and not restrictCool then
    ...
    m_flow_limited = AixLib.Utilities.Math.Functions.smoothMax(
    ...
    if restrictHeat and restrictCool then
      ...
      dhSetAct = AixLib.Utilities.Math.Functions.smoothLimit(
    ...
    elseif restrictHeat then
      ...
      dhSetAct = AixLib.Utilities.Math.Functions.smoothMin(
      ...
      dhSetAct = AixLib.Utilities.Math.Functions.smoothMax(
...
<p>
  This model sets the temperature of the medium that leaves <code>port_a</code> to the value given by the input <code>TSet</code>, subject to optional limitations on the heating and cooling capacity.
</p>
<p>
  In case of reverse flow, the set point temperature is still applied to the fluid that leaves <code>port_b</code>.
</p>
<p>
...
<p>
  This model has no pressure drop. See <a href=\"modelica://AixLib.Fluid.HeatExchangers.HeaterCooler_T\">AixLib.Fluid.HeatExchangers.HeaterCooler_T</a> for a model that instantiates this model and that has a pressure drop.
</p></html>",revisions="<html>
<ul>
  <li>January 26, 2016, by Michael Wetter:<br/>
    Removed inequality comparison of real numbers in <code>restrictCool</code> and in <code>restrictHeat</code> as this is not allowed in Modelica.
  </li>
