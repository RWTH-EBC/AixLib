...
<p>
  All valves are connected to a model of a motor with hysteresis. A more efficient implementation that approximates a motor but lacks hysteresis would be to set the valve parameter <code>filteredOpening=true</code> instead of using the motor model.
