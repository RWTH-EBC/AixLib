...
<code>C = corFact * sum(a[i]*P[i]^b[i] for i in 1:nT)</code><br /><br />
All flow coefficient models presented in this library are based
...
<code>m&#775; = C*A<sub>valve</sub>*sqrt(2*&rho;<sub>inlet</sub>*dp).
</code>
</p>
...
<tr>
<td><code>A</code></td> 
<td>Cross-sectional flow area</td> 
</tr> 
<tr>
<td><code>d<sub>inlet</sub></code></td> 
<td>Diameter of the pipe at valve's inlet</td> 
</tr> 
<tr>
<td><code>p<sub>inlet</sub></code></td> 
<td>Pressure at valve's inlet</td> 
</tr> 
<tr>
<td><code>p<sub>outlet</sub></code></td> 
<td>Pressure at valve's outlet</td> 
</tr> 
<tr>
<td><code>&rho;<sub>inlet</sub></code></td> 
<td>Density at valve's inlet</td> 
</tr> 
<tr>
<td><code>&rho;<sub>outlet</sub></code></td> 
<td>Density at valve's outlet</td> 
</tr> 
<tr>
<td><code>T<sub>inlet</sub></code></td> 
<td>Temperature at valve's inlet</td> 
</tr> 
<tr>
<td><code>&mu;<sub>inlet</sub></code></td> 
<td>Dynamic viscosity at valve's inlet</td> 
</tr> 
<tr>
<td><code>&sigma;<sub>inlet</sub></code></td> 
<td>Surface tension at valve's inlet</td> 
</tr> 
<tr>
<td><code>C<sub>outlet</sub></code></td> 
<td>Specific heat capacity at valve's outlet</td> 
</tr> 
<tr>
<td><code>h<sub>fg</sub></code></td> 
<td>Heat of vaparisation</td> 
</tr> 
</table>
<h4>Polynomial flow coefficient models</h4>
<p>
...
<a href=\"modelica://AixLib.Fluid.Actuators.Valves.ExpansionValves.Utilities.FlowCoefficient.PolynomialFlowCoefficient\">
...
<code>C = corFact * a * product(P[i]^b[i] for i in 1:nT)</code><br /><br />
All flow coefficient models presented in this library are based
on a literature review. Therefore, the variable <code>corFact</code>
allows a correction of the flow coefficient if the general modelling 
approach presented in the litarature differs from
...
<h4>Power flow coefficient models</h4>
<p>
...
<td><code>1.5 - 15 &deg;C</code></td> 
  ...
  class PolynomialApproachMetastability
    "Polynomial Approaches for metastability degree"
    extends Modelica.Icons.Information;
...
<p>
A generic polynomial approach is presented below:<br /><br />
<code>p<sub>th</sub> =  sum(a[i]*P[i]^b[i] for i in 1:nT)</code><br /><br />
All metastability models presented in this library are based
...
<tr>
<td><code>P<sub>up</sub></code></td> 
<td>Absolute upstream pressure (MPa)</td> 
</tr> 
<tr>
<td><code>&Delta;T<sub>sc</sub></code></td> 
<td>subcooled temperature (°C)</td> 
</tr> 
<tr>
<td>(<code>&Delta;T<sub>sc</sub></code>)&sup2;</td> 
<td>subcooled temperature (°C)</td> 
</tr> 
<tr>
<td><code>A<sub>th</sub></code></td> 
<td>geometric throat area (mm2)</td> 
</tr> 
 
</table>
<h4>Polynomial flow coefficient models</h4>
<p>
Actually, one polynomial approaches are implemented in this package.
...
<tr>
<td>ChenLiang 2009</td> 
<td><code>p<sub>th</sub> = a0 + a1*p<sub>up</sub>+ a2*<code>&Delta;T<sub>sc</sub></code>
+ a3*(<code>&Delta;T<sub>sc</sub></code>)&sup2;+a4*A<sub>th</sub>
...
<td><code>1.5 - 10 &deg;C</code></td> 
  ...
  end PolynomialApproachMetastability;

  class PowerApproachMetastability
    "Power Approaches for metastability degree"
    extends Modelica.Icons.Information;
...
<p>
A generic power approach is presented below:<br /><br />
<code>p<sub>th</sub> =  a * product(P[i]^b[i] for i in 1:nT)</code><br /><br />
All metastability degree models presented in this library are based
on a literature review.
</code>
</p>
<h4>Common model variables</h4>
...
<tr>
<td><code>p<sub>c</sub></code></td> 
<td>critical pressure</td> 
</tr> 
<tr>
<td><code>T<sub>sc</sub></code></td> 
<td>subcooling Temperature</td> 
</tr> 
<tr>
<td><code>T<sub>c</sub></code></td> 
<td>critical Temperature</td> 
</tr> 
<tr>
<td><code>&rho;<sub>l</sub></code></td> 
<td>Inlet liquid denisty</td> 
</tr> 
<tr>
<td><code>&rho;<sub>outlet</sub></code></td> 
<td>Density at valve's outlet</td> 
...
<tr>
<td><code>D;<sub>e</sub></code></td> 
<td>equivalent diameter</td> 
</tr> 
<tr>
<td><code>p<sub>th</sub></code></td> 
<td>Pressure at the throat</td> 
</tr> 

</table>
<h4>Power flow coefficient models</h4>
<p>
Actually, one power approaches are implemented in this package.
...
<tr>
<td>Chen Liang 2008</td> 
<td><code>p<sub>th</sub> = a*(p<sub>up</sub>/p<sub>c</sub>)^b1*(T<sub>sc</sub>/T<sub>c</sub>)^b2* (&rho;<sub>l</sub>/1000*p<sub>up</sub>/(C<sub>p,l</sub>*T<sub>c</sub>))^b3 
*(<code>&mu;<sub>inlet</sub></code>/(D<sub>e</sub>*sqrt(&rho;<sub>l</sub>*p<sub>up</sub>) *10^6))^b4*(D<sub>e</sub>/D)^b5 * p<sub>c</sub> 
...
<td><code>1.5 - 10 &deg;C</code></td> 
  ...
  end PowerApproachMetastability;
  annotation (Documentation(revisions="<html>
<ul>
  <li>
  October 19, 2017, by Mirko Engelpracht:<br/>
...
<p>
Expansion valve models implemented in this library use a flow coefficient
model by default to calculate the relationship between mass flow rate and
pressure drop. In the following, all flow coefficient models implemented
in this library are shortly summarised.
Furthermore, all expansion valve models have a parameter to calculate 
transient behaviour of opening and closing the valves. This approach is
...
<td><code>m&#775; = C*A<sub>valve</sub>*sqrt(2*&rho;<sub>inlet</sub>*dp)
</code></td> 
...
replaceable model and must by defined by the User. Further information
is given in the following sections:
</p>
<ol>
<li>
<a href=\"modelica://AixLib.Fluid.Actuators.Valves.ExpansionValves.UsersGuide.Approaches.PolynomialApproach\">
Polynomial approaches</a></li>
<li>
<a href=\"modelica://AixLib.Fluid.Actuators.Valves.ExpansionValves.UsersGuide.Approaches.PowerApproach\">
Power approaches</a></li>
</ol>
...
<p>
All expansion valve models have a parameter <code>useInpFil</code> that 
is used to model the valve's transient behaviour while opening or closing. 
