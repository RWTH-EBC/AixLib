...
partial model PartialFlowValve
  ...
  // outflow function
   Modelica.SIunits.SpecificHeatCapacityAtConstantPressure cp = Medium.specificHeatCapacityCp(staInl);

   Modelica.SIunits.SpecificHeatCapacityAtConstantVolume cv = Medium.specificHeatCapacityCv(staInl);
   Real kappa;
  //Real Y_undercritical=sqrt((kappa/(kappa - 1))*x^(1/kappa)*(x^(1/kappa) - x))
    Real Y_undercritical=sqrt((kappa/(kappa - 1)))*sqrt(x^(2/kappa)-(x)^((kappa+1)/kappa))
    "discharge-Function für undercritical area";
  ...
  Real x_krit= ((kappa+1)/2)^(kappa/(1-kappa)) "Critical Pressure ratio";
...
equation
  ...
  Y = if x>= x_krit then Y_undercritical else if x <=x_krit then Y_critical
     else 0;
