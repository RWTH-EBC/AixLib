...
<p>
  Ideal actuator motor model with hysteresis and finite actuation speed. If the current actuator position <code>y</code> is below (or above) the input signal <code>u</code> by an amount bigger than the hysteresis <code>delta</code>, then the position <code>y</code> is increased (decreased) until it reaches <code>u</code>. The output <code>y</code> is bounded between <code>0</code> and <code>1</code>.
</p>
<p>
  <b>Note:</b> This model can introduce state events which increase the computation time. For a more efficient implementation that approximates a motor, set in the valve or damper model the parameter <code>filteredOpening=true</code> instead of using this motor model. See also <a href=\"modelica://AixLib.Fluid.Actuators.UsersGuide\">AixLib.Fluid.Actuators.UsersGuide</a>.
</p></html>",revisions="<html>
<ul>
  <li>September 8, 2008 by Michael Wetter:<br/>
    Added to instance <code>int</code> the attribute <code>y(stateSelect=StateSelect.always)</code>. Without this attribute, the model <a href=\"modelica://AixLib.Fluid.Examples.TwoWayValves\">AixLib.Fluid.Examples.TwoWayValves</a> sets <code>y=3</code> which is consistent with this model.
  </li>
  <li>September 8, 2008 by Michael Wetter:<br/>
