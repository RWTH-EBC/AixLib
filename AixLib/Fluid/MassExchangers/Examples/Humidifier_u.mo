...
model Humidifier_u "Model that demonstrates the ideal humidifier model"
  ...
  AixLib.Fluid.MassExchangers.Humidifier_u humSte(
  ...
  AixLib.Fluid.MassExchangers.Humidifier_u humDyn(
...
equation
  ...
  annotation (
    ...
    __Dymola_Commands(file=
          ...
          "modelica://AixLib/Resources/Scripts/Dymola/Fluid/MassExchangers/Examples/Humidifier_u.mos"

        "Simulate and plot"),

    Documentation(info="<html>
<p>
  Model that demonstrates the use of an ideal humidifier. Both humidifer models are identical, except that one model is configured as a steady-state model, whereas the other is configured as a dynamic model. Both humidifiers add water to the medium to track a set-point for the outlet temperature using adiabatic cooling. The temperature of the water that is added to the medium is determined by the parameter <code>T</code> of the humidifier models.
</p></html>",revisions="<html>
<ul>
  <li>April 23, 2013, by Michael Wetter:<br/>
