...
<p>
The geometry is specified in the interface with the <code>surfaceAreas[n]</code> and the <code>lengths[n]</code> along the flow path.
Moreover the fluid flow is characterized for different types of devices by the average velocities <code>vs[n+1]</code> of fluid flow.
</p>
</html>", revisions="<html>
<ul>
