...
the <code>surfaceAreas[n]</code> and the boundary mass fractions <code>flowPorts[n].Xi</code>.
</p>
An extending model implementing this interface needs to define one equation: the relation between the predefined fluid mass fractions <code>Xis[n]</code>,
the boundary mass fractions <code>flowPorts[n].Xi</code>, and the mass flow rates <code>m_flows[n]</code>.
</p>
</html>", revisions="<html>
<ul>
<li>
