...
partial model Carnot
  ...
  parameter AixLib.Fluid.Types.EfficiencyInput effInpEva
  ...
  parameter AixLib.Fluid.Types.EfficiencyInput effInpCon
  ...
  Real etaPL(final unit = "1")=
    ...
    else AixLib.Utilities.Math.Functions.polynomial(a=a, x=yPL)
  ...
  Real COPCar(min=0) = TUse / AixLib.Utilities.Math.Functions.smoothMax(
  ...
  Modelica.SIunits.Temperature TCon(start=TCon_nominal)=
    ...
    if effInpCon == AixLib.Fluid.Types.EfficiencyInput.port_a then

             Medium1.temperature(staA1)

           elseif effInpCon == AixLib.Fluid.Types.EfficiencyInput.port_b or

                  effInpCon == AixLib.Fluid.Types.EfficiencyInput.volume then
  ...
  Modelica.SIunits.Temperature TEva(start=TEva_nominal)=
    ...
    if effInpEva == AixLib.Fluid.Types.EfficiencyInput.port_a then

             Medium2.temperature(staA2)

           elseif effInpEva == AixLib.Fluid.Types.EfficiencyInput.port_b or

                  effInpEva == AixLib.Fluid.Types.EfficiencyInput.volume then
...
<p align=\"center\" style=\"font-style:italic;\">
  η<sub>Carnot,0</sub> = COP<sub>0</sub> ⁄ (T<sub>use,0</sub> ⁄ (T<sub>con,0</sub>-T<sub>eva,0</sub>)),
</p>
<p>
  where <i>T<sub>use</sub></i> is the temperature of the the useful heat, e.g., the evaporator temperature for a chiller or the condenser temperature for a heat pump.
</p>
<p>
...
<p>
  where <i>COP<sub>Carnot</sub></i> is the Carnot efficiency and <i>η<sub>PL</sub></i> is the part load efficiency, expressed using a polynomial. This polynomial has the form
</p>
<p align=\"center\" style=\"font-style:italic;\">
  η<sub>PL</sub> = a<sub>1</sub> + a<sub>2</sub> y + a<sub>3</sub> y<sup>2</sup> + ...
</p>
<p>
  where <i>y ∈ [0, 1]</i> is either the part load for cooling in case of a chiller, or the part load of heating in case of a heat pump, and the coefficients <i>a<sub>i</sub></i> are declared by the parameter <code>a</code>.
</p>
<h4>
  Implementation
</h4>
<p>
  To make this base class applicable to chiller or heat pumps, it uses the boolean constant <code>COP_is_for_cooling</code>. Depending on its value, the equations for the coefficient of performance and the part load ratio are set up.
