...
initial algorithm
  dMonotone := AixLib.Utilities.Math.Functions.splineDerivatives(
    x=incAngDat,
    y=incAngModDat,
...
equation
  ...
  HSkyDifTil))*
      smooth(1, if TFlu[i] < TMedMax2
        then 1
        else AixLib.Utilities.Math.Functions.smoothHeaviside(TMedMax-TFlu[i], dTMax));
  end for;
  annotation (
...
AixLib.Utilities.Math.Functions.smoothHeaviside</a> function.
</p>
