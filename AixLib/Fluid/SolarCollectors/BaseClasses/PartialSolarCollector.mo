within AixLib.Fluid.SolarCollectors.BaseClasses;
partial model PartialSolarCollector "Partial model for solar collectors"
 extends AixLib.Fluid.Interfaces.LumpedVolumeDeclarations;
  ...
  parameter Modelica.Units.SI.HeatCapacity CTot=
    if per.CTyp==AixLib.Fluid.SolarCollectors.Types.HeatCapacity.TotalCapacity then
      per.C
    elseif per.CTyp==AixLib.Fluid.SolarCollectors.Types.HeatCapacity.DryCapacity then
  ...
  parameter AixLib.Fluid.SolarCollectors.Types.NumberSelection nColType=
  AixLib.Fluid.SolarCollectors.Types.NumberSelection.Number
  ...
  parameter Modelica.Units.SI.Area totalArea=0
    "Total area of panels in the simulation" annotation (Dialog(group=
          "Area declarations", enable=(nColType == AixLib.Fluid.SolarCollectors.Types.NumberSelection.Area)));

  parameter AixLib.Fluid.SolarCollectors.Types.SystemConfiguration sysConfig=
  AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Series
  ...
  parameter Integer nPanelsSer=0 "Number of array panels in series"
    annotation(Dialog(group="Configuration declarations", enable= (sysConfig == AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Array)));
  ...
  AixLib.BoundaryConditions.SolarIrradiation.DiffusePerez HDifTilIso(
  ...
  replaceable parameter AixLib.Fluid.SolarCollectors.Data.GenericASHRAE93 per
    constrainedby AixLib.Fluid.SolarCollectors.Data.BaseClasses.Generic
    "Performance data"
...
protected
  ...
  parameter Real nPanels_internal=
    if nColType == AixLib.Fluid.SolarCollectors.Types.NumberSelection.Number then
      nPanels
    else
      totalArea/per.A "Number of panels used in the simulation";
  parameter Real nPanelsSer_internal=
    if sysConfig == AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Series then
      nPanels
    else if sysConfig == AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel then
      1
    else
      nPanelsSer "Number of panels in series";
  parameter Real nPanelsPar_internal=
    if sysConfig == AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel then
      nPanels
    else if sysConfig == AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Series then
      1
    else
...
initial equation
  ...
  if sysConfig==AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Array then
...
e.g., if <code>sysConfig == AixLib.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel</code>.<br/>
