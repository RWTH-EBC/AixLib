...
partial model PartialEmpiricalFlow
  ...
  Modelica.Blocks.Interfaces.RealInput u(min=0) if useInputPort
...
equation
  connect(u, openingArea.u)
    annotation (Line(points={{0,120},{0,70},{18,70}}, color={0,0,127},
      pattern=DynamicSelect(LinePattern.Dash, if useInputPort then LinePattern.Solid else LinePattern.Dash)));
