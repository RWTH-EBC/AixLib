...
equation
  ...
  if abs(beta_deg) <= 90 then
    ...
    V_flow_w =if noEvent(u >= u_lim) then 0.0357*openingArea_1.A*(u - u_lim)
       else 0;
  else
    /*Leeward*/
    C_T = 1.355 - 0.179*u;
  ...
  assert(interimRes1 > Modelica.Constants.eps,
    ...
    AssertionLevel.warning);
  V_flow_th =if noEvent(interimRes1 > Modelica.Constants.eps) then 1/3*
    openingArea_1.A*C_d*sqrt(interimRes1) else 0;
