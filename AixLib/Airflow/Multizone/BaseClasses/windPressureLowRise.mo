...
<p>
  where <i>x</i> is the length of the wall that will be connected to this model, and <i>y</i> is the length of the adjacent wall as shown in the figure below.
</p>
<p align=\"center\">
  <img src=\"modelica://AixLib/Resources/Images/Airflow/Multizone/BaseClasses/windPressureLowRise.png\" border=\"1\" alt=\"Definition" of="" the="" aspect="">
</p>
<p>
  Based on the wind incidence angle <i>α</i> and the side ratio of the walls, the model computes how much the wind pressure is attenuated compared to the reference wind pressure <code>Cp0</code>. The reference wind pressure <code>Cp0</code> is a user-defined parameter, and must be equal to the wind pressure at zero wind incidence angle, i.e., <i>α = 0</i>. Swami and Chandra (1987) recommend <i>C<sub>p0</sub> = 0.6</i> for all low-rise buildings as this represents the average of various values reported in the literature. The attenuation factor is
</p>
<p align=\"center\" style=\"font-style:italic;\">
  C<sub>p</sub> ⁄ C<sub>p0</sub> = ln(1.248 - 0.703 sin(α ⁄ 2) - 1.175 sin<sup>2</sup>(α) - 0.131 sin<sup>3</sup>(2 α G) + 0.769 cos(α ⁄ 2) +0.071 G<sup>2</sup> * sin<sup>2</sup>(α ⁄ 2) + 0.717 cos<sup>2</sup>(α ⁄ 2)),
</p>
<p>
  where <i>C<sub>p</sub></i> is the wind pressure coefficient for the current angle of incidence.
</p>
<p>
  This function is used in <a href=\"modelica://AixLib.Fluid.Sources.Outside_CpLowRise\">Buildings.Fluid.Sources.Outside_CpLowRise</a> which can be used directly with components of this package.
</p>
<h4>
  References
</h4>
...
<ul>
  ...
  <li>January 26, 2016, by Michael Wetter:<br/>
    Removed <code>constant</code> keyword for <code>a180</code> as its value depends on the input of the function.
  </li>
  <li>October 27, 2011 by Michael Wetter:<br/>
