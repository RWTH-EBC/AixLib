...
model Example
  ...
  Modelica.Blocks.Sources.Sine outdoorTemperature(amplitude=5, freqHz=1/86400,
    offset=273.15 + 10)
...
equation
  ...
  connect(roomSetTemperature.y, FVUController.roomSetTemperature) annotation (
      Line(points={{-73,-18},{-46,-18},{-46,-16}},   color={0,0,127}));
  ...
  connect(FVU.ExhaustAir,room_out. ports[1]) annotation (Line(
      ...
      color={0,127,255},
      smooth=Smooth.None));
  connect(heating_sink.ports[1],FVU. Heater_Return) annotation (Line(points={{26,20},
          {26,-6},{95.2,-6},{95.2,-36}},        color={0,127,255}));
  connect(cooling_source.ports[1],FVU. Cooler_Flow) annotation (Line(points={{126,21},
          {126,-26},{105.2,-26},{105.2,-36}},    color={0,127,255}));
  connect(roomTemperature1.y, heating_source.T_in)
    annotation (Line(points={{45,84},{58,84},{58,41.8}},    color={0,0,127}));
  connect(roomTemperature2.y, cooling_source.T_in) annotation (Line(points={{105,84},
          {105,84},{130,84},{130,43}},        color={0,0,127}));
  connect(roomTemperature.y, room_out.T_in) annotation (Line(points={{-73,58},{-73,
          58},{198,58},{198,-70},{180,-70}},            color={0,0,127}));
  connect(FVU.Cooler_Return, cooling_sink.ports[1])
    annotation (Line(points={{102.2,-36},{102,-36},{102,22}},
                                                         color={0,127,255}));
  connect(heating_source.ports[1], FVU.Heater_Flow) annotation (Line(points={{54,22},
          {54,22},{54,10},{98.2,10},{98.2,-36}},       color={0,127,255}));
  connect(ambient_in.ports[1], FVU.OutgoingExhaustAir) annotation (Line(points={{24,-43},
          {42,-43},{42,-43.4},{70,-43.4}},         color={0,127,255}));
  ...
  connect(FVUController.coolingValveOpening, FVU.coolingValveOpening)
    annotation (Line(points={{-6,8},{44,8},{107.2,8},{107.2,-36}}, color={0,0,127}));
  connect(FVUController.heatingValveOpening, FVU.heatingValveOpening)
    annotation (Line(points={{-6,2},{40,2},{100.2,2},{100.2,-36}}, color={0,0,127}));
