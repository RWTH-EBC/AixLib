within AixLib.Electrical.AC.OnePhase.Sensors;
  ...
  Modelica.Blocks.Interfaces.RealOutput theta(unit="rad", displayUnit="deg")
    "Voltage phasor angle"
        ...
        transformation(extent={{60,-40},{80,-20}}), iconTransformation(extent={{60,
            -40},{80,-20}})));
equation
  theta = AixLib.Electrical.PhaseSystems.OnePhase.phase(term.v);
  if perUnit then
    V = AixLib.Electrical.PhaseSystems.OnePhase.systemVoltage(term.v)/V_nominal;
  else
    V = AixLib.Electrical.PhaseSystems.OnePhase.systemVoltage(term.v);
  end if;
  term.i = zeros(AixLib.Electrical.PhaseSystems.OnePhase.n);
  annotation (
