within AixLib.Electrical.AC.OnePhase.Lines;
model TwoPortRLC "Model of an RLC element with two electrical ports"
  ...
  parameter Modelica.Units.SI.Voltage Vc_start[2]={V_nominal,0}
    ...
    annotation (Dialog(enable=(mode == AixLib.Electrical.Types.Load.FixedZ_dynamic)));
...
initial equation
  if C > 0 and mode == AixLib.Electrical.Types.Load.FixedZ_dynamic then
    Vc = Vc_start;
  end if;
equation
  ...
  L/2*omega*AixLib.Electrical.PhaseSystems.OnePhase.j(terminal_p.i) +
    terminal_p.i*diagonal(ones(PhaseSystem_p.n)*R_actual/2) = terminal_p.v - Vc;
  L/2*omega*AixLib.Electrical.PhaseSystems.OnePhase.j(terminal_n.i) +
    terminal_n.i*diagonal(ones(PhaseSystem_n.n)*R_actual/2) = terminal_n.v - Vc;

  if C > 0 then
    if mode == AixLib.Electrical.Types.Load.FixedZ_dynamic then
      // Dynamics of the system
      C*der(Vc) + omega*C*AixLib.Electrical.PhaseSystems.OnePhase.j(Vc) = Ic;
    else
      // steady state relationship
      omega*C*AixLib.Electrical.PhaseSystems.OnePhase.j(Vc) = Ic;
    end if;
  else
...
<p align=\"center\">
<img alt=\"image\" src=\"modelica://AixLib/Resources/Images/Electrical/AC/OnePhase/Lines/twoPortRLC.png\"/>
</p>
<p>
