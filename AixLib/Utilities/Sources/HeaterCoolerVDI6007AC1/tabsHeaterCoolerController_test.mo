...
Modelica.Blocks.Sources.Constant TAirThresholdCoolingTabs(k=if not recOrSep then TThreshold_Cool_Tabs else zoneParam.TThresholdCoolerTabs)
  ...
  Modelica.Blocks.Interfaces.RealInput TOpe "Operative indoor air temperature"
    annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={2,108})));
  Math.MovingAverage movingAverageTOpe(aveTime=4*3600)
  ...
  tabsHeatingCurve tabs_HeatingCurve(
    ...
    power_low=if not recOrSep then power_Heater_Tabs*0.2 else zoneParam.powerHeatTabs
        *0.2,
  ...
  tabsHeatingCurve tabsCoolingCurve(
    ...
    power_low=if not recOrSep then power_Cooler_Tabs*0.2 else zoneParam.powerCoolTabs
        *0.2,
    T_upperlimit=273.15 + 25,
    T_lowerlimit=273.15 + 20,
...
equation
  ...
  connect(TOpe, movingAverageTOpe.u) annotation (Line(points={{2,108},{1.5,108},
            {1.5,87.2},{2,87.2}},     color={0,0,127}));
  ...
  connect(movingAverageTOpe.y, tabs_HeatingCurve.tDryBul)
    annotation (Line(points={{2,73.4},{-12,73.4},{-12,42}}, color={0,0,127}));
  connect(movingAverageTOpe.y, tabsCoolingCurve.tDryBul) annotation (Line(
        points={{2,73.4},{-20,73.4},{-20,-46},{-18,-46}}, color={0,0,127}));
