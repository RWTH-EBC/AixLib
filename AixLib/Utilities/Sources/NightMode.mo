...
model NightMode "Module to establish if an element should opperate in night mode or not."
  ...
  Modelica.Blocks.Interfaces.BooleanOutput SwitchToNightMode "Connector of Boolean output signal" annotation(Placement(transformation(extent = {{78.5, -7}, {98.5, 13}}), iconTransformation(extent = {{78.5, -7}, {98.5, 13}})));
equation
  connect(hourOfDay.HOD, lessEqualThreshold.u) annotation(Line(points = {{-64, 10}, {-64, 1.5}, {-21, 1.5}, {-21, -12}, {-13.5, -12}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(hourOfDay.HOD, greaterThreshold.u) annotation(Line(points = {{-64, 10}, {-64, 1.5}, {-21, 1.5}, {-21, 15}, {-12.95, 15}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(lessEqualThreshold.y, IsNight.u2) annotation(Line(points = {{3.75, -12}, {9, -12}, {9, -3}, {16.5, -3}}, color = {255, 0, 255}, smooth = Smooth.None));
