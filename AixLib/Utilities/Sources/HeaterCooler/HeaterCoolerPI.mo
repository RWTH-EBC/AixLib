...
model HeaterCoolerPI "heater and cooler with variable setpoints"
  ...
  parameter Boolean staOrDyn = true "Static or dynamic activation of heater" annotation(choices(choice = true "Static", choice =  false "Dynamic",
                  radioButtons = true));
  Modelica.Blocks.Interfaces.RealInput setPointCool if
                                                    Cooler_on annotation (
      Placement(transformation(extent={{-120,-60},{-80,-20}}),
        iconTransformation(
        extent={{-20,-20},{20,20}},
        rotation=90,
        origin={-48,-48})));
  Modelica.Blocks.Interfaces.RealInput setPointHeat if
                                                    Heater_on annotation (
  ...
  Modelica.Blocks.Sources.BooleanExpression booleanExpressionHeater(y=if not
        recOrSep then Heater_on else zoneParam.Heater_on) if staOrDyn
  ...
  Modelica.Blocks.Sources.BooleanExpression booleanExpressionCooler(y=if not
        recOrSep then Cooler_on else zoneParam.Cooler_on) if staOrDyn
...
equation
  if staOrDyn then
    ...
    connect(heaterActive, pITempHeat.onOff);
    connect(coolerActive, pITempCool.onOff);

  end if;
  connect(setPointHeat, pITempHeat.soll)
