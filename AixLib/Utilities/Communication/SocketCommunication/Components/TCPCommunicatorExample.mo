...
<p>
  This is a small example Block which allows to establish a TCP Connection between a Client (i.e. Dymola) and a Server (External) it sends the value of input <code>u[1]</code> as a string to the server and receives a string message. This received string message should only contain a real number as it is converted into a <code>Real</code> value afterwards and forwarded to output <code>y[1]</code>. Check <code>SocketCommunication.Examples.ExampleClientLoop</code> for a executable example.
