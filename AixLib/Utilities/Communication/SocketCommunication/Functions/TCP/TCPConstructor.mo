...
<p>
  Intializes a TCP socket and connects to a server on a certain port. Comprises <code>SocketInit()</code> and <code>SocketConnect()</code>.
</p>
<h4>
  Usage of function
</h4>
...
<p>
  state == 0, everything fine, state == 1, error where an error message will be reported in the simulation messages window. Error codes and descriptions can be found in <code>UsersGuide</code>.
</p>
<h4>
  C Source Code of <code>TCPConstructor()</code>
</h4>
<p>
  Source code of <code>TCPConstructor()</code> in external header file.
</p>
<pre>
