...
<p>
  If the constant <code>approximateWetBulb</code> is <code>true</code>, then the block uses the approximation of Stull (2011) to compute the wet bulb temperature without requiring a nonlinear equation. Otherwise, the model will introduce one nonlinear equation. The approximation by Stull is valid for a relative humidity of <i>5%</i> to <i>99%</i>, a temperature range from <i>-20</i>°C to <i>50</i>°C and standard sea level pressure. For this range of data, the approximation error is <i>-1</i> Kelvin to <i>+0.65</i> Kelvin, with a mean error of less than <i>0.3</i> Kelvin.
</p>
<p>
  Otherwise a calculation based on an energy balance is used. See <a href=\"https://github.com/iea-annex60/modelica-annex60/issues/474\">#474</a> for a discussion. The model is validated in <a href=\"modelica://AixLib.Utilities.Psychrometrics.Examples.TWetBul_TDryBulPhi\">AixLib.Utilities.Psychrometrics.Examples.TWetBul_TDryBulPhi</a>.
</p>
<p>
  For a model that takes the mass fraction instead of the relative humidity as an input, see <a href=\"modelica://AixLib.Utilities.Psychrometrics.TWetBul_TDryBulXi\">AixLib.Utilities.Psychrometrics.TWetBul_TDryBulXi</a>.
</p>
<h4>
  References
</h4>
