...
model HeatConv_inside "Natural convection computation according to B. Glueck, choice between several types of surface orientation, or constant factor"
  ...
  */
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alpha_custom "Constant heat transfer coefficient" annotation(Dialog(descriptionLabel = true, enable = IsAlphaConstant));
  ...
  Modelica.SIunits.CoefficientOfHeatTransfer alpha "variable heat transfer coefficient";
protected
  Modelica.SIunits.Temp_C posDiff = noEvent(abs(port_b.T - port_a.T)) "Positive temperature difference";
equation
  /*
        port_b -> wall
        port_a -> air
