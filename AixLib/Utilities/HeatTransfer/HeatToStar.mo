...
equation
  ...
  // Negative solutions also occurs when using max(T,0), therefore, 100 K is used.
  Therm.Q_flow = Modelica.Constants.sigma * eps * A * (
    max(Therm.T,100) * max(Therm.T,100) * max(Therm.T,100) * max(Therm.T,100) - 
    max(Star.T,100) * max(Star.T,100) * max(Star.T,100) * max(Star.T,100));
