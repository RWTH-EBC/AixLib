...
<p>In this model the surface orientation can be chosen from a menu for an easier adoption to new situations. Following methods to calculate the <b>heat convection coefficient</b> <span style=\"font-family: Courier New;\">hCon</span> can be chosen: </p>
<ol>
<li>EN ISO 6946: <span style=\"font-family: Courier New;\">hCon</span> depends on the direction of heat transfer (horizontal: <span style=\"font-family: Courier New;\">hCon</span>= 2.5 m^2 K/W, upwards: <span style=\"font-family: Courier New;\">hCon</span>= 5 m^2 K/W, downwards: <span style=\"font-family: Courier New;\">hCon</span>=0.7 m^2 K/W, EN ISO 6946 table C.1). Switching the heat convection coefficient due to a chance of direction of heat transfer would lead to a state event. This would force the solver to solve a totally changed equation system and extend the calculation time. Therefore the <span style=\"font-family: Courier New;\">regStep</span> function is used to get a continous and differenciable expression. If the temperature difference between <span style=\"font-family: Courier New;\">port_b</span> and <span style=\"font-family: Courier New;\">port_a</span> is between -<span style=\"font-family: Courier New;\">dT_small</span> and <span style=\"font-family: Courier New;\">dT_small</span> a 2nd order polynomial is used for a smooth transition from 5 to 0.7 (facing up) or from 0.7 to 5 (facing down). </li>
<li>B. Glueck (default): The following equations are used to calculate the heat convection coefficient depending on the direction of heat transfer (p. 26):<br>horizontal: <span style=\"font-family: Courier New;\">hCon = 1.6 * |port_b.T - port_a.T|^0.3</span><br>upwards: <span style=\"font-family: Courier New;\">hCon = 2 * |port_b.T - port_a.T|^0.31</span><br>downwards: <span style=\"font-family: Courier New;\">hCon = 0.54 * |port_b.T - port_a.T|^0.31</span><br>The smooth function is used in case of changing direction of heat transfer. </li>
<li>Constant heat convection coefficient: There is also the possibility of setting a constant <span style=\"font-family: Courier New;\">hCon</span> value (<span style=\"font-family: Courier New;\">hCon_const</span>). </li>
</ol>
<p><b><span style=\"color: #008000;\">Limitations</span> </b></p>
<p>... of the approaches calculating <span style=\"font-family: Courier New;\">hCon</span>: </p>
<ul>
...
<ul>
<li><i>May 6, 2021</i> by Felix Stegemerten / Larissa K&uuml;hn<br>Bugfixing Method &quot;Glueck&quot;</li>
<li><i>May 30, 2019</i> by Katharina Brinkmann / Philipp Mehrfeld:<br><a href=\"https://github.com/RWTH-EBC/AixLib/issues/711\">#711</a>:<br>- add smooth + noEvent functions<br>- ISO approach now linearized when heat flow reverses (depending on newly introduced <span style=\"font-family: Courier New;\">dT_small</span>)<br>- Approach acc. to Glueck can change heat flow during simulation.<br>- Standard Calculation Method now &quot;Glueck&quot; due to faster simulation speed </li>
<li><i>October 12, 2016&nbsp;</i> by Tobias Blacha:<br>Algorithm for HeatConv_inside is now selectable via parameters </li>
<li><i>June 17, 2015&nbsp;</i> by Philipp Mehrfeld:<br>Added EN ISO 6946 equations and corrected usage of constant hCon_const </li>
