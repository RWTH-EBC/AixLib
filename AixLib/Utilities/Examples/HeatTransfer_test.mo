...
model HeatTransfer_test "Test routine for heat transfer models"
  ...
  HeatTransfer.HeatConv_inside heatConv_universal(A = 16, alpha_custom = 2, surfaceOrientation = 2) annotation(Placement(transformation(extent = {{-10, -18}, {10, 2}})));
  ...
  Modelica.Blocks.Sources.Constant constTempInside(k = 298) annotation(Placement(transformation(extent = {{7, -7}, {-7, 7}}, rotation = 0, origin = {93, -15})));
equation
  ...
  connect(TempOutside.port, heatCond.port_a) annotation(Line(points = {{-60, 10}, {-40, 10}, {-40, 12}, {-10, 12}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(TempOutside.port, heatConv_universal.port_a) annotation(Line(points = {{-60, 10}, {-40, 10}, {-40, -8}, {-10, -8}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(heatConv.port_b, TempInside.port) annotation(Line(points = {{10, 48}, {46, 48}, {46, 10}, {60, 10}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(heatCond.port_b, TempInside.port) annotation(Line(points = {{10, 12}, {46, 12}, {46, 10}, {60, 10}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(heatConv_universal.port_b, TempInside.port) annotation(Line(points = {{10, -8}, {46, -8}, {46, 10}, {60, 10}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(sineWindSpeed.y, heatTransfer_Outside.WindSpeedPort) annotation(Line(points = {{-23.5, -19}, {-18, -19}, {-18, -35.2}, {-9.2, -35.2}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(TempOutside.port, heatConv.port_a) annotation(Line(points = {{-60, 10}, {-40, 10}, {-40, 48}, {-10, 48}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(constTempOutside.y, TempOutside.T) annotation(Line(points = {{-85.3, -13}, {-82, -13}, {-82, 10}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(constTempInside.y, TempInside.T) annotation(Line(points = {{85.3, -15}, {84, -15}, {84, -16}, {84, -16}, {84, 10}, {82, 10}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(TempOutside.port, heatTrans.port_a) annotation(Line(points = {{-60, 10}, {-40, 10}, {-40, -46}, {-10, -46}}, color = {191, 0, 0}, smooth = Smooth.None));
