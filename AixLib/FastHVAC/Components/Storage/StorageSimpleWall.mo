...
//                             annotation(Dialog(enable = use_heatingCoil2,tab="Heating Coils and Rod"));
 ...
 final parameter Integer dis_HC2 = n_HC2_up-n_HC2_low+1;
  ...
  BaseClasses.HeatingCoil heatingCoil1(
    ...
    pipeRecordHC=data.pipeHC1) if use_heatingCoil1 annotation (Placement(transformation(
        extent={{-15,-12},{15,12}},
        rotation=270,
        origin={-72,59})));
  BaseClasses.HeatingCoil heatingCoil2(
    ...
    calculateHConvIn=calculateAlphaInside,
...
protected
  parameter Real k_zyl(final unit="W/K") = 2*Modelica.Constants.pi*data.hTank/n/(1/(hConvIn*data.dTank/2) + 1/data.lambdaIns*log((data.dTank/2 + data.sIns)/(data.dTank/2)) + 1/(hConvOut*(data.dTank/2 + data.sIns)));
