...
public
 ...
 annotation (Dialog( enable = heatLossToAmbient));
         parameter Real RadiationWarning = 1;
  ...
  AixLib.Utilities.Interfaces.Star star if withRadiation
  ...
  AixLib.Utilities.HeatTransfer.HeatConv heatConv(alpha=alpha, A=Modelica.Constants.pi
        *outerDiameter*length) if withConvection
  ...
  AixLib.Utilities.HeatTransfer.HeatToStar twoStar_RadEx(eps=eps, A=Modelica.Constants.pi
        *outerDiameter*length) if withRadiation
...
equation
  assert(RadiationWarning>2,  "radiation effects won't work without convection", level = AssertionLevel.warning);
   //Connect the heat ports from the pipe to the pipe wall
      for i in 1:nNodes loop
        connect(pipeBase.heatPorts[i],pipeWall.port_a);
          ...
          //nothing
         if (not withRadiation and not withInsulation and not withConvection) then
           connect(pipeWall.port_b,heatPort_outside);

         end if;
          //only radiation (doesn't work)
          if (withRadiation and not withInsulation and not withConvection) then
            connect(pipeWall.port_b, twoStar_RadEx.Therm);
            connect(pipeWall.port_b,heatPort_outside);

          end if;
          // only insulation
         if (withInsulation and not withRadiation and not withConvection) then
           connect(pipeWall.port_b,insulation.port_a);
           connect(insulation.port_b,heatPort_outside);

         end if;
         // only convection
         if (withConvection and not withRadiation and not withInsulation) then
           connect(pipeWall.port_b,heatConv.port_a);
           connect(heatConv.port_b, heatPort_outside);

         end if;
         //convection and radiation
         if (withConvection and withRadiation and not withInsulation) then
             connect(pipeWall.port_b,heatConv.port_a);
             connect(heatConv.port_b, heatPort_outside);
             connect(pipeWall.port_b, twoStar_RadEx.Therm);
         ...
         if (withConvection and withInsulation and not withRadiation) then
             connect(pipeWall.port_b,insulation.port_a);
         ...
         // radiation and insulation (doesn't work)
          if (withRadiation and withInsulation and not withConvection) then
              connect(pipeWall.port_b,insulation.port_a);
         ...
         if (withRadiation and withInsulation and withConvection) then
             connect(pipeWall.port_b,insulation.port_a);
             connect(insulation.port_b,  heatConv.port_a);
             connect(heatConv.port_b, heatPort_outside);
             connect(insulation.port_b, twoStar_RadEx.Therm);
...
//
//          elseif (withRadiation and withInsulation) then
//            connect(pipeWall.port_b,insulation.port_a);
//            connect(insulation.port_b, twoStar_RadEx.Therm);
//          end if;
//          //convection
//          if not withInsulation and not withConvection then
//            connect(pipeWall.port_b,heatPort_outside);
//          elseif withInsulation and withConvection then
//            connect(pipeWall.port_b,insulation.port_a);
//            connect(insulation.port_b,  heatConv.port_a);
//            connect(heatConv.port_b, heatPort_outside);
