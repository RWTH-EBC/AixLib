...
final parameter          Boolean withRadiationParam=if not withConvection then false else withRadiation
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer hConvOutside=8
    "Heat transfer coefficient to ambient"                      annotation (Dialog( enable=
          withConvection));
 ...
 annotation (Dialog( enable = withRadiation));
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer hConvInsideFix=30
    "Fix value for heat transfer coefficient inside pipe"                                                                          annotation(Dialog(enable=not
          calculateHConv));
  ...
  FastHVAC.Components.Pipes.BaseClasses.PipeBase pipeBase(
    ...
    nNodes=nNodes,
    length=length,
    hConvInFix=hConvInsideFix,
    calculateHConvIn=calculateHConv) annotation (Placement(transformation(extent={{-20,-78},{20,-38}})));
protected
