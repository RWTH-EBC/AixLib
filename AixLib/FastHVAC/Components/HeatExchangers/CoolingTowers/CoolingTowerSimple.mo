...
model CoolingTowerSimple
  ...
  parameter Modelica.SIunits.TemperatureDifference TApproach=2
    "Approach Temperature, Difference between water outflow and air inflow temperatures in K";

  parameter Modelica.SIunits.Temperature TAdiabaticSwitch=297.15
    "Adiabatic Switch Temperature, Air Temperatures over that";

  parameter Modelica.SIunits.MassFlowRate m_flow_water_min=3
    "Minimum water consumption in adiabatic mode";

  parameter Modelica.SIunits.MassFlowRate m_flow_water_max=15
    "Maximum water consumption in adiabatic mode";

  parameter Modelica.SIunits.Temperature TAdiabaticMax=308.15
    "Maximum air temperature where maximum water flow rate occures";
...
equation
 ...
 m_flow_water_value = 0;
  ...
  connect(
       fluidSource.enthalpyPort_b, enthalpyPort_b)
 annotation (Line(points={{66,1},{80,1},{80,0},{100,0}}, color={176,0,0}));
  connect(
       realExpression1.y, fluidSource.dotm) annotation (Line(points={{25,-8},
       {36,-8},{36,-2.6},{48,-2.6}}, color={0,0,127}));
  connect(
       add.y, fluidSource.T_fluid)
 annotation (Line(points={{32,13.4},{32,4.2},{48,4.2}}, color={0,0,127}));
  connect(
       ApproachTemperatureSource.y, add.u1)
 annotation (Line(points={{59,50},{35.6,50},{35.6,27.2}}, color={0,0,127}));
  connect(
       adiabaticSwitch.y, add.u2)
 annotation (Line(points={{1,60},{28.4,60},{28.4,27.2}}, color={0,0,127}));
  connect(
       TAirDry, adiabaticSwitch.u1) annotation (Line(points={{-118,80},{-40,80},
       {-40,68},{-22,68}}, color={0,0,127}));
  connect(
       TAirWetBulb, adiabaticSwitch.u3) annotation (Line(points={{-118,40},{-40,
       40},{-40,52},{-22,52}}, color={0,0,127}));
  connect(
       less.y, adiabaticSwitch.u2)
 annotation (Line(points={{-41,60},{-22,60}}, color={255,0,255}));
  connect(
       TAirDry, less.u1) annotation (Line(points={{-118,80},{-80,80},{-80,60},
       {-64,60}}, color={0,0,127}));
  connect(
       AdiabaticSwitchTemperature.y, less.u2)
 ...
 annotation (Line(points={{110,-40},{110,-40}}, color={0,0,127}));
  connect(
       enthalpyPort_a, massFlowRate.enthalpyPort_a) annotation (Line(points={
       {-100,0},{-92,0},{-92,-0.1},{-78.8,-0.1}}, color={176,0,0}));
  connect(
       massFlowRate.enthalpyPort_b, vessel.enthalpyPort_a) annotation (Line(
     points={{-61,-0.1},{-54.5,-0.1},{-54.5,0},{-47,0}}, color={176,0,0}));
  connect(
       realExpression.y, m_flow_water)
