...
model ERC_Zone_simpleTabs "Model of a ERC-Thermal Zone Including CCA and AHU"
  ...
  BoundaryConditions.WeatherData.ReaderTMY3        weaDat(
    ...
    filNam=ModelicaServices.ExternalReferences.loadResource("modelica://AixLib/Resources/weatherdata/TRY2015_Jahr_City_Aachen.mos"))
    "Weather data reader"
  ...
  ModularAHU.GenericAHU genericAHU1(
    ...
    T_amb=293.15,
    m1_flow_nominal=12000/3600*1.2*3,
    m2_flow_nominal=0.5,
    T_start=293.15,
    usePreheater=false,
  ...
  BoundaryConditions.WeatherData.Bus        weaBus
    "Weather data bus"
    annotation (Placement(transformation(extent={{-34,20},{0,52}}),
    iconTransformation(extent={{-150,388},{-130,408}})));
  BaseClasses.ZoneBus zoneBus
  ...
  Modelica.Blocks.Interfaces.RealInput TsetAHU
    "Connector of second Real input signal" annotation (Placement(
        transformation(extent={{-114,46},{-88,72}}), iconTransformation(extent=
            {{-114,46},{-88,72}})));
...
equation
  ...
  connect(thermalZone1.TAir, zoneBus.TZoneMea) annotation (Line(points={{60.8,50.2},
          {78.4,50.2},{78.4,100.05},{0.05,100.05}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(genericAHU1.genericAHUBus, zoneBus.ahuBus) annotation (Line(
  ...
  connect(boundaryOutsideAir1.T_in, weaBus.TDryBul) annotation (Line(points={{-66.8,
          -21.6},{-94,-21.6},{-94,36},{-17,36}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  ...
  connect(x_pTphi1.T, weaBus.TDryBul) annotation (Line(points={{-78.8,-4},{-94,
          -4},{-94,36},{-17,36}},
                              color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(x_pTphi1.phi, weaBus.relHum) annotation (Line(points={{-78.8,-6.4},{
          -78.8,-6},{-94,-6},{-94,36},{-17,36}},
                                           color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-3,6},{-3,6}},
      horizontalAlignment=TextAlignment.Right));
  connect(x_pTphi1.p_in, weaBus.pAtm) annotation (Line(points={{-78.8,-1.6},{
          -78,-1.6},{-78,-2},{-94,-2},{-94,36},{-17,36}},
                                                      color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-3,6},{-3,6}},
  ...
  connect(bouWaterhot1.ports[1], genericAHU1.port_b5) annotation (Line(points={{
          ...
          127,255}));
  connect(bouWatercold1.ports[1], genericAHU1.port_a4) annotation (Line(points={{-18,-80},
          {-18,-32}},                    color={0,127,255}));
  connect(genericAHU1.port_b4, bouWatercold.ports[1]) annotation (Line(points={{
          -14.7273,-32},{-14,-32},{-14,-76},{46,-76},{46,-78},{8,-78}},  color={
          0,127,255}));
  connect(tabsSimple.heatPort, thermalZone1.intGainsConv) annotation (Line(
        points={{74,-43},{68,-43},{68,28.16},{58.56,28.16}}, color={191,0,0}));
  connect(tabsSimple.Q_flow, Q_flowTabs)
    annotation (Line(points={{64,-56},{-100,-56}}, color={0,0,127}));
  connect(ctrAhu.genericAHUBus, zoneBus.ahuBus) annotation (Line(
      points={{-36,58.1},{-18,58.1},{-18,100.05},{0.05,100.05}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(ctrAhu.Tset, TsetAHU) annotation (Line(points={{-58,58},{-80,58},{-80,
          59},{-101,59}}, color={0,0,127}));
  connect(thermalZone1.TAir, TAirRoom) annotation (Line(points={{60.8,50.2},{
          106,50.2},{106,50}}, color={0,0,127}));
  annotation (experiment(
      StopTime=3600,
      Interval=60,
      __Dymola_Algorithm="Dassl"));
end ERC_Zone_simpleTabs;
