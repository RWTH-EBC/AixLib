...
block CtrRegVSet "Controller for heating and cooling registers"
  //Boolean choice;
  ...
  parameter Modelica.SIunits.VolumeFlowRate VflowSet = 0.01 "Set point Volume Flow in m³/s" annotation (Dialog(enable=
  ...
  Modelica.Blocks.Sources.Constant constTflowSet(final k=VflowSet) if not
    useExternalVset                                                                       annotation (Placement(transformation(extent={{-100,-60},{-80,-40}})));
  ...
  BaseClasses.RegisterBus registerBus annotation (Placement(transformation(
          extent={{74,-26},{128,26}}), iconTransformation(extent={{88,-14},{116,
            14}})));
  Modelica.Blocks.Interfaces.RealInput VMea if useExternalVMea
...
equation
  ...
  connect(PID.u_m,VMea)  annotation (Line(
      points={{0,-62},{0,-120}},
      color={0,0,127},
      pattern=LinePattern.Dash));
  ...
  connect(PID.y, registerBus.hydraulicBus.valveSet) annotation (Line(points={{
      ...
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(PID.u_m, registerBus.hydraulicBus.VFlowOutMea) annotation (Line(
        points={{0,-62},{0,-80},{101.135,-80},{101.135,0.13}}, color={0,0,127}),
      Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
