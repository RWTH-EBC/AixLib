...
block CtrRegBasicVflow "Controller for heating and cooling registers"
  //Boolean choice;
  ...
  Modelica.Blocks.Sources.Constant constVflowSet(final k=VflowSet) if not useExternalVset annotation (Placement(transformation(extent={{-100,-60},{-80,-40}})));
...
equation
  ...
  if useExternalVMea==false then
      connect(PID.u_m, registerBus.hydraulicBus.VFlowOutMea) annotation (Line(
        points={{0,-62},{0,-86},{62,-86},{62,0.13},{101.135,0.13}}, color={0,0,127}),
      Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
