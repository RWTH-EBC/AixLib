...
model ManualControl
  ...
  ManualControl_ModularBoiler.deviceStatusDelay deviceStatusDelay1(time_minOff=time_minOff, time_minOn=
        time_minOn)
    annotation (Placement(transformation(extent={{-48,12},{-28,32}})));
  Modelica.Blocks.Logical.LogicalSwitch logicalSwitch
    annotation (Placement(transformation(extent={{-16,-10},{4,10}})));
  Modelica.Blocks.Sources.BooleanExpression booleanExpression(y=manualTimeDelay)
...
equation
  ...
  connect(isOn, logicalSwitch.u3) annotation (Line(points={{-100,0},{-44,0},{
          -44,-8},{-18,-8}},
                       color={255,0,255}));
  connect(deviceStatusDelay1.y, logicalSwitch.u1) annotation (Line(points={{-27,
  ...
  connect(booleanExpression.y, logicalSwitch1.u2) annotation (Line(points={{7,
          28},{16,28},{16,0},{24,0}}, color={255,0,255}));
  connect(PLR, greaterEqualThreshold.u) annotation (Line(points={{-102,76},{-84,
          76},{-84,50},{-81.6,50}}, color={0,0,127}));
  connect(greaterEqualThreshold.y, deviceStatusDelay1.u) annotation (Line(
  ...
  connect(logicalSwitch1.y, switch1.u2) annotation (Line(points={{47,0},{54,0},
          {54,50},{-6,50},{-6,68},{-2,68}}, color={255,0,255}));
  connect(booleanExpression.y, switch2.u2) annotation (Line(points={{7,28},{34,
          28},{34,68},{44,68}}, color={255,0,255}));
  connect(PLR, switch2.u3) annotation (Line(points={{-102,76},{-46,76},{-46,44},
          {40,44},{40,60},{44,60}}, color={0,0,127}));
  connect(switch1.y, switch2.u1) annotation (Line(points={{21,68},{32,68},{32,
