...
model Control_wPump_wFeedback
  ...
  Modelica.Blocks.Interfaces.RealOutput y annotation (Placement(transformation(
        ...
        rotation=270,
        origin={-40,-60})));
  Modelica.Blocks.Interfaces.RealOutput dTWater annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={0,-60})));
  ...
  Modelica.Blocks.Interfaces.RealOutput y_valve annotation (Placement(
        ...
        origin={-60,-60})));
protected
  Modelica.Blocks.Sources.RealExpression TSet_Cold(y=TColdNom)
    annotation (Placement(transformation(extent={{-10,-26},{10,-6}})));
equation

  connect(realExpression.y, switch3.u1) annotation (Line(points={{71.1,50},{58,50},
          {58,14}},         color={0,0,127}));
  connect(pLRMin.y, switch4.u2) annotation (Line(points={{-39.1,35},{-1.8,35}},
                           color={255,0,255}));
  connect(switch4.y, switch3.u3) annotation (Line(points={{18.9,35},{42,35},{42,
          14}},        color={0,0,127}));
  connect(realExpression.y, switch4.u1) annotation (Line(points={{71.1,50},{-22,
          50},{-22,42.2},{-1.8,42.2}},                               color={0,0,
          127}));

  connect(pLRMin.y, switch7.u2) annotation (Line(points={{-39.1,35},{-30,35},{-30,
          6}},        color={255,0,255}));
  connect(realExpression.y, switch7.u1) annotation (Line(points={{71.1,50},{30,50},
  ...
  connect(tHotMax.y, greater.u2) annotation (Line(points={{84.5,1.95},{84,1.95},
          {84,21.6},{81.6,21.6}},
                              color={0,0,127}));
  connect(TCold, controlBoilerNotManufacturer.TCold) annotation (Line(points={{-100,
          -20},{-82,-20},{-82,13},{-74,13}},    color={0,0,127}));
  connect(y, switch7.y)
    annotation (Line(points={{-40,-60},{-40,-40},{-30,-40},{-30,-17}},
                                                  color={0,0,127}));
  connect(controlBoilerNotManufacturer.DeltaTWater_b, dTWater) annotation (Line(
        points={{-51,4.8},{-48,4.8},{-48,-32},{0,-32},{0,-60}},       color={0,0,
          127}));
  connect(switch3.y, PLR) annotation (Line(points={{50,-9},{50,-40},{-20,-40},{-20,
          -60}}, color={0,0,127}));
  connect(THot_Boiler, controlBoilerNotManufacturer.THot) annotation (Line(
  ...
  connect(THot, greater.u1)
    annotation (Line(points={{-100,-60},{-90,-60},{-90,-80},{96,-80},{96,28},{81.6,
          28}},                                             color={0,0,127}));
  connect(PLR_in, pLRMin.u) annotation (Line(points={{-100,60},{-66,60},{-66,35},
          {-59.8,35}}, color={0,0,127}));
  connect(PLR_in, switch4.u3) annotation (Line(points={{-100,60},{-26,60},{-26,27.8},
          {-1.8,27.8}},                    color={0,0,127}));
  connect(dTwater_in, controlBoilerNotManufacturer.DeltaTWater_a) annotation (
      Line(points={{-100,40},{-80,40},{-80,7},{-74,7}},      color={0,0,127}));
  connect(TSet_Cold.y,PIValve. u_s)
    annotation (Line(points={{11,-16},{16.8,-16}},   color={0,0,127}));
  connect(TCold, PIValve.u_m) annotation (Line(points={{-100,-20},{-20,-20},{-20,
          -30},{24,-30},{24,-23.2}}, color={0,0,127}));
  connect(y_valve, PIValve.y) annotation (Line(points={{-60,-60},{-60,-38},{36,-38},
          {36,-16},{30.6,-16}}, color={0,0,127}));
