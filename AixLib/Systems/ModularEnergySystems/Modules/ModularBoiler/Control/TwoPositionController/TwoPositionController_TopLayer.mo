within AixLib.Systems.ModularEnergySystems.Modules.ModularBoiler.Control.TwoPositionController;
model TwoPositionController_TopLayer
  "Two position controller using top level of buffer storage for calculation"
  extends
    BaseClasses.partialTwoPositionController(
    n=1,
...
equation
  ...
  connect(TLayers[n], onOffController.u) annotation (Line(points={{-100,-22},{
          -34,-22},{-34,0},{30,0}},
                           color={0,0,127}));
...
<li>Tref: With this parameter, the user can select the set temperature</li>
        ...
        coordinateSystem(preserveAspectRatio=false)));
end TwoPositionController_TopLayer;
