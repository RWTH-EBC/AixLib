...
model ModularBoiler
  ...
  parameter Boolean use_advancedControl
   ...
   replaceable model TwoPositionController_top =
    ...
    Tref=Tref,
    bandwidth=bandwidth,
    n=n)                                                                                                           annotation(choicesAllMatching=true, Dialog(tab="Control", group="Two position control"));
  ...
  hierarchicalControl_modular hierarchicalControl_modularBoilerNEW1(
    ...
    TOffset=TOffset,
    TMax=TMax,
    redeclare model TwoPositionController_top = TwoPositionController_top,
...
equation
 ...
 if simpleTwoPosition then
  ...
  connect(regulation_modularBoiler.PLRset,
    hierarchicalControl_modularBoilerNEW1.PLRin) annotation (Line(points={{-42,59.6},{-20,59.6},{
          -20,55.8},{0,55.8}},                   color={0,0,127}));
  ...
  connect(hierarchicalControl_modularBoilerNEW1.PLRset, heatGeneratorNoControl.PLR)
    annotation (Line(points={{20.2,56},{30,56},{30,16},{-16,16},{-16,5.4},{-10,5.4}},
                 color={0,0,127}));
  connect(hierarchicalControl_modularBoilerNEW1.PLRset,
    regulation_modularBoiler.PLRMea) annotation (Line(points={{20.2,56},{30,56},{30,32},{-68,32},
          {-68,49.4},{-62,49.4}},                 color={0,0,127}));
...
<p>The control model is build up hierarchically. The emergency swith precedes all types of regulation. Furthermore, the user has possibility to use an extern control which outpus has to be the PLR (partial load ratio). It is also possible to set a minimum running time for the heat generator, during which it runs continuously and can only be switched off by the emergency stop switch. Further Information on the individual parameters at <a href=\"ControlUnity.hierarchicalControl_modular\">HierarchicalControl</a>. </p>
</html>"),
