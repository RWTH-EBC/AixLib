...
model Admixture
  ...
  AixLib.Fluid.FixedResistances.PressureDrop res(
    ...
    allowFlowReversal=false,                     m_flow_nominal=m_flow_nominalCon, dp_nominal=
...
<p>This model represents a simplified curcuit for a admixture control. It includes a pipe with pressure loss and a pump that is parametrized via a characteristic diagram. The mixing of the flow and return is realized by a equal percentage three-way valve. All important interfaces are realized via the bus connector. </p>
</html>"));
end Admixture;
