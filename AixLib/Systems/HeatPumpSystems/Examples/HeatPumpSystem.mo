...
model HeatPumpSystem "Example for a heat pump system"
  ...
  Modelica.Blocks.Sources.CombiTimeTable timTab(
    ...
    table=[-6*3600,0; 8*3600,4000; 18*3600,0])
                          "Time tabl, Tolerance=1e-06e for internal heat gain"
  ...
  AixLib.Systems.HeatPumpSystems.HeatPumpSystem heatPumpSystem(
    ...
    use_minRunTime=true, Tolerance=1e-06,
    minRunTime(disp, Tolerance=1e-06layUnit="min"),
    use_minLocTime=true, Tolerance=1e-06,
    minLocTime(disp, Tolerance=1e-06layUnit="min"),
    use_runPerHou=true,
    pre_n_start=false,
    use_antLeg=false,
    use_refIne=true,
    initType=Modelica.Blocks.Types.Init.InitialOutput,
    minTimeAntLe, Tolerance=1e-06g(displayUnit="min") = 900,
    scalingFactor=1,
