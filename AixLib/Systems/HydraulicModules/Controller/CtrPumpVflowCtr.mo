...
block CtrPumpVflowCtr "Controller for unmixed circuit with valve"
  //Boolean choice;
  ...
  Modelica.Blocks.Sources.Constant constVflowSet1(final k=1) if       not useExternalVset annotation (Placement(transformation(extent={{16,-6},
            {36,14}})));
equation
...
equation
  ...
  connect(PID.y, hydraulicBus.pumpBus.rpmSet) annotation (Line(points={{5,-50},
          {48,-50},{48,0.12},{100.12,0.12}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  connect(constVflowSet1.y, hydraulicBus.valveSet) annotation (Line(points={{37,
          4},{68,4},{68,0.12},{100.12,0.12}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
