...
package CCCS "calculating the overall costs  according to CCCS evaluation method to evaluate the performance of control strategies"
  ...
  model EnergyCosts "calculating the energy costs as part of the operational costs to evaluate the performance of a control strategy according to CCCS evaluation method"
    ...
    Modelica.Blocks.Math.Gain CostFactorDistrictHeating(k=49.41)
    ...
    Modelica.Blocks.Sources.Constant Constant(k=-30) annotation (Placement(
          visible = true, transformation(origin = {-114, 75}, extent = {{-10, -11}, {10, 11}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant FixedCostsConnectionDistrictHeating(k=1690)
      annotation (Placement(visible = true, transformation(extent = {{-18, 94}, {2, 114}}, rotation = 0)));
    Modelica.Blocks.Math.Gain CostFactorConnectionDistrictHeating(k=27.14)
      annotation (Placement(transformation(extent={{-48,62},{-28,82}})));
    Modelica.Blocks.Math.Gain CostFactorDistrictCooling(k=81)
      annotation (Placement(visible = true, transformation(extent = {{-130, 32}, {-110, 52}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant FixedCostsConnection_DistrictCooling(k=1690)
      annotation (Placement(visible = true, transformation(extent = {{-46, -14}, {-26, 6}}, rotation = 0)));
    Modelica.Blocks.Math.Gain CostFactorConnection_DistrictCooling(k=27.14)
      annotation (Placement(visible = true, transformation(extent = {{-42, 16}, {-22, 36}}, rotation = 0)));
    Modelica.Blocks.Math.Gain CostFactorElectricity(k=235)
      annotation (Placement(visible = true, transformation(extent = {{12, -46}, {32, -26}}, rotation = 0)));
    Modelica.Blocks.Interfaces.RealOutput EnergyCost
      annotation (Placement(transformation(extent={{100,-10},{120,10}})));
    Modelica.Blocks.Math.MultiSum Sum_EnergyCost(nu=3)
      annotation (Placement(visible = true, transformation(extent = {{68, -6}, {80, 6}}, rotation = 0)));
    Modelica.Blocks.Math.MultiSum Cost_DistrictCooling(nu=2)
      annotation (Placement(visible = true, transformation(extent = {{28, 36}, {40, 48}}, rotation = 0)));
    Modelica.Blocks.Math.MultiSum PowerConnection_DistrictCooling(nu=2)
      annotation (Placement(visible = true, transformation(extent = {{-74, 20}, {-62, 32}}, rotation = 0)));
    Modelica.Blocks.Math.MultiSum CostConnection_DistrictCooling(nu=1)
      annotation (Placement(visible = true, transformation(extent = {{0, 20}, {12, 32}}, rotation = 0)));
    Modelica.Blocks.Math.MultiSum PowerConnection_DistrictHeating(nu=2)
      annotation (Placement(transformation(extent={{-76,66},{-64,78}})));
    Modelica.Blocks.Math.MultiSum CostConnectio_DistrictHeating(nu=2)
      annotation (Placement(transformation(extent={{-6,68},{6,80}})));
    Modelica.Blocks.Math.MultiSum Cost_DistrictHeating(nu=2)
  ...
  model EmissionsCosts "calculating the costs for emissions as part of the operational costs to evaluate the performance of a control strategy according to CCCS evaluation method"
    Modelica.Blocks.Math.Gain EmissionsFactorDistrictHeating(k=0.2)
      annotation (Placement(transformation(extent={{-38,24},{-18,44}})));
    Modelica.Blocks.Math.Gain EmissionsFactorDistrictCooling(k=0.527)
      annotation (Placement(transformation(extent={{-40,-10},{-20,10}})));
    Modelica.Blocks.Math.Gain EmissionsFactorElectricity(k= 0.474)
      annotation (Placement(transformation(extent={{-38,-44},{-18,-24}})));
    Modelica.Blocks.Math.Gain CostFactorEmissions(k= 0.0258)
...
BusSystems.Bus_measure bus_measure
  ...
  model LifespanReductionCosts "calculating costs of lifespan reduction due to wear as part of operating costs to evaluate the performance of a control strategy according to CCCS evaluation method"
   ...
   Real n;     
// number of cycles during simulation period
  equation
...
//Overall investment costs
  ...
  equation
    ...
    annotation(
      ...
      __OpenModelica_commandLineOptions = "");end LRM;
  annotation (
