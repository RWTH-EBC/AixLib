...
package CCCS
  ...
  model EnergyCosts "calculating the energy costs as part of the operational costs to evaluate the performance of a control strategy according to CCCS evaluation method"
    ...
    Modelica.Blocks.Sources.Constant EnergyDemandOfDistrictCooling(k=0)
      "Auxiliary Constant - to be replaced by corresponding connection measure bus - CostFactorDistrictCooling in case district cooling is used in model"
      annotation (Placement(visible=true, transformation(
          origin={-154,43},
          extent={{-10,-11},{10,11}},
          rotation=0)));
    Modelica.Blocks.Sources.Constant EnergyDemandForDistrictHeating(k=0)
      "auxiliary constant - to be replaced by corresponding connection measure bus - CostFactorDistrictHeating in case district heating is used in model"
      annotation (Placement(visible=true, transformation(
          origin={-152,111},
          extent={{-10,-11},{10,11}},
          rotation=0)));
    Modelica.Blocks.Sources.Constant NominalPowerOfDistrictHeating(k=0)
      "auxiliary constant - to be replaced by corresponding connection measure bus - PowerConnectionDistrictHeating in case district heating is used in model"
      annotation (Placement(visible=true, transformation(
          origin={-152,77},
          extent={{-10,-11},{10,11}},
          rotation=0)));
    Modelica.Blocks.Sources.Constant NominalPowerOfDistrictCooling(k=0)
      "auxiliary constant - to be replaced by corresponding connection measure bus - PowerConnectionDistrictCooling in case district cooling is used in model"
      annotation (Placement(visible=true, transformation(
          origin={-124,9},
          extent={{-10,-11},{10,11}},
          rotation=0)));
  equation
    connect(bus_measure1.Total_Fuel, CostFactor_Fuel.u) annotation (
      Line(points={{-159.95,0.05},{-153,0.05},{-153,-36},{-154,-36},{-154,-76},
            {12,-76}},                                                                        color = {0, 0, 127}));
    ...
    connect(CostFactorConnection_DistrictCooling.y, CostConnection_DistrictCooling.u[1]) annotation (
      Line(points={{-21,26},{-10,26},{-10,28.1},{0,28.1}},
                                          color = {0, 0, 127}));
    connect(CostFactorDistrictCooling.y, Cost_DistrictCooling.u[1]) annotation (
      Line(points={{-109,42},{-40,42},{-40,44.1},{28,44.1}},
                                            color = {0, 0, 127}));
    ...
    connect(CostFactor_Fuel.y, Sum_EnergyCost.u[4]) annotation (Line(points={{
            35,-76},{68,-76},{68,-3.15}}, color={0,0,127}));
    connect(CostConnection_DistrictCooling.y, Cost_DistrictCooling.u[2])
      annotation (Line(points={{13.02,26},{16,26},{16,24},{28,24},{28,39.9}},
          color={0,0,127}));
    connect(FixedCostsConnection_DistrictCooling.y,
      CostConnection_DistrictCooling.u[2]) annotation (Line(points={{-25,-4},{
            -4,-4},{-4,23.9},{4.44089e-16,23.9}}, color={0,0,127}));
    connect(EnergyDemandOfDistrictCooling.y, CostFactorDistrictCooling.u)
      annotation (Line(points={{-143,43},{-132,43},{-132,42}}, color={0,0,127}));
    connect(EnergyDemandForDistrictHeating.y, CostFactorDistrictHeating.u)
      annotation (Line(points={{-141,111},{-137.5,111},{-137.5,112},{-132,112}},
          color={0,0,127}));
    connect(NominalPowerOfDistrictHeating.y, PowerConnection_DistrictHeating.u[
      2]) annotation (Line(points={{-141,77},{-132,77},{-132,56},{-84,56},{-84,
            69.9},{-76,69.9}}, color={0,0,127}));
    connect(NominalPowerOfDistrictCooling.y, PowerConnection_DistrictCooling.u[
      2]) annotation (Line(points={{-113,9},{-78,9},{-78,23.9},{-74,23.9}},
          color={0,0,127}));
    annotation (
  ...
  model EmissionsCosts "calculating the costs for emissions as part of the operational costs to evaluate the performance of a control strategy according to CCCS evaluation method"
    ...
    Modelica.Blocks.Math.MultiSum Emissions(nu=4)
                                            annotation (
      Placement(visible = true, transformation(origin = {4, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant EnergyDemandForDistrictHeating(k=0)
      "auxiliary constabt - to be replaced by corresponding connection measure bus - EmissionsFactorDistrictHeating in case district heating is used in model"
      annotation (Placement(visible=true, transformation(
          origin={-68,35},
          extent={{-10,-11},{10,11}},
          rotation=0)));
    Modelica.Blocks.Sources.Constant EnergyDemandForDistrictCooling(k=0)
      "auxiliary constant - to be replaced by corresponding connection measure bus - EmissionsFactorDistrictCooling in case district cooling is used in model"
      annotation (Placement(visible=true, transformation(
          origin={-68,-1},
          extent={{-10,-11},{10,11}},
          rotation=0)));
  equation
    ...
    connect(EmissionsFactorDistrictHeating.y, Emissions.u[1]) annotation (Line(
          points={{-17,34},{-10,34},{-10,5.25},{-6,5.25}}, color={0,0,127}));
    connect(EmissionsFactorDistrictCooling.y, Emissions.u[2]) annotation (Line(
          points={{-19,0},{-12,0},{-12,1.75},{-6,1.75}}, color={0,0,127}));
    connect(EmissionsFactorElectricity.y, Emissions.u[3]) annotation (Line(
          points={{-17,-34},{-6,-34},{-6,-1.75}}, color={0,0,127}));
    connect(EmissionsFactorFuel.y, Emissions.u[4]) annotation (Line(points={{
            -17,-70},{-6,-70},{-6,-5.25}}, color={0,0,127}));
    connect(EnergyDemandForDistrictHeating.y, EmissionsFactorDistrictHeating.u)
      annotation (Line(points={{-57,35},{-40,35},{-40,34}}, color={0,0,127}));
    connect(EnergyDemandForDistrictCooling.y, EmissionsFactorDistrictCooling.u)
      annotation (Line(points={{-57,-1},{-54,-1},{-54,0},{-42,0}}, color={0,0,
            127}));
    annotation (
