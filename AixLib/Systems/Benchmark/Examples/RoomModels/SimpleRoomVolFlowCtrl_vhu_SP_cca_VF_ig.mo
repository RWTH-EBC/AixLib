...
model SimpleRoomVolFlowCtrl_vhu_SP_cca_VF_ig
  ...
  AixLib.Systems.EONERC_MainBuilding.Tabs2 tabs4_1(
    ...
    pipe(cPip=500))
    annotation (Placement(transformation(extent={{22,-60},{62,-20}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput TAirRoom "Indoor air temperature"
    annotation (Placement(transformation(extent={{92,54},{112,74}})));
  Controller.CtrTabsVSet ctrTabsVSet(
    k=100,
    rpm_pump=3500,                   ctrThrottleVflowCtr1(Ti=30),
      ctrThrottleVflowCtr(Ti=30))
    annotation (Placement(transformation(extent={{-108,138},{-88,158}})));
  ...
  Modelica.Blocks.Sources.CombiTimeTable internalGains(
    ...
    annotation(Placement(transformation(extent={{8,-13},{22,1}})));
  ...
  ThermalZones.ReducedOrder.ThermalZone.ThermalZone thermalZone1(
    ...
    annotation (Placement(transformation(extent={{4,28},{54,74}})));
  Modelica.Blocks.Interfaces.RealInput mFlowTabsHot
    "Connector of second Real input signal" annotation (Placement(
        transformation(extent={{-164,156},{-142,178}}), iconTransformation(
          extent={{-162,124},{-140,146}})));
  Modelica.Blocks.Interfaces.RealInput mFlowTabsCold
    "Connector of second Real input signal" annotation (Placement(
        transformation(extent={{-164,128},{-142,150}}), iconTransformation(
          extent={{-162,124},{-140,146}})));
  Modelica.Blocks.Interfaces.RealInput TsetAHU
    "Connector of second Real input signal" annotation (Placement(
        transformation(extent={{-162,92},{-140,114}}), iconTransformation(
          extent={{-162,124},{-140,146}})));
equation
  ...
  connect(bouWaterhot1.ports[2], tabs4_1.port_b1) annotation (Line(points={{-16,-76},
          {58,-76},{58,-59.6364}},      color={0,127,255}));
  connect(tabs4_1.port_a1, bouWaterhot.ports[1]) annotation (Line(points={{26,-60},
          {26,-80},{22,-80}},        color={0,127,255}));
  connect(bouWatercold.ports[1], tabs4_1.port_a2) annotation (Line(points={{-8,-116},
          {2,-116},{2,-102},{34,-102},{34,-60}},       color={0,127,255}));
  connect(bouWatercold1.ports[1], tabs4_1.port_b2) annotation (Line(points={{48,-116},
          {48,-59.6364},{50,-59.6364}},   color={0,127,255}));
  ...
  connect(ctrTabsVSet.tabsBus, tabs4_1.tabsBus) annotation (Line(
      points={{-88,148},{-12,148},{-12,-41.6364},{21.8,-41.6364}},
      color={255,204,51},
      thickness=0.5));
  connect(ctrVentilationUnitBasic.genericAHUBus, ventilationUnit1.genericAHUBus)
    annotation (Line(
      points={{-88,108.1},{-72,108.1},{-72,43.25},{-57,43.25}},
      color={255,204,51},
      thickness=0.5));
  ...
  connect(bouWaterhot2.ports[1], ventilationUnit1.port_a4) annotation (Line(
        points={{-50,-118},{-52.4,-118},{-52.4,-12}}, color={0,127,255}));
  connect(bouWatercold.ports[2], ventilationUnit1.port_a3) annotation (Line(
        points={{-4,-116},{-4,-114},{-70.8,-114},{-70.8,-12}}, color={0,127,255}));
  ...
  connect(weaBus, thermalZone1.weaBus) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(ventilationUnit1.port_a2, thermalZone1.ports[1]) annotation (Line(
        points={{-34,28},{28,28},{28,34.44},{23.125,34.44}}, color={0,127,255}));
  connect(ventilationUnit1.port_b1, thermalZone1.ports[2]) annotation (Line(
        points={{-33.54,13},{35.23,13},{35.23,34.44},{34.875,34.44}}, color={0,
          127,255}));
  connect(mFlowTabsHot, ctrTabsVSet.vFlowHotSet) annotation (Line(points={{-153,
          167},{-130.5,167},{-130.5,152.1},{-108.3,152.1}}, color={0,0,127}));
  connect(ctrTabsVSet.vFlowColdSet, mFlowTabsCold) annotation (Line(points={{
          -108.3,143.9},{-126.15,143.9},{-126.15,139},{-153,139}}, color={0,0,
          127}));
  connect(TsetAHU, ctrVentilationUnitBasic.Tset) annotation (Line(points={{-151,
          103},{-131.5,103},{-131.5,108},{-110,108}}, color={0,0,127}));
  annotation (
...
end SimpleRoomVolFlowCtrl_vhu_SP_cca_VF_ig;
