...
record ZoneBaseRecord "the used record standard"
  ...
  parameter Modelica.SIunits.HeatFlowRate Q_flow_heat = 1 "Heat flow rate" annotation(Dialog(tab = "Heater", enable = Heater_on));
  parameter Modelica.SIunits.HeatFlowRate Q_N = 30000
    "nominal heating power of zone" annotation(Dialog(tab = "Heater", enable = Heater_on));
  parameter Real h_heater = 30000 "Upper limit controller output" annotation(Dialog(tab = "Heater", group = "Controller", enable = Heater_on));
  parameter Real l_heater = 0 "Lower limit controller output" annotation(Dialog(tab = "Heater", group = "Controller", enable = Heater_on));
  parameter Real KR_heater = 10000 "Gain of the controller, h_heater/60" annotation(Dialog(tab = "Heater", group = "Controller", enable = Heater_on));
  parameter Modelica.SIunits.Time TN_heater = 1
    "Time constant of the controller, KR_heater*0.15" annotation(Dialog(tab = "Heater", group = "Controller", enable = Heater_on));
  parameter Real weightfactor_heater = 1 "Weightfactor" annotation(Dialog(tab = "Heater", enable = Heater_on));
  parameter Boolean Heater_on = true "if use heater component" annotation(Dialog(tab = "Heater"));
  parameter Modelica.SIunits.HeatFlowRate Q_flow_cooler = 1 "Heat flow rate" annotation(Dialog(tab = "Cooler", enable = Cooler_on));
  parameter Real h_cooler = 0 "Upper limit controller output" annotation(Dialog(tab = "Cooler", group = "Controller", enable = Cooler_on));
  parameter Real l_cooler = -30000 "Lower limit controller output" annotation(Dialog(tab = "Cooler", group = "Controller", enable = Cooler_on));
  parameter Real KR_cooler = 10000 "Gain of the controller, abs(l_cooler/80)" annotation(Dialog(tab = "Cooler", group = "Controller", enable = Cooler_on));
  parameter Modelica.SIunits.Time TN_cooler = 1
    "Time constant of the controller, l_cooler*0.15" annotation(Dialog(tab = "Cooler", group = "Controller", enable = Cooler_on));
  parameter Real weightfactor_cooler = 1 "Weightfactor" annotation(Dialog(tab = "Cooler", enable = Cooler_on));
  parameter Boolean Cooler_on = true "if use chiller component" annotation(Dialog(tab = "Cooler"));
  parameter Integer ActivityTypePeople = 3 "Physical activity" annotation(Dialog(tab = "Inner loads", group = "Persons"), choices(choice = 2 "light", choice = 3
  ...
  parameter Integer ActivityTypeMachines = 2 "Machine activity" annotation(Dialog(tab = "Inner loads", group = "Machines"), choices(choice = 1 "low", choice = 2 "middle", choice = 3 "high", radioButtons = true));
  ...
  parameter Modelica.SIunits.Area RoomArea = 20 "Area of room" annotation(Dialog(tab = "Inner loads", group = "Lighting"));
  ...
  parameter Modelica.SIunits.ThermalResistance R1i = 0.000656956 "Resistor 1" annotation(Dialog(tab = "Inner walls", enable = withInnerwalls));
  parameter Modelica.SIunits.HeatCapacity C1i = 12049200 "Capacity 1" annotation(Dialog(tab = "Inner walls", enable = withInnerwalls));
  parameter Modelica.SIunits.Area Ai = 60.5 "Inner wall area" annotation(Dialog(tab = "Inner walls", enable = withInnerwalls));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaiwi = 2.1
    "Inner wall's coefficient of heat transfer" annotation(Dialog(tab = "Inner walls", enable = withInnerwalls));
  parameter Modelica.SIunits.Emissivity epsi = 0.9 "Emissivity" annotation(Dialog(tab = "Inner walls", enable = withInnerwalls));
  ...
  parameter Modelica.SIunits.ThermalResistance RRest = 0.001717044
    "Resistor Rest" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Modelica.SIunits.ThermalResistance R1o = 0.02045808 "Resistor 1" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Modelica.SIunits.HeatCapacity C1o = 4896650 "Capacity 1" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Modelica.SIunits.Area Ao = 25.5 "Outer wall area" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaowi = 2.7
    "Outer wall's coefficient of heat transfer (inner side)" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaowo = 20
    "Outer wall's coefficient of heat transfer (outer side)" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  ...
  parameter Modelica.SIunits.Emissivity epso = 0.9 "Emissivity" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Real aowo = 0.7
    "Outer wall's coefficient of absorption (outer side)" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  ...
  parameter Modelica.SIunits.TransmissionCoefficient gsunblind[n] = {1, 1, 1, 1}
    "Energy transmittances if sunblind is closed" annotation(Dialog(tab = "Windows", group = "Shading", enable = if withWindows and withOuterwalls then true else false));
  ...
  parameter Modelica.SIunits.Emissivity epsw = 0.9 "Emissivity" annotation(Dialog(tab = "Windows", enable = if withWindows and withOuterwalls then true else false));
  ...
  parameter Boolean withAHU = false
    "Choose if zone is connected to central air handling unit" annotation(Dialog(tab = "Room air"), choices(choice = true "yes", choice = false "no", radioButtons = true));
  ...
  parameter Boolean useConstantACHrate = false
    "Choose if a constant infiltration rate is used" annotation(Dialog(tab = "Room air", group = "ACH"), choices(choice = true "yes", choice = false "no", radioButtons = true));
  parameter Real baseACH = 0.2 "base ACH rate for ventilation controller" annotation(Dialog(tab = "Room air", group = "ACH"));
  parameter Real maxUserACH = 1.0 "additional ACH value for max. user activity"
    annotation(Dialog(tab = "Room air", group = "ACH"));
  parameter Real maxOverheatingACH[2] = {3.0, 2.0}
    "additional ACH value when overheating appears, transition range in K" annotation(Dialog(tab = "Room air", group = "ACH"));
  parameter Real maxSummerACH[3] = {1.0, 273.15 + 10, 273.15 + 17}
    "additional ACH in summer, Tmin, Tmax" annotation(Dialog(tab = "Room air", group = "ACH"));
  parameter Real winterReduction[3]={0.2,273.15,273.15 + 10}
    "reduction factor of userACH for cold weather." annotation (Dialog(tab="Room air", group="ACH"));
