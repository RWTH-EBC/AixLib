...
record ZoneBaseRecord "the used record standard"
  ...
  parameter Modelica.SIunits.Temp_K T0all = 273.15 + 20.1 "Initial temperature for all components" annotation(Dialog(descriptionLabel = false));
  ...
  parameter Modelica.SIunits.HeatFlowRate Q_N = 30000 "nominal heating power of zone" annotation(Dialog(tab = "Heater", enable = Heater_on, descriptionLabel = false));
  ...
  parameter Modelica.SIunits.Time TN_heater = 1 "Time constant of the controller, KR_heater*0.15" annotation(Dialog(tab = "Heater", group = "Controller", enable = Heater_on, descriptionLabel = false));
  ...
  parameter Modelica.SIunits.Time TN_cooler = 1 "Time constant of the controller, l_cooler*0.15" annotation(Dialog(tab = "Cooler", group = "Controller", enable = Cooler_on, descriptionLabel = false));
  ...
  parameter Integer ActivityTypePeople = 3 "Physical activity" annotation(Dialog(tab = "Inner loads", group = "Persons", compact = true, descriptionLabel = false), choices(choice = 2 "light", choice = 3 "moderate", choice = 4 "heavy ", radioButtons = true));
  ...
  parameter Real RatioConvectiveHeatPeople = 0.5 "Ratio of convective heat from overall heat output" annotation(Dialog(tab = "Inner loads", group = "Persons", descriptionLabel = false));
  ...
  parameter Real RatioConvectiveHeatMachines = 0.6 "Ratio of convective heat from overall heat output" annotation(Dialog(tab = "Inner loads", group = "Machines", descriptionLabel = false));
  ...
  parameter Real RatioConvectiveHeatLighting = 0.5 "Ratio of convective heat from overall heat output" annotation(Dialog(tab = "Inner loads", group = "Lighting", descriptionLabel = false));
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaiwi = 2.1 "Inner wall's coefficient of heat transfer" annotation(Dialog(tab = "Inner walls", descriptionLabel = false, enable = withInnerwalls));
  ...
  parameter Boolean withOuterwalls = true "If outer walls (including windows) are existent" annotation(Dialog(tab = "Outer walls"));
  parameter Modelica.SIunits.ThermalResistance RRest = 0.001717044 "Resistor Rest" annotation(Dialog(tab = "Outer walls", descriptionLabel = false, enable = withOuterwalls));
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaowi = 2.7 "Outer wall's coefficient of heat transfer (inner side)" annotation(Dialog(tab = "Outer walls", descriptionLabel = false, enable = withOuterwalls));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer alphaowo = 20 "Outer wall's coefficient of heat transfer (outer side)" annotation(Dialog(tab = "Outer walls", descriptionLabel = false, enable = withOuterwalls));
  ...
  parameter Real aowo = 0.7 "Outer wall's coefficient of absorption (outer side)" annotation(Dialog(tab = "Outer walls", descriptionLabel = false, enable = withOuterwalls));
  parameter Integer n = 4 "Number of orientations (without ground)" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Real weightfactorswall[n] = {0.5, 0.2, 0.2, 0.1} "Weightfactors of the walls (Aj/Atot)" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Real weightfactorground = 0 "Weightfactor of the earth (0 if not considered)" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Modelica.SIunits.Temp_K temperatureground = 286.15 "Temperature of the earth" annotation(Dialog(tab = "Outer walls", enable = withOuterwalls));
  parameter Boolean withWindows = true "If windows are existent" annotation(Dialog(tab = "Windows", enable = withOuterwalls));
  parameter Real splitfac = 0 "part of convective heat transfer through windows" annotation(Dialog(tab = "Windows", enable = withOuterwalls));
  ...
  parameter Modelica.SIunits.TransmissionCoefficient gsunblind[n] = {1, 1, 1, 1} "Energy transmittances if sunblind is closed" annotation(Dialog(tab = "Windows", group = "Shading", descriptionLabel = false, enable = if withWindows and withOuterwalls then true else false));
  parameter Modelica.SIunits.RadiantEnergyFluenceRate Imax = 200 "Intensity at which the sunblind closes" annotation(Dialog(tab = "Windows", group = "Shading", descriptionLabel = false, enable = if withWindows and withOuterwalls then true else false));
  parameter Real weightfactorswindow[n] = {0, 0, 0, 0} "Weightfactors of the windows" annotation(Dialog(tab = "Windows", enable = if withWindows and withOuterwalls then true else false));
  ...
  parameter Modelica.SIunits.TransmissionCoefficient g = 0.7 "Total energy transmittance" annotation(Dialog(tab = "Windows", descriptionLabel = false, enable = if withWindows and withOuterwalls then true else false));
  parameter Modelica.SIunits.Length Lchar = 12 "characterisic length of building (longer side)";
  parameter Modelica.SIunits.Length Bchar = 11 "characterisic width of building (smaller side)";
  ...
  parameter Boolean withAHU = false "Choose if zone is connected to central air handling unit" annotation(Dialog(tab = "Room air", descriptionLabel = false), choices(choice = true "yes", choice = false "no", radioButtons = true));
  parameter Real minAHU(unit = "m3/(h.m2)") = 0 "Minimum specific air flow supplied by the AHU" annotation(Dialog(tab = "Room air"));
  parameter Real maxAHU(unit = "m3/(h.m2)") = 4 "Maximum specific air flow supplied by the AHU" annotation(Dialog(tab = "Room air"));
  parameter Boolean useConstantACHrate = false "Choose if a constant infiltration rate is used" annotation(Dialog(tab = "Room air", group = "ACH", descriptionLabel = false), choices(choice = true "yes", choice = false "no", radioButtons = true));
  ...
  parameter Real maxUserACH = 1.0 "additional ACH value for max. user activity" annotation(Dialog(tab = "Room air", group = "ACH", descriptionLabel = false));
  parameter Real maxOverheatingACH[2] = {3.0, 2.0} "additional ACH value when overheating appears, transition range in K" annotation(Dialog(tab = "Room air", group = "ACH", descriptionLabel = false));
  parameter Real maxSummerACH[3] = {1.0, 273.15 + 10, 273.15 + 17} "additional ACH in summer, Tmin, Tmax" annotation(Dialog(tab = "Room air", group = "ACH", descriptionLabel = false));
