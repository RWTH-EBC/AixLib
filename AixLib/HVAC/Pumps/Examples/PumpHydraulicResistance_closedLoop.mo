within AixLib.HVAC.Pumps.Examples;

model PumpHydraulicResistance_closedLoop "Example with pump, hydraulic resistance and pipes in a closed loop"
  import AixLib;
  extends Modelica.Icons.Example;
  Pump pump(V_flow(fixed = false), MinMaxCharacteristics = AixLib.DataBase.Pumps.Pump1(), V_flow_max = 2, ControlStrategy = 2) annotation(Placement(transformation(extent = {{-40, 20}, {-20, 40}})));
  HydraulicResistances.HydraulicResistance hydraulicResistance(zeta = 2) annotation(Placement(transformation(extent = {{26, 20}, {46, 40}})));
  Pipes.StaticPipe pipe(l = 10, D = 0.01) annotation(Placement(transformation(extent = {{-4, 20}, {16, 40}})));
  Pipes.StaticPipe pipe1(l = 10, D = 0.01) annotation(Placement(transformation(extent = {{-12, -20}, {-32, 0}})));
  ...
  Sources.Boundary_p PointFixedPressure(use_p_in = false) annotation(Placement(transformation(extent = {{-100, 20}, {-80, 40}})));
equation
