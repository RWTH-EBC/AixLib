...
model PumpRadiatorThermostaticValve
  ...
  Pumps.Pump pump(MinMaxCharacteristics = AixLib.DataBase.Pumps.Pump1(), V_flow_max = 2, ControlStrategy = 2, V_flow(fixed = false),
  ...
  Valves.ThermostaticValve simpleValve(Influence_PressureDrop = 0.15, Kvs = 0.4, Kv_setT = 0.12,                    leakageOpening = 0,
    redeclare package Medium = Medium,
    m_flow_small=1e-4,
    dp(start=20000))                                                                                                     annotation(Placement(transformation(extent = {{32, 10}, {52, 30}})));
  Radiator radiator(RadiatorType = AixLib.DataBase.Radiators.ThermX2_ProfilV_979W(),
    redeclare package Medium = Medium,
    m_flow_nominal=0.01)                                                             annotation(Placement(transformation(extent = {{112, 10}, {134, 30}})));
