...
function m_flow_of_dp "Calculate mass flow rate as function of pressure drop due to friction"
  input Modelica.SIunits.Pressure dp "Pressure loss due to friction (dp = port_a.p - port_b.p)";
...
algorithm
  ...
  lambda2 := abs(dp) * 2 * diameter ^ 3 * rho / (length * mu * mu) "Known as lambda2=f(dp)";
