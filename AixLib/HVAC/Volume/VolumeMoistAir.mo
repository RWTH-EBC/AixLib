...
model VolumeMoistAir "Model of a moist Air volume with heat port"
  ...
  Modelica.SIunits.Temperature T(start = T0, nominal = T0) "Temperature inside the CV in K";
  ...
  Modelica.SIunits.Pressure p_Saturation "Saturation Pressure of Steam in Air in CV";
  ...
  Real X(min = 0) "mass fractions of water (liquid and steam) to dry air m_w/m_a in CV";
  Real X_Steam(min = 0) "mass fractions of steam to dry air m_w/m_a in CV";
  Real X_Water(min = 0) "mass fractions of liquid water to dry air m_w/m_a in CV";
  Real X_Saturation(min = 0) "saturation mass fractions of water to dry air m_w/m_a in CV";
  parameter Boolean useTstart = true "true if volume temperature should be initialized";
protected
  parameter Modelica.SIunits.Density rho_Water = baseParameters.rho_Water "Density of water";
  parameter Modelica.SIunits.SpecificHeatCapacity cp_Water = baseParameters.cp_Water "Specific heat capacity";
  // PARAMETERS FOR STEAM:
  parameter Modelica.SIunits.MolarMass M_Steam = baseParameters.M_Steam "Molar Mass of Steam";
  parameter Modelica.SIunits.SpecificHeatCapacity cp_Steam = baseParameters.cp_Steam "Specific heat capacity of Steam";
  parameter Modelica.SIunits.SpecificEnthalpy r_Steam = baseParameters.r_Steam "Specific enthalpy of vapoisation";
  // PARAMETERS FOR AIR:
  parameter Modelica.SIunits.MolarMass M_Air = baseParameters.M_Air "Molar Mass of Dry Air";
  parameter Modelica.SIunits.SpecificHeatCapacity cp_Air = baseParameters.cp_Air "Specific heat capacity of Dry Air";
...
equation
  ...
  mX_flow_a = portMoistAir_a.m_flow * actualStream(portMoistAir_a.X_outflow) "water mass flow";
  mX_flow_b = portMoistAir_b.m_flow * actualStream(portMoistAir_b.X_outflow) "water mass flow";
  mX = V * rho_Air * X;
  der(mX) = mX_flow_a + mX_flow_b;
  heatPort.T = T;
  annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics), Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {170, 255, 255}, fillPattern = FillPattern.Sphere, fillColor = {170, 255, 255})}), Documentation(info = "<html>
