...
model SpeedControlOfFan "Fan Speed Control Example"
  ...
  Sources.BoundaryMoistAir_phX boundaryMoistAir_phX2(use_p_in = false, p = 100000) annotation(Placement(transformation(extent = {{-10, 10}, {10, -10}}, rotation = 180, origin = {50, 0})));
  FanSimple fanSimple(UseRotationalSpeedInput = true) annotation(Placement(transformation(extent = {{-30, 10}, {-10, 30}})));
  Ductwork.VolumeFlowController volumeFlowController(D = 0.3) annotation(Placement(transformation(extent = {{6, 25}, {26, 15}})));
  Modelica.Blocks.Sources.RealExpression V_dot_set(y = 0.4) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 180, origin = {16, 0})));
  Modelica.Blocks.Sources.RealExpression n_relative(y = 0.6) annotation(Placement(transformation(extent = {{-52, 26}, {-32, 46}})));
  Sources.BoundaryMoistAir_phX boundaryMoistAir_phX1(use_p_in = false, h = 1e3) annotation(Placement(transformation(extent = {{-72, -12}, {-52, 8}})));
