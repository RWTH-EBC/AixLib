...
model GroundfloorHeatPump
  ...
  HeatGeneration.HeatPump heatPump(volumeCondenser(T(start = 323.15, fixed = true)), tablePower = [0.0, 273.15, 283.15; 308.15, 1100, 1150; 328.15, 1600, 1750], tableHeatFlowCondenser = [0.0, 273.15, 283.15; 308.15, 4800, 6300; 328.15, 4400, 5750]) annotation(Placement(transformation(extent = {{-38, -30}, {-14, -4}})));
  HeatGeneration.Utilities.FuelCounter fuelCounter annotation(Placement(transformation(extent = {{-23, -46}, {-7, -34}})));
  Pumps.Pump pump1 annotation(Placement(transformation(extent = {{-56, -1}, {-42, -15}})));
  Pipes.StaticPipe pipe(D = 0.01, l = 2) annotation(Placement(transformation(extent = {{-80, -17}, {-62, 1}})));
  Sources.Boundary_ph boundary_ph(h = 4184 * 8) annotation(Placement(transformation(extent = {{-100, -15}, {-86, -1}})));
  Sources.Boundary_ph boundary_ph1 annotation(Placement(transformation(extent = {{-100, -33}, {-86, -19}})));
...
equation
  ...
  connect(boundary_ph.port_a, pipe.port_a) annotation(Line(points = {{-86, -8}, {-80, -8}}, color = {0, 127, 255}, smooth = Smooth.None));
  connect(heatPump.port_b_source, boundary_ph1.port_a) annotation(Line(points = {{-36.8, -26.1}, {-80, -26.1}, {-80, -26}, {-86, -26}}, color = {0, 127, 255}, smooth = Smooth.None));
