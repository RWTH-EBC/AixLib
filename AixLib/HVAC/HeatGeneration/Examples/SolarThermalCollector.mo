...
model SolarThermalCollector
  ...
  SolarThermal solarThermal(A = 2, Collector = AixLib.DataBase.SolarThermal.VacuumCollector(),
    redeclare package Medium = Medium,
    m_flow_nominal=0.01)                                                                       annotation(Placement(transformation(extent = {{0, -10}, {20, 10}})));
...
equation
  ...
  connect(tempAndRad.T_out, solarThermal.T_air) annotation(Line(points={{4,75.4},
          {4,10.8}},                                                                                        color = {0, 0, 127}, smooth = Smooth.None));
