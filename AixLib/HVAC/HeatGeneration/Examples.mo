...
package Examples
  ...
  package Verification "Verification Examples"
    extends Modelica.Icons.ExamplesPackage;

    model Test_HeatPump
      import HVAC;
     extends Modelica.Icons.Example;
      Modelica.Blocks.Sources.BooleanPulse    booleanConstant(period=1000)
      ...
      Modelica.Blocks.Sources.Pulse mFlow_Co_pulse(
        amplitude=0.5,
        period=1000,
        offset=0,
        startTime=0,
    ...
    equation
      connect(TsuSourceRamp.y, boundary.T_in)    annotation (Line(
          points={{-59,10},{-54,10},{-54,18},{-46,18}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(mFlow_Co_pulse.y, boundary3.m_flow_in)   annotation (Line(
          points={{-59,-50},{20,-50},{20,-40}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(boundary2.ports[1], temperature.port_b) annotation (Line(
          points={{76,14},{62,14}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(temperature.T, T_Co_out) annotation (Line(
          ...
          color={0,0,127},
          smooth=Smooth.None));
      connect(boundary.ports[1], heatPump.port_Ev_in) annotation (Line(
          points={{-24,14},{-14,14},{-14,17},{-4,17}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(boundary1.ports[1], heatPump.port_Ev_out) annotation (Line(
          points={{-26,-8},{-16,-8},{-16,3},{-4,3}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(boundary3.ports[1], heatPump.port_Co_in) annotation (Line(
          points={{40,-48},{32,-48},{32,3},{22,3}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(temperature.port_a, heatPump.port_Co_out) annotation (Line(
          points={{42,14},{34,14},{34,17},{22,17}},
          color={0,127,255},
          smooth=Smooth.None));
      connect(heatPump.Pel_out, Pel) annotation (Line(
          points={{4,1},{4,-10},{110,-10}},
          color={0,0,127},
          smooth=Smooth.None));
      connect(booleanConstant.y, heatPump.OnOff_in) annotation (Line(
