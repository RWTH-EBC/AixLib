...
model SolarThermal "Model of a solar thermal panel"
  ...
  Modelica.Blocks.Interfaces.RealInput Irradiation "Solar irradiation on a horizontal plane in W/m2" annotation(Placement(transformation(extent = {{-20, -20}, {20, 20}}, rotation = 270, origin = {10, 108})));
  ...
  Utilities.SolarThermalEfficiency solarThermalEfficiency(Collector = Collector) annotation(Placement(transformation(extent = {{-76, 48}, {-56, 68}})));
...
equation
  connect(T_in.signal, solarThermalEfficiency.T_col) annotation(Line(points = {{-70, 10}, {-70, 26}, {-71, 26}, {-71, 47.4}}, color = {0, 0, 127}, smooth = Smooth.None));
  ...
  connect(prescribedHeatFlow.port, volume.heatPort) annotation(Line(points = {{0, 24}, {0, 10}}, color = {191, 0, 0}, smooth = Smooth.None));
