...
partial model RecuperatorNoMediumVarcp "recuperator model with selectable flow arrangement and variable cp for integration with media model."
  parameter Integer flowType = 3 "Flow type" annotation(choices(choice = 1 "counter", choice = 2 "co", choice = 3 "cross", radioButtons = true));
  parameter Modelica.SIunits.Temperature T1in0 = 273.15 + 5 "Medium 1 inlet temperature at design point";
  parameter Modelica.SIunits.Temperature T1out0 = 295.495 "Medium 1 inlet temperature at design point";
  parameter Modelica.SIunits.Temperature T2in0 = 273.15 + 26 "Medium 2 inlet temperature at design point";
  parameter Modelica.SIunits.Temperature T2out0 = 282.0586 "Medium 2 inlet temperature at design point";
  parameter Modelica.SIunits.MassFlowRate m_flow10 = 0.079866 "mass flow rate at design point";
  parameter Modelica.SIunits.MassFlowRate m_flow20 = 0.079883 "mass flow rate at design point";
  final parameter Modelica.SIunits.ThermalConductance C10 = m_flow10 * 1011 "Heat capacity flow rate of medium 1 at design point";
  final parameter Modelica.SIunits.ThermalConductance C20 = m_flow20 * 1011 "Heat capacity flow rate of medium 2 at design point";
  final parameter Real epsilon0 = C10 / min(C10, C20) * (T1out0 - T1in0) / (T2in0 - T1in0) "Heat exchanger characterisic in design point";
  final parameter Real Z0 = min(C10, C20) / max(C10, C20) "heat capacity flow ratio";
  parameter Real expo = 0.78 "exponent. h = 7.2 * v^0.78 [W/(m2.K)]. 5 <= v <= 30 m/s. See Jurges 1924.";
  final parameter Real r = (m_flow10 * (T1in0 + T1out0) / 2 / (m_flow20 * (T2in0 + T2out0) / 2)) ^ expo "(hA)10/(hA)20 ";
  Real NTU0(start = 4.57158) "Number of transfer units at design point";
  discrete Modelica.SIunits.ThermalConductance UA0 "U * A value at design condition";
  Real NTU(nominal = 4.57158) "Number of transfer units";
  Real epsilon(nominal = epsilon0, start = epsilon0) "Heat exchanger characteristic";
  ...
  Modelica.SIunits.SpecificHeatCapacity cp1(min = 900, max = 1300, start = 1011, nominal = 1011) "Specific heat capacity of medium 1. Will be provided by media model";
  Modelica.SIunits.SpecificHeatCapacity cp2(min = 900, max = 1300, start = 1011, nominal = 1011) "Specific heat capacity of medium 2. Will be provided by media model";
equation
  when initial() then
    UA0 = NTU0 * min(C10, C20);
  end when;
  if flowType == 1 then
    NTU0 = if abs(1 - Z0) < Modelica.Constants.eps then epsilon0 / (1 - epsilon0) else 1 / (Z0 - 1) * log((1 - epsilon0) / (1 - epsilon0 * Z0)) "counter-current flow";
  elseif flowType == 2 then
    NTU0 = -log((-epsilon0) - epsilon0 * Z0 + 1) / (Z0 + 1) "co-current flow";
  else
    exp((exp(-NTU0 ^ 0.78 * Z0) - 1) / (Z0 * NTU0 ^ (-0.22))) = 1 - epsilon0 "cross flow (no explicit formula available)";
  end if;
  ...
  NTU = UA / min(C1, C2);
  if flowType == 1 then
    epsilon = if abs(1 - Z) < Modelica.Constants.eps then 1 / (1 + 1 / NTU) else (1 - exp(-NTU * (1 - Z))) / (1 - Z * exp(-NTU * (1 - Z))) "counter-current flow";
  elseif flowType == 2 then
    epsilon = (1 - exp(-NTU * (1 + Z))) / (1 + Z) "co-current flow";
  else
    epsilon = 1 - exp((exp(-NTU * Z * NTU ^ (-0.22)) - 1) / (Z * NTU ^ (-0.22))) "cross flow";
  end if;
