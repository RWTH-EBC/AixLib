...
model Aeration
  extends
    AixLib.Building.Components.Ventilation.BaseClasses.PartialVentilationOutside;

  parameter Boolean withFan = true;
  parameter Boolean withInfiltration = true;
  FanUnit fanUnit(redeclare package Medium = Medium, V_flow_nom=V_flow_nom, DV_flowDp=DV_flowDp) if withFan
    annotation (Placement(transformation(extent={{-10,40},{10,60}})));
  Infiltration infiltration(redeclare package Medium = Medium, q50=q50, wall_length=wall_length, wall_height=wall_height) if withInfiltration
...
equation
  ...
  connect(fanUnit.port_b, port_b) annotation (Line(points={{10,50},{50,50},
            {50,0},{100,0}},
                           color={0,127,255}));
  connect(WindSpeedPort, fanUnit.WindSpeedPort) annotation (Line(points={{-106,0},
            {-60,0},{-60,50},{-10.6,50}},        color={0,0,127}));
  connect(WindDirectionPort, fanUnit.WindDirectionPort) annotation (
      Line(points={{-106,-40},{-50,-40},{-50,46},{-10.6,46}}, color={0,0,127}));
  end if;
  ...
  connect(infiltration.port_b, port_b) annotation (Line(points={{10,-50},{50,-50},
            {50,0},{100,0}},
                           color={0,127,255}));
  connect(WindSpeedPort, infiltration.WindSpeedPort) annotation (Line(points={{-106,0},
            {-60,0},{-60,-50},{-10.6,-50}},  color={0,0,127}));
  connect(WindDirectionPort, infiltration.WindDirectionPort) annotation (Line(
        points={{-106,-40},{-50,-40},{-50,-54},{-10.6,-54}}, color={0,0,127}));
  end if;
