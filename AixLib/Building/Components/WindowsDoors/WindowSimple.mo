...
model WindowSimple "Window with radiation and U-Value"
  ...
  parameter DataBase.WindowsDoors.Simple.OWBaseDataDefinition_Simple WindowType = DataBase.WindowsDoors.Simple.WindowSimple_EnEV2009() "Window type" annotation(Dialog(group = "Window type", enable = selectable, descriptionLabel = true));
  parameter Real frameFraction(max = 1.0) = if selectable then WindowType.frameFraction else 0.2 "Frame fraction" annotation(Dialog(group = "Window type", enable = not selectable, descriptionLabel = true));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer Uw = if selectable then WindowType.Uw else 1.50 "Thermal transmission coefficient of whole window" annotation(Dialog(group = "Window type", enable = not selectable));
  parameter Real g = if selectable then WindowType.g else 0.60 "Coefficient of solar energy transmission" annotation(Dialog(group = "Window type", enable = not selectable));
  Utilities.Interfaces.SolarRad_in solarRad_in annotation(Placement(transformation(extent = {{-100, 50}, {-80, 70}}, rotation = 0)));
  Utilities.HeatTransfer.SolarRadToHeat RadCondAdapt(coeff = g, A = windowarea * (1 - frameFraction)) annotation(Placement(transformation(extent = {{-50, 52}, {-30, 72}}, rotation = 0)));
  Utilities.HeatTransfer.HeatToStar twoStar_RadEx(Therm(T(start = T0)), Star(T(start = T0)), A = (1 - frameFraction) * windowarea, eps = WindowType.Emissivity) annotation(Placement(transformation(extent = {{30, 50}, {50, 70}}, rotation = 0)));
...
equation
  connect(RadCondAdapt.heatPort, twoStar_RadEx.Therm) annotation(Line(points = {{-31, 60}, {30.8, 60}}));
  connect(solarRad_in, RadCondAdapt.solarRad_in) annotation(Line(points = {{-90, 60}, {-50.1, 60}}, color = {0, 0, 0}));
