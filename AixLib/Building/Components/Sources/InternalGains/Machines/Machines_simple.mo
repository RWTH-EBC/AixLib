...
<p>
  Heat source with convective and radiative component. The load is determined by a power input signal.
</p>
<p>
...
<p>
  The input signal can take values from 0 to an arbitrary maximum value.
</p>
<h4>
  <span style=\"color:#008000\">Known limitation</span>
</h4>
<p>
  The parameter A cannot be set by default since other models must be able to implement their own equations for A. For example in the model <a href=\"Building.Components.Sources.InternalGains.Machines.Machines_Avar\">Machines_Avar</a> a changing radiative area is implemented.
</p>
<h4>
  <span style=\"color:#008000\">Assumptions</span>
</h4>
<p>
  The surface for radiation exchange is computed from the schedule, which leads to a surface area of zero, when no activity takes place. In particular cases this might lead to an error as depending of the rest of the system a division by this surface will be introduced in the system of equations -&gt; division by zero. For this reason a lower limitation of 1e-4 m2 has been introduced.
</p>
<h4>
