...
equation
  ...
  cos_theta_z_help = sin(InDeclinationSun)*sin(
    Latitude) + cos(InDeclinationSun)*cos(Latitude)*
    cos(InHourAngleSun);
  ...
  term = cos(InDeclinationSun)*sin(Tilt)*sin(
    Azimut)*sin(InHourAngleSun);
  cos_theta_help = sin(InDeclinationSun)*sin(Latitude)
    *cos(Tilt) - sin(InDeclinationSun)*cos(
    Latitude)*sin(Tilt)*cos(Azimut) + cos(
    InDeclinationSun)*cos(Latitude)*cos(Tilt)
    *cos(InHourAngleSun) + cos(InDeclinationSun)*sin(Latitude)*sin(Tilt)*cos(Azimut)*cos(InHourAngleSun) + term;
  ...
  OutTotalRadTilted.I = max(0, R*InBeamRadHor + 0.5*(1 + cos(
  ...
  OutTotalRadTilted.I_diff = 0.5*(1 + cos(Tilt))*InDiffRadHor;
  OutTotalRadTilted.I_gr = GroundReflection*(InBeamRadHor + InDiffRadHor)*((1 - cos(Tilt))/2);
