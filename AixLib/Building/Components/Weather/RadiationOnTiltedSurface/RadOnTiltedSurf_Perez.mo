...
<p>
  The <b>RadOnTiltedSurf</b> model uses output data of the <b><a href=\"AixLib.Building.Components.Weather.BaseClasses.Sun\">Sun</a></b> model and weather data weather data (beam and diffuse radiance on a horizontal surface for TRY format, or beam normal and global horizontal for TMY format) to compute total and beam radiance on a tilted surface.
</p>
<p>
  Input: It needs information on the height, the tilt angle and the azimut angle of the surface, the latitude of the location and the ground reflection coefficient.
</p>
<p>
  Output: In addition to the calculation of the irradiance on tilted surfaces the model has some output variables that are needed from other in the SunIrradiation model embedded models.
</p>
<h4>
  <span style=\"color:#008000\">Known Limitations</span>
</h4>
<p>
  Be aware that the calculation of the total solar radiation may cause problems at simulation times close to sunset and sunrise, because at this simulation times are probably the maxima of the difference between the calculated incedent-angle and the real angle of incedence at the real irradiation-time.
</p>
<p>
  Another limitation is the equation of the groundreflection approximation. It is only estmated on basis of the <b><a href=\"http://www.nrel.gov/midc/srrl_bms/\">NREL</a></b> (US National Renewable Energy Laboratory; http://www.nrel.gov/midc/srrl_bms) data for the albedo of the year 2011 and should be revised.
...
<ul>
  ...
  <li>
    ...
    Implemented new diffuse irradiation modell from Perez Irradiation on tilted Surfaces.
  </li>
  <li>
