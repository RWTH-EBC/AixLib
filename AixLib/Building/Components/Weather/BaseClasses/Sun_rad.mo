...
model Sun_rad "Computes the sun's altitude of the current site"
...
import Modelica.SIunits.Conversions.to_deg;
  ...
  Real SolarTime;
  Real TimeEquation;
  Modelica.Blocks.Interfaces.RealOutput OutHourAngleSun(unit="rad")
                                                         annotation (Placement(transformation(extent={{80,-36},{100,-16}})));
  Modelica.Blocks.Interfaces.RealOutput OutDeclinationSun(unit="rad")
...
// hour angle of sun, first term calculates local time of day from continuous time signal
      HourAngleSun = (SolarTime-12) * Modelica.Constants.pi/12 "HourAngleSun=0 deg means sun peak";
      if (HourAngleSun > Modelica.Constants.pi) then
        OutHourAngleSun = HourAngleSun - (2*Modelica.Constants.pi);
      elseif (HourAngleSun < -Modelica.Constants.pi) then
        OutHourAngleSun = HourAngleSun + (2*Modelica.Constants.pi);
      else
...
// declination of sun
      DeclinationSun = 0.006918 - 0.399912 * cos(OutDayAngleSun) + 0.070257 * sin(OutDayAngleSun) - 0.006758 * cos(2*OutDayAngleSun) + 0.000907 * sin(2*OutDayAngleSun) - 0.002679 * cos(3*OutDayAngleSun) + 0.00148 * sin(3*OutDayAngleSun);
