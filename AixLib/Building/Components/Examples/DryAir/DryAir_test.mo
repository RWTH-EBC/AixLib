...
model DryAir_test "Simulation to test the dry air models"
  ...
  Components.DryAir.InfiltrationRate_DIN12831 infiltrationRate_DIN12831 annotation(Placement(transformation(extent = {{-12, 12}, {8, 32}})));
...
equation
  ...
  connect(dynamicVentilation.port_inside, airload.port) annotation(Line(points = {{7.4, -5}, {19.5, -5}, {19.5, -4}, {31, -4}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(fixedHeatFlow.port, airload1.port) annotation(Line(points = {{-70, 82}, {-38, 82}, {-38, 78}, {-11, 78}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(TempOutsideDaycurve.port, varAirExchange.port_a) annotation(Line(points = {{-70, 50}, {-41, 50}, {-41, 48}, {-12, 48}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(TempInside.port, varAirExchange.port_b) annotation(Line(points = {{70, 50}, {49, 50}, {49, 48}, {8, 48}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(TempOutsideDaycurve.port, infiltrationRate_DIN12831.port_a) annotation(Line(points = {{-70, 50}, {-50, 50}, {-50, 22}, {-12, 22}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(TempInside.port, infiltrationRate_DIN12831.port_b) annotation(Line(points = {{70, 50}, {40, 50}, {40, 22}, {8, 22}}, color = {191, 0, 0}, smooth = Smooth.None));
  connect(sine.y, TempOutsideDaycurve.T) annotation(Line(points = {{-86.6, 26}, {-92, 26}, {-92, 50}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(sine1.y, varAirExchange.InPort1) annotation(Line(points = {{-23.5, 37}, {-17.75, 37}, {-17.75, 41.6}, {-11, 41.6}}, color = {0, 0, 127}, smooth = Smooth.None));
  connect(TempOutsideDaycurve.port, dynamicVentilation.port_outside) annotation(Line(points = {{-70, 50}, {-50, 50}, {-50, -5}, {-11.6, -5}}, color = {191, 0, 0}, smooth = Smooth.None));
