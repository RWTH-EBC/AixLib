...
model Liu_deg "Liu mit calculation in degrees"
  import AixLib;
  ...
  AixLib.Building.Components.Weather.RadiationOnTiltedSurface.RadOnTiltedSurf_Liu
    radOnTiltedSurf_Liu(Tilt=0, Azimut=0)
  ...
  Modelica.Blocks.Routing.DeMultiplex3 deMultiplex(n3 = 9) annotation(Placement(transformation(extent={{-60,-66},
            {-40,-46}})));
equation
  connect(sun.OutDayAngleSun, radOnTiltedSurf_Liu.InDayAngleSun) annotation (
      Line(points={{-37.4,13.78},{-24.7,13.78},{-24.7,9.48},{-13.48,9.48}},
        color={0,0,127}));
  connect(sun.OutHourAngleSun, radOnTiltedSurf_Liu.InHourAngleSun) annotation (
      Line(points={{-37.4,9.62},{-25.7,9.62},{-25.7,7.08},{-13.48,7.08}}, color
        ={0,0,127}));
  connect(sun.OutDeclinationSun, radOnTiltedSurf_Liu.InDeclinationSun)
  ...
  connect(WeatherData.y,deMultiplex. u) annotation(Line(points={{-73,-56},{-62,
          -56}},                                                                           color = {0, 0, 127}));
  connect(deMultiplex.y2[1], radOnTiltedSurf_Liu.solarInput1) annotation (Line(
        points={{-39,-56},{-26,-56},{-26,38},{-11.32,38},{-11.32,20.76}}, color
        ={0,0,127}));
  connect(deMultiplex.y1[1], radOnTiltedSurf_Liu.solarInput2) annotation (Line(
        points={{-39,-49},{-26,-49},{-26,38},{0.44,38},{0.44,20.76}}, color={0,
          0,127}));
  annotation (
    experiment(
      StopTime=315360000,
