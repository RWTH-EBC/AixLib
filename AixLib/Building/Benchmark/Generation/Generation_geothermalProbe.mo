...
model Generation_geothermalProbe
  ...
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T=284.15)
                                                                                      annotation(Placement(transformation(extent={{-82,6},
            {-74,14}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature1(T=285.15)
                                                                                      annotation(Placement(transformation(extent={{-82,26},
            {-74,34}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature2(T=286.15)
                                                                                      annotation(Placement(transformation(extent={{-82,44},
            {-74,52}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature3(T=287.15)
                                                                                      annotation(Placement(transformation(extent={{-82,62},
            {-74,70}})));
...
equation
  connect(pipe.port_b, Fulid_out_Geothermal)
    annotation (Line(points={{10,-4},{100,-4}}, color={0,127,255}));
  ...
  connect(vol.ports[1], Fluid_in_Geothermal) annotation (Line(points={{-4,-50},
          {48,-50},{48,-46},{100,-46}}, color={0,127,255}));
  connect(pipe.port_a, vol.ports[2]) annotation (Line(points={{-10,-4},{-42,-4},
          {-42,-50},{0,-50}}, color={0,127,255}));
