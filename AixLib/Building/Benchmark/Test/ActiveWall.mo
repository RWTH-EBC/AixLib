...
model ActiveWall
  ...
  parameter AixLib.DataBase.WindowsDoors.Simple.OWBaseDataDefinition_Simple WindowType=
    ...
    "Choose a window type from the database" annotation (Dialog(tab="Window",
        enable=withWindow and outside), choicesAllMatching=true);
  ...
  // Parameter einfügen
  parameter Modelica.SIunits.Length pipe_length = 10;
  parameter Modelica.SIunits.Length pipe_diameter = 0.016;
  parameter Real pipe_thermal_resistance = 0;
  // Initial temperature
  parameter Modelica.SIunits.Temperature T0 = Modelica.SIunits.Conversions.from_degC(20)
    "Initial temperature"                                                                                      annotation(Dialog(tab = "Advanced Parameters"));
  // COMPONENT PART
  AixLib.Building.Components.Walls.BaseClasses.HTWallSel_activeLayer Wall(
    h=wall_height,
    l=wall_length,
    T0=T0,
    clearance=clearance,
    ...
    withActiveLayer=withActiveLayer) "Wall"
    annotation (Placement(transformation(extent={{-20,14},{2,34}})));
  BaseLib.HeatTransfer.SolarRadToHeat SolarAbsorption(coeff=solar_absorptance,
      A=wall_height*wall_length - clearance) if outside annotation (Placement(
        transformation(origin={-39,89}, extent={{-10,-10},{10,10}})));
  ...
  AixLib.Building.Components.Weather.Sunblinds.Sunblind Sunblind(
    n=1,
    gsunblind={Blinding},
  ...
  BaseLib.Interfaces.Adaptors.HeatStarToComb heatStarToComb annotation (
      Placement(transformation(
        extent={{-10,8},{10,-8}},
        rotation=180,
        origin={69,-1})));
  BaseLib.Interfaces.HeatStarComb thermStarComb_inside annotation (Placement(
...
equation
  ...
  connect(port_a1, pipe.port_a)
    annotation (Line(points={{102,46},{40,46},{40,58}}, color={0,127,255}));
  connect(pipe.port_b, port_b1)
    annotation (Line(points={{40,78},{40,90},{102,90}}, color={0,127,255}));
  connect(thermalResistor.port_b, pipe.heatPorts[1]) annotation (Line(points={{26,
          68},{28,68},{28,68.1},{35.6,68.1}}, color={191,0,0}));
