...
model TestCase6
  ...
  BaseClasses.ReducedOrderModel.ReducedOrderModelVDI
                                                  reducedModel(                    Aw = 7, g = 0.15,                                     epsi = 1, epso = 1, T0all(displayUnit = "K") = 295.15, R1i = 0.000595515, Ai = 75.5,                                         Ao = 10.5, withWindows = false,
    C1i=1.48362e+007,
    RRest=0.042768721,
    R1o=0.004367913,
    C1o=1.6008e+006,
    Vair=0.001,
    alphaiwi=2.23642384)                                                                                                     annotation(Placement(transformation(extent = {{54, 26}, {88, 66}})));
...
equation
  ...
  connect(machinesRadiative.port, HeatToStar.Therm) annotation(Line(points = {{22, -89}, {28, -89}, {28, -88}, {40.8, -88}}, color = {191, 0, 0}));
  connect(innerLoads.y[2], machinesRadiative.Q_flow) annotation(Line(points = {{-27, -88}, {-16, -88}, {-16, -89}, {-4, -89}}, color = {0, 0, 127}));
  connect(infiltrationTemp.y, reducedModel.ventilationTemperature) annotation(Line(points = {{16.5, 1}, {21.25, 1}, {21.25, 36.4}, {57.4, 36.4}}, color = {0, 0, 127}));
  connect(infiltrationRate.y, reducedModel.ventilationRate) annotation(Line(points={{40.5,1},
          {40.5,13.5},{64.54,13.5},{64.54,28}},                                                                                           color = {0, 0, 127}));
  connect(HeatToStar.Star, reducedModel.internalGainsRad) annotation(Line(points={{59.1,
          -88},{83.75,-88},{83.75,28}},                                                                                      color = {95, 95, 95}, pattern = LinePattern.Solid));
  connect(outdoorTemp.port, reducedModel.equalAirTemp) annotation(Line(points = {{-4, 49}, {26.5, 49}, {26.5, 46.8}, {57.4, 46.8}}, color = {191, 0, 0}));
  connect(heatFlowSensor.port_a, varTemp.port) annotation(Line(points = {{24, -28}, {14, -28}}, color = {191, 0, 0}));
  connect(setTemp.y[1], varTemp.T) annotation(Line(points = {{-41, -28}, {-8, -28}}, color = {0, 0, 127}));
  connect(heatFlowSensor.port_b, reducedModel.internalGainsConv) annotation(Line(points={{44,-28},
          {74.4,-28},{74.4,28}},                                                                                                color = {191, 0, 0}));
