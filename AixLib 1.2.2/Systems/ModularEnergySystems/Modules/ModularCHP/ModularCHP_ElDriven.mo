...
parameter Modelica.Units.SI.Temperature DesHotT "Design supply temperature";
  ...
  Fluid.Movers.SpeedControlled_y fanHC(
    ...
    per(pressure(V_flow={0,V_flow_HC,V_flow_HC*2}, dp={(5000)/0.8,(5000),0})),
    addPowerToMedium=false) "pump heating circuit" annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-100,-22})));
  ...
  Fluid.Movers.SpeedControlled_y fanCC(
    ...
    per(pressure(V_flow={0,m_flow_nominalCC/1000,m_flow_nominalCC/1000*2}, dp={
            (2500)/0.8,2500,0})),
    addPowerToMedium=false) "pump cooling circuit" annotation (Placement(
        transformation(
        extent={{10,10},{-10,-10}},
        rotation=270,
        origin={-36,-16})));
  ...
  Fluid.Actuators.Valves.ThreeWayEqualPercentageLinear val_Feedback(
    ...
    final m_flow_nominal=m_flow_nominalHC,
    dpValve_nominal=2500,
    fraK=1,
    R=100) annotation (Placement(transformation(extent={{8.5,7.5},{-8.5,-7.5}},
        rotation=-90,
        origin={-97.5,-51.5})));
  ...
  Modelica.Blocks.Sources.RealExpression TMin1(y=DesHotT)
    "MinimalStartTemp"
    annotation (Placement(transformation(extent={{-260,-96},{-206,-60}})));
  Typquartiere.Controls.OnOff
        onOff3(bandwidth_plus=2, bandwidth_minus=2)
    annotation (Placement(transformation(extent={{-64,20},{-44,40}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=1)
    annotation (Placement(transformation(extent={{-262,-64},{-248,-44}})));
protected
...
//   end if;
  ...
  connect(genericCHP.Pel, integrator1.u) annotation (Line(points={{15,9.4},{30,
          9.4},{30,38},{48,38}},
                            color={0,0,127}));
  connect(genericCHP.powerDemand, integrator.u)
    annotation (Line(points={{15,4},{32,4},{32,10},{48,10}}, color={0,0,127}));
  ...
  connect(fanHC.port_b, val_Feedback.port_2) annotation (Line(points={{-100,-32},
          {-100,-38},{-97.5,-38},{-97.5,-43}}, color={0,127,255}));
  connect(val_Feedback.port_1, TColdHeatCircuit.port_a) annotation (Line(points
        ={{-97.5,-60},{-98,-60},{-98,-72},{-56,-72}}, color={0,127,255}));
  ...
  connect(val_Feedback.port_3, THotHeatCircuit1.port_a) annotation (Line(points={{-90,
          -51.5},{-72,-51.5},{-72,-36},{86,-36}},
        color={0,127,255}));
  connect(THotHeatCircuit1.T, PID1.u_m) annotation (Line(points={{75,-26},{42,
          -26},{42,-96},{-164,-96},{-164,-90}}, color={0,0,127}));
  connect(PID1.u_s, TMin1.y)
    annotation (Line(points={{-176,-78},{-203.3,-78}}, color={0,0,127}));
  ...
  connect(cHPControlBus.m_flow_set, fanHC.y) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(realExpression1.y, val_Feedback.y) annotation (Line(points={{-247.3,
          -54},{-222,-54},{-222,-51.5},{-106.5,-51.5}}, color={0,0,127}));
