...
class SyntaxTests(object):
	
	"""Class to Check the HTML Structure in AixLib Modelica Library"""
	
	def __init__(self, moFile ,Package, Library):
		self.moFile = moFile
...
if __name__ == '__main__':    
	...
	check_test_group = parser.add_argument_group("arguments to check Dymola HTML code")
	check_test_group.add_argument("-f",'--files', metavar='f', type=str, nargs='+', help='a file to be processed')
	...
	from view_html_errors_v2 import SyntaxTests
	
	
	for file in args.files:
		HTML_Syntax_Test = SyntaxTests(moFile = file, Package = args.single_package, Library = args.path)
		#print(file)
		#print(type(file))
		print(f"\nProcessing file {file}\n")
		#htmlList = HTML_Syntax_Test.getInfoRevisionsHTML(moFile=file)
		htmlList = HTML_Syntax_Test.getInfoRevisionsHTML()
	
		htmlStr = HTML_Syntax_Test.join_body(htmlList=htmlList, substitutions_dict={'\\"': '"'})
		#htmlStr = HTML_Syntax_Test.join_body()
		print("\n-------- HTML Code --------")
		print(f"\n{number_print_List(htmlList)}")
        
		document_corr, errors = HTML_Syntax_Test.htmlCorrection(htmlStr=htmlStr)
		docCorrStr = HTML_Syntax_Test.number_print_List(document_corr.split('\n'), sep='\n')
		...
		print("\n-------- Errors --------")
		print(f"\n{errors}")
	HTML_Syntax_Test._listAllModel()