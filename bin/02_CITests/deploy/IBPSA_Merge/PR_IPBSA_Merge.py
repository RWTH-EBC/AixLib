...
class PULL_REQUEST_GITHUB(object):
	...
	def post_pull_request(self):
		...
		payload = '{\n    \"title\": \"IBPSA Merge '+self.Working_Branch+'\",\n    \"body\": \"Please pull this branch IBPSA_Merge to your local repository. **Open the AixLib library in dymola and check whether errors may have occurred.** If you need to change bugs or changes to the models of the AixLib, push the changes with the Commit **fix errors manually**. You can output the different reference files between the IBPSA and the AixLib using the CI. To do this, enter the commit **Trigger CI - give different reference results** in a push. If you want to update the reference files using the CI, enter the commit **Trigger CI - Update reference results** during the push. The CI outputs the reference files as artifacts in Gitlab.If the tests in the CI have passed successfully, merge the branch IBPSA_Merge to development branch.  **Delete** the Branch ' +self.Correct_Branch+ '\",\n    \"head\": \"'+self.OWNER+':'+self.Correct_Branch+'\",\n    \"base\": \"'+self.Working_Branch+'\"\n  \n}'
		headers = {
