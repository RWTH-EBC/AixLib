...
model MultizoneEquipped_Rad
  ...
  Modelica.Blocks.Math.MultiSum TOpe[numTabs](
  each k={0.5,0.5,-1},
  each nu=3)
    "Reverses ventilation rate" annotation (Placement(transformation(
        extent={{-7,-7},{7,7}},
        rotation=270,
        origin={159,-2})));
  Modelica.Blocks.Sources.Constant ToKelvin[numTabs](k=273.15)
    "Outdoor coefficient of heat transfer for walls" annotation (Placement(
        transformation(
        extent={{-4,-4},{4,4}},
        rotation=270,
        origin={180,38})));
  Modelica.Blocks.Continuous.LimPID RadPID[numTabs](
    each controllerType=Modelica.Blocks.Types.SimpleController.PI,
    each k=100,
    each Ti=500,
    yMax=TabsHeatLoad,
    each yMin=0)
    annotation (Placement(transformation(extent={{-198,-18},{-186,-6}})));
  Modelica.Blocks.Sources.Constant RefTemp1[numTabs](k=21)
    "Outdoor coefficient of heat transfer for walls" annotation (Placement(
        transformation(
        extent={{-4,-4},{4,4}},
        rotation=0,
        origin={-210,-12})));
  Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor heatFlowSensor[numTabs]
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=90,
        origin={-138,-60})));
  Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor heatFlowSensor1[numTabs]
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=180,
        origin={-116,-30})));
  Fluid.HeatExchangers.Radiators.RadiatorEN442_2 rad[numTabs]
    annotation (Placement(transformation(extent={{-178,-44},{-158,-24}})));
  Fluid.Sources.MassFlowSource_T boundary[numTabs](
    redeclare package Medium = MediumWater,
    use_m_flow_in=true,
    use_T_in=true,
    nPorts=1)
    annotation (Placement(transformation(extent={{-204,-40},{-192,-28}})));
  Fluid.Sources.Boundary_pT bou[numTabs](redeclare package Medium = MediumWater,
      nPorts=1)
    annotation (Placement(transformation(extent={{-142,-38},{-150,-30}})));
protected
  ...
  if ATabs[1] > 0 then
    for i in 1:numTabs loop
      if zoneParam[TabsConnection[i]].ATabs > 0 then
        //connect(HeatTabs[i].port, zone[TabsConnection[i]].tabs) annotation (Line(points={{18,-136},{72,-136},{72,54.33},{45.14,54.33}}, color={191,0,0}));

      end if;
    end for;
  end if;

  connect(ToKelvin.y, TOpe.u[3]) annotation (Line(points={{180,33.6},{180,24},{
          155.733,24},{155.733,5}},
                               color={0,0,127}));
  connect(zone.TAir, TOpe.u[1]) annotation (Line(points={{82.1,85.9},{82.1,90},
          {162.267,90},{162.267,5}},
                               color={0,0,127}));
  connect(zone.TRad, TOpe.u[2]) annotation (Line(points={{82.1,81.8},{82.1,82},{
          159,82},{159,5}},    color={0,0,127}));
  connect(TOpe.y, RadPID.u_m) annotation (Line(points={{159,-10.19},{159,-118},
          {-218,-118},{-218,-20},{-192,-20},{-192,-19.2}}, color={0,0,127}));
  connect(heatFlowSensor.port_a, intGainsConv) annotation (Line(points={{-138,
          -66},{-138,-70},{-100,-70}}, color={191,0,0}));
  connect(heatFlowSensor1.port_a, intGainsRad) annotation (Line(points={{-110,
          -30},{-108,-30},{-108,-40},{-100,-40}}, color={191,0,0}));
  connect(rad.heatPortRad, heatFlowSensor1.port_b) annotation (Line(points={{
          -166,-26.8},{-166,-20},{-134,-20},{-134,-30},{-122,-30}}, color={191,
          0,0}));
  connect(rad.heatPortCon, heatFlowSensor.port_b) annotation (Line(points={{
          -170,-26.8},{-170,-16},{-138,-16},{-138,-54}}, color={191,0,0}));
  connect(RefTemp1.y, RadPID.u_s)
    annotation (Line(points={{-205.6,-12},{-199.2,-12}}, color={0,0,127}));
  connect(boundary.ports[1:1], rad.port_a)
    annotation (Line(points={{-192,-34},{-178,-34}}, color={0,127,255}));
  connect(bou.ports[1:1], rad.port_b)
    annotation (Line(points={{-150,-34},{-158,-34}}, color={0,127,255}));
  connect(RadPID.y, boundary.m_flow_in) annotation (Line(points={{-185.4,-12},{
          -178,-12},{-178,-24},{-214,-24},{-214,-29.2},{-205.2,-29.2}}, color={
          0,0,127}));
